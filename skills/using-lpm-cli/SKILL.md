...
- **Lockfiles are committed:** always commit `lpm.lock` (TOML, diffable). Workspaces keep one root `lpm.lock` and no `lpm.lockb`; successful recursive install migrates legacy member lockfiles into it. For a standalone project, commit `lpm.lockb` when LPM writes it and do not synthesize it for TOML-only graphs.
