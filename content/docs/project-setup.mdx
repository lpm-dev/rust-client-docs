...
For `lpm env push`, malformed `lpm.json` is the deliberate exception: the command warns on stderr and pushes without new schema metadata, so the server retains its last-known-good schema. It never uploads a partially parsed configuration. Commands that need configuration—validation, aliases, inheritance, schema operations, and OIDC policy environment resolution—still fail fast. Configuration-independent local env commands are not blocked by an unrelated malformed file. Oversized or unreadable files remain hard errors rather than being treated as malformed optional metadata.
