...
| --- | --- | --- |
| [`package.json`](#packagejson) | Standard npm fields (dependencies, scripts, engines) **plus** an `"lpm"` block for project-shared LPM behavior (script policy, trust, linker, sandbox, overrides) | Yes |
...
| [`lpm.toml`](#lpmtoml) | Per-project CLI defaults (today: save-prefix, save-exact) — tool behavior, not publishable | Team's call |
...
Optional. Sits next to `package.json` and pins per-project CLI defaults. Today it owns the [save policy](/docs/packages/save-policy):
