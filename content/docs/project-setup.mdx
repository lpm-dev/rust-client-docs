...
Malformed `lpm.json` fails before any project-bound environment operation can mutate local or remote state. That includes local secret operations, named `--env` resolution, validation, aliases, inheritance, schema operations, OIDC policy environment resolution, and `lpm env push`, which needs the complete alias, inheritance, and schema metadata to preserve environment boundaries. Only account-level operations that do not consult project configuration, such as browser pairing and sharing-key rotation, are unaffected by an unrelated malformed file. Oversized or unreadable configuration remains a hard error whenever the command needs it.
