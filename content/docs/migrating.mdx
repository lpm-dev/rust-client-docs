...
Migration keeps snapshots of the files it manages. [lpm migrate --rollback](/docs/packages/migrate#rollback-and-recovery) restores those files.
Rollback does not restore `node_modules`, store contents, or arbitrary files changed by project scripts.
...
Commit `lpm.lock`, the updated `.npmrc`, and the (possibly mutated) `package.json`. Discard the source lockfile (`package-lock.json` / `pnpm-lock.yaml` / `yarn.lock` / `bun.lock*`) only after everyone on the team has switched over — leaving it in place during transition lets developers keep using the old PM until they cut over.

## Walkthroughs

<Cards>
  <Card title="Migrating from npm" href="/docs/guides/migrating-from-npm" description="Convert package-lock.json to the current lpm.lock format." />
