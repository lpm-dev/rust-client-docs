...
| **Cooldown on recent direct versions.** LPM blocks direct/root dependency versions published less than 24h ago by default. | `lpm install --allow-new` per-invocation, or `package.json > lpm > minimumReleaseAge` per-project. |
