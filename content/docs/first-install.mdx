...
`node_modules/` is laid out **hoisted by default** (npm-style flat). Workspaces auto-flip to isolated (pnpm-style symlinks). Override per-invocation with `--linker=isolated|hoisted`.
