...
A package's access mode on LPM.dev Registry: **Private** (default), **Free**, **Pool**, or **Marketplace**. Distribution is separate from package format. See [LPM.dev package distribution](/docs/packages/package-distribution).
...
The authoritative `lpm.lock` TOML file. It records exact package instances, sources, dependency targets, and workspace selections. Current schema 13 has no binary companion. See [Lockfile format](/docs/reference/lockfile-format).

### lpm.dev

The LPM.dev Registry monetization layer. Hosts Private, Free, Pool, and Marketplace packages under the `@lpm.dev/` scope. **LPM CLI is registry-agnostic** — lpm.dev is one of several registries it can talk to. See [Registries](/docs/registries).
...
A dep declared like `"my-react": "npm:react@^19.0.0"`. Local name (`my-react`) diverges from the canonical target (`react`). Tracked in lockfiles via `alias-dependencies` and `root-aliases`.
