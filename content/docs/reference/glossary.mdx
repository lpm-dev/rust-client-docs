...
A change in a package's publisher identity between a prior approval and a candidate version. LPM blocks installs on drift; opt out per-package via `lpm install --ignore-provenance-drift <pkg>` or fully via `--ignore-provenance-drift-all`.
