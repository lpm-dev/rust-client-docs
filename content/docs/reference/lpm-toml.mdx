...
`package` is the exact direct dependency name to allow. `similar-to` narrows the exception to the popular package LPM detected; omit it only when the same local name may intentionally resemble more than one popular package. `reason` is required so the exception is reviewable in code review.
