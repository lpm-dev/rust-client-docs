...
LPM reads a number of environment variables to override behavior, route auth, or expose diagnostic knobs. Most users never need to touch these — they exist for CI, debugging, recovery, and uncommon workflows.
...
| `LPM_REGISTRY_URL`      | Override the registry URL. Equivalent to `--registry <URL>`. Use only trusted registry endpoints: this controls where package metadata requests and registry-scoped credentials are sent.                                                                                                                                                                                                                                                                                                                                                 |
...
| `LPM_NPM_FIREWALL`             | Tighten firewall.lpm.dev verdict checks for npm packages in this process. Accepts `off`, `report`, or `enforce`, but cannot weaken a stricter `~/.lpm/config.toml > [firewall].mode` value. Active modes send LPM auth because the hosted verdict service is an LPM Pro/org feature. Persistent equivalent: `lpm config firewall --set report\|enforce`. |
...
| `LPM_INSTALL_MIN_VERSION_OVERRIDE` | Standalone installer | Set to `1` to bypass the installer's minimum-version floor when intentionally installing an older release. SHA-256 and Sigstore checks still run unless `LPM_INSTALL_INSECURE=1` is also set. |
