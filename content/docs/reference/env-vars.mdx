...
| `LPM_VAULT_ID`          | Override the resolved vault ID for `lpm env pull --oidc` only. The current implementation still requires a local `lpm.json > "vault"` value before it reads this override, so it cannot bootstrap a checkout with no vault ID. Other `lpm env` operations ignore it. See [Secrets vault — Per-project identity](/docs/infra/secrets-vault#per-project-identity).                                                                                                                                                                                                       |
