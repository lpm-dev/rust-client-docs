...
| `LPM_REGISTRY_URL`      | Override the registry URL. Equivalent to `--registry <URL>`. Use only trusted registry endpoints: this controls where package metadata requests and registry-scoped credentials are sent. The MCP server uses the same effective URL for registry-scoped keychain lookup when the variable is inherited by the editor. Direct non-loopback HTTP also requires `--insecure`; that flag does not allow an HTTPS redirect to downgrade to HTTP.                                                                                              |
