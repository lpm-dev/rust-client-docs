...
| Variable   | Effect                                                                                                                         |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `LPM_HOME` | Override the LPM root (`~/.lpm` by default). Moves the store, cache, global installs, and user config. Useful for hermetic CI runs and local development. It does not move the OS-managed policy used by `lpm security protect`. |
...
| `LPM_NPM_FIREWALL`             | Tighten firewall.lpm.dev verdict checks for npm packages in this process. Accepts `off`, `monitor`, or `enforce`, but cannot weaken a stricter `~/.lpm/config.toml > [firewall].mode` value. `report` remains accepted as a legacy alias for `monitor`. Active modes send LPM auth because the hosted verdict service is an LPM Pro/org feature; in CI, `LPM_TOKEN` or stored auth is used first, and registry-audience OIDC is exchanged only when no bearer is already available. Persistent equivalent: `lpm config firewall --set monitor\|enforce`. |
