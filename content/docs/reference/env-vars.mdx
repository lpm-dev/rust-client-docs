...
| `LPM_OIDC_TOKEN` | Pre-supplied OIDC token for `lpm setup --oidc` and `lpm publish --provenance`. Bypasses the live exchange. |
| `LPM_GITLAB_OIDC_TOKEN` | GitLab-specific OIDC token. Honored by GitLab CI integrations. |
...
| Variable | Effect |
| --- | --- |
| `LPM_LINKER` | Set to `hoisted` for npm-style flat `node_modules`. Default behavior is the isolated (pnpm-style) layout. Equivalent to `--linker=hoisted`. |
| `LPM_CONCURRENT_DOWNLOADS` | Cap the number of concurrent tarball downloads during install. |
