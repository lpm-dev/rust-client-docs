...
The Layer 5 [LLM advisor](/docs/packages/security-audit#verdict-cache) persists verdicts at `$LPM_HOME/cache/l4-verdicts.json`. A repeat install skips the advisor round-trip only when the package source, integrity, canonical executable hash, prompt inputs, provider, and effective model identity all match the cached review.
