...
The Layer 5 [LLM advisor](/docs/packages/security-audit#verdict-cache) persists verdicts at `$LPM_HOME/cache/l4-verdicts.json`. A repeat install skips the advisor round-trip only when the package source, integrity, canonical package-executable hash, prompt inputs, provider, observed CLI version, and effective model identity all match the cached review. LPM bypasses persistent caching when it cannot observe either the CLI version or effective model. Claude and Codex therefore require an explicit LPM model selection because their CLI-managed default model is not observable reliably.
...
| `LPM_TRIAGE_CLAUDE_MODEL` | Pin the model passed to `claude --model`. Persistent verdict caching for Claude is disabled when unset because the CLI's configured/default model is opaque to LPM. |
| `LPM_TRIAGE_CODEX_MODEL` | Pin the model passed to `codex exec --model`. Persistent verdict caching for Codex is disabled when unset because the CLI's configured/default model is opaque to LPM. |
| `LPM_TRIAGE_OLLAMA_MODEL` | Select the Ollama model used by the triage advisor. Defaults to `llama3.2`. The effective model participates in verdict-cache identity. |
