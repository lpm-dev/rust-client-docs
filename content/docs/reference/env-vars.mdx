...
| `LPM_NPM_FIREWALL`             | Tighten lpm.dev npm firewall verdict checks for this process. Accepts `off`, `report`, or `enforce`, but cannot weaken a stricter `~/.lpm/config.toml > [firewall].mode` value. Persistent equivalent: `lpm config firewall --set report\|enforce`. |
