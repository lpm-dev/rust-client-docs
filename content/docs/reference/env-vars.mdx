...
| `LPM_NPM_FIREWALL`             | Tighten firewall.lpm.dev verdict checks for npm packages in this process. Accepts `off`, `report`, or `enforce`, but cannot weaken a stricter `~/.lpm/config.toml > [firewall].mode` value. Persistent equivalent: `lpm config firewall --set report\|enforce`. |
