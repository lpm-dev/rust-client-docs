...
| `LPM_REMOTE_CACHE_TOKEN` | Bearer token for hosted cache reads and writes. Falls back to `LPM_TOKEN` or the token from `lpm login`. |
| `LPM_REMOTE_CACHE_URL` | Override the hosted cache endpoint. Defaults to the configured registry plus `/v8`. HTTP is accepted only for localhost. |
...
| `LPM_REMOTE_CACHE_SIGNATURE_KEY` | HMAC key for signed remote artifacts. Required when `lpm.json > remoteCache.signature` is `true`. |
