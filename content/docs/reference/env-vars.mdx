...
| Variable | Value |
| --- | --- |
| `npm_lifecycle_event` | Current script phase or task name. |
| `npm_lifecycle_script` | Declared command, without extra CLI arguments. |
| `npm_package_name` | Current package name, or an empty value if absent. |
| `npm_package_version` | Current package version, or an empty value if absent. |
| `npm_package_json` | Absolute path to the current package's manifest. |
| `INIT_CWD` | Directory where the CLI command started. |

Generated values replace inherited values for the same keys. Workspace scripts and dependency scripts retain the original `INIT_CWD`.

See [npm script execution](/docs/packages/npm-compatibility#script-execution) for hooks and failure behavior.
...
| `NPM_CONFIG_PROVENANCE` | Exact lowercase `true` enables generated publish provenance. Empty is unset. Other nonempty values disable it. The lowercase alias `npm_config_provenance` is also supported. Conflicting aliases cause an error. See [publish provenance](/docs/packages/publish#provenance). |
...
| `LPM_REMOTE_CACHE_TEAM` | Override the organization slug in `lpm.json > remoteCache.team`. Organization access requires a token scoped to that organization. Without either setting, the token selects its personal or organization namespace. |
...
See [Remote task cache](/docs/infra/remote-cache) for configuration and CI examples.

## Triage advisor cache

The [LLM advisor](/docs/packages/security-audit#verdict-cache) stores verdicts at `$LPM_HOME/cache/l4-verdicts.json`. Reuse requires matching package content, script hash, prompt, provider, and provider configuration.

For Ollama, the configured model and endpoint identify the cache. Changing either requests a new decision. The provider binary version does not identify remote model revisions.
...
| `LPM_FORCE_FILE_VAULT` | In debug/test builds, bypass native vault-key storage and force the encrypted-file fallback under `~/.lpm/vaults/` with `~/.lpm/.vault-fallback-key`. Release builds ignore this override and use the platform default. See [Secrets vault — Local storage](/docs/infra/secrets-vault#local-storage-by-platform). |
