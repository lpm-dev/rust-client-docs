...
| `LPM_VAULT_ID` | Override the project's resolved vault-id when running `lpm env` operations. Useful in CI workflows that target a vault other than what the checked-out project's `.lpm/vault-id` would resolve to (e.g., the `lpm ci setup github-actions` snippet sets this on the `lpm env pull --oidc` step). Falls back to the project's vault-id when unset. |
...
| `LPM_FORCE_FILE_VAULT` | Disable the OS keychain for secrets storage and force the encrypted-file fallback under `~/.lpm/.vault-data/`. Used by the test suite; also useful for sandboxed environments. See [Secrets vault — Local storage](/docs/infra/secrets-vault#local-storage). |
