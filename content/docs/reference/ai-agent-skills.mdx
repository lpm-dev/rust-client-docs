...
If `package.json > files` would exclude `.lpm/skills`, a real `lpm publish` adds the directory before creating the tarball; `--check` and `--dry-run` model that change in memory without touching the manifest. When `files` is omitted, the implicit publish walker traverses only `.lpm/skills`, includes only validated direct publisher-authored Markdown files, and excludes every other `.lpm` certificate, log, install hash, audit, and runtime-state file. Projects may also contain installed package skill directories; valid manifest-owned consumer directories are ignored during publisher validation and excluded even when an explicit files glob names them.
