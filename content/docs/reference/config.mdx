...
If a managed policy at `/etc/lpm/security-policy.toml` owns that control, the write fails with `error_code: "security_floor"` instead.

Directly editing `~/.lpm/config.toml` is not equivalent to approval for these keys. A raw file edit is only a proposal; LPM may ignore it or refuse with `security_approval_required` / `security_floor` when the value is weaker than the current approved floor.
