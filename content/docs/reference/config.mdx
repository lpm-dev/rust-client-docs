...
Two of the keys above have dedicated interactive wizards that handle picking + validation for you:
...
`lpm config triage` auto-detects which advisor providers are installed on your machine and only offers the available ones (plus `none`). Both wizards write to the same `~/.lpm/config.toml` that `lpm config set` would.
