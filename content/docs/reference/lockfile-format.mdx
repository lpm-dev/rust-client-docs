...
| Field | Type | Notes |
| --- | --- | --- |
| `lockfile-version` | u32 | Schema version. Current: `6`. Schema-versioned (not tool-versioned) so older clients can refuse cleanly when the schema bumps. Version `2` made peer state authoritative; version `3` added platform + optional metadata; version `4` added registry signature metadata; version `5` added importer snapshots for frozen-lockfile installs and patch checksum records; version `6` added persisted dependency `engines.node` constraints. |
...
Importer snapshots record the manifest inputs that must match before a frozen install can replay the lockfile. LPM CLI currently writes the root importer as `"."`.
...
| `integrity` | string \| absent | Exact declared SRI integrity hash (`sha512-…`, `sha256-…`, or legacy `sha1-…`). Populated when the registry provides it; LPM CLI verifies and preserves the declared algorithm and digest for cache reuse and replay. |
