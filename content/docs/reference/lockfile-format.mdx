...
| Field | Type | Notes |
| --- | --- | --- |
| `lockfile-version` | u32 | Schema version. Current: `4`. Schema-versioned (not tool-versioned) so older clients can refuse cleanly when the schema bumps. Version `2` made peer state authoritative; version `3` added platform + optional metadata; version `4` added registry signature metadata. |
