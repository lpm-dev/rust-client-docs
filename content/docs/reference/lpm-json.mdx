...
| `personalSyncedAt` | string | RFC 3339 timestamp for the last successful personal sync observed by this checkout. |
| `orgVersions.<slug>` | int | Last known org-shared cloud-vault version per org slug. |
| `orgSyncedAt.<slug>` | string | RFC 3339 timestamp for the last successful sync observed for each organization. |

The `lpm env push` and `lpm env pull` commands use this metadata for server conflict detection. Do not edit these fields.
