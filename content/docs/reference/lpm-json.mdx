...
Only `oxlint`, `biome`, and `rolldown` are managed plugin keys today. Any other key (`tools.typescript`, `tools.tsgo`, `tools.eslint`, …) is ignored by the resolver — `lpm` warns once per command invocation that the pin is ignored. For [`lpm check`](/docs/dev/check), declare `typescript` in `package.json > devDependencies` to pin the default compiler. The `--engine tsgo` mode uses the version bundled with LPM CLI. A local `@typescript/native-preview` dependency does not change that managed version.
...
For [`lpm lint`](/docs/dev/lint) and [`lpm fmt`](/docs/dev/fmt), members inherit the workspace root pin unless they supply their own. Invalid configuration fails the affected run.
...
[`LPM_VAULT_ID`](/docs/reference/env-vars#auth-and-routing) is read only by `lpm env pull --oidc`. A non-empty, trimmed value takes precedence over this local `vault` field and can bootstrap a checkout without one. If the variable is unset, empty, or whitespace-only, LPM CLI falls back to this field; the pull fails if neither source exists. Other `lpm env` operations ignore it. See [Environment project identity](/docs/infra/secrets-vault#configure-a-project).
...
| `dependsOn` | string[] | `[]` | Other tasks that must finish first. `"build"` = same package; `"^build"` = the same task in upstream workspace deps, with workspace selection. |
...
Use [`cacheEnv`](/docs/dev/task-runner#cache-key) to exclude inherited variables that do not affect the result.
Absolute script paths and executable identity remain cache inputs, so this setting does not guarantee reuse across machines or checkout paths.
...
`scripts/**`, `bin/**`, root `*.js`, `*.cjs`, `*.mjs`, `*.ts`, `*.tsx`, and `*.jsx` files,
`package.json`, `lpm.json`, `tsconfig.json`, `tsconfig.*.json`, `*.config.js`, `*.config.ts`, and `*.config.mjs`.
...
These patterns also control [task watch mode](/docs/dev/run#watch-finite-tasks).
...
Each `inputs` and `outputs` item must be a valid, project-relative glob. LPM CLI
rejects absolute paths, parent traversal, and invalid glob syntax before it
starts a task.

Output patterns combine regular-file matches. `dist/*` matches direct files,
and `dist/**` includes nested files. A leading `!` is literal, not an exclusion.

On a cache hit, LPM CLI accepts only files that match the current `outputs`.
It rejects an archive with undeclared files before it changes project outputs.
An accepted restore removes declared files that are absent from the cached result.
See [Remote task cache](/docs/infra/remote-cache#declare-output-files-precisely).
...
| `team` | string | token's namespace | Organization slug. An organization selection requires a token scoped to that organization. Without this field, personal tokens use personal storage and organization tokens use organization storage. |
...
See [Remote task cache](/docs/infra/remote-cache) for setup, team access, CI examples, and storage limits.
...
| `port` | number | — | Preferred service port. Host-routed services, explicit primaries, and recognized sole development servers can receive an automatic port. Generic workers stay portless unless primary endpoint features request a port |
...
| `readyUrl` | string | — | HTTP URL on `localhost`, `127.0.0.1`, or `[::1]`. The first 2xx response succeeds; redirects are not followed |
| `readyTimeout` | number | `30` | Shared deadline for listener ownership and application readiness, from `1` to `3600` seconds |
| `env` | object | `{}` | Extra env vars injected into this service only |
| `restart` | bool | `false` | Restart after a non-zero exit, with delays from 1 to 30 seconds and at most 10 attempts. More than 60 seconds continuously ready resets the budget |
