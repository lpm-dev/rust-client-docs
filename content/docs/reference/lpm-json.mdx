...
[`LPM_VAULT_ID`](/docs/reference/env-vars#auth-and-routing) is read only by `lpm env pull --oidc`, after LPM CLI has successfully resolved this local `vault` field. It can replace an existing resolved value for that pull, but it cannot bootstrap a checkout with no vault ID. Other `lpm env` operations ignore it. See [Secrets vault — Per-project identity](/docs/infra/secrets-vault#per-project-identity).
