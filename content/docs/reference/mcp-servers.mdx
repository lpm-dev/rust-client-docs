...
Before a refresh replaces the cached runtime, LPM CLI checks its selected executable.
A failed refresh preserves the previous installation.
[lpm cache prune --apply](/docs/packages/cache) waits until the server stops and retains the registered runtime.

The managed server uses Node.js from outside the caller's project and `node_modules` directories.
Its interpreter lookup excludes relative paths and project-controlled symlinks.
The server still runs from the caller's directory and receives its documented LPM.dev Registry credentials.
If no suitable Node.js executable is available, the command stops before it starts the server.
