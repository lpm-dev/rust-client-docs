...
VS Code uses the same shape under the `servers` key instead of `mcpServers`. The published package is `@lpm-registry/mcp-server`; LPM CLI deliberately writes its `latest` dist-tag. `npx -y` resolves and runs the current published release when the editor starts, so no separate install step is required.
...
The MCP server uses `LPM_TOKEN` first. When it is unset, the server reads the OS keychain credential created by `lpm login` for the effective registry URL. Keychain lookup is registry-scoped, so `https://lpm.dev`, local development registries, and custom registries do not accidentally share credentials.

The registry defaults to `https://lpm.dev`. To use another trusted registry, set `LPM_REGISTRY_URL` in the environment inherited by the editor or in that editor's MCP server environment configuration. `lpm mcp setup` does not copy the current shell's registry override into editor configuration.
...
`lpm mcp` takes no specific flags besides the global flags. `--json` emits one structured document. Setup output includes `package`, `package_spec`, `version_policy`, `command`, and `args` alongside `success`, `server`, and `configured`; remove and status use `removed_from` and `editors`.
