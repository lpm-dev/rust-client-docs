---
title: MCP servers
description: "Wire Claude Code, Cursor, Windsurf, VS Code, and Claude Desktop to LPM's MCP server."
---
...
Manages [MCP](https://modelcontextprotocol.io) (Model Context Protocol) server configuration for AI editors. `lpm mcp setup` writes the LPM MCP server entry into each detected editor's MCP config so the editor's agent can call LPM tools (search, install hint, audit, etc.) on your behalf.

## Examples

```bash
lpm mcp setup                      # write the "lpm-registry" entry to every detected editor
...
The positional argument is the **server name** that gets written into each editor's `mcpServers` (or VS Code's `servers`) map — it is **not** an editor selector. `lpm mcp setup` always walks every editor in the supported list and writes the entry; pass a name to override the default `lpm-registry`.

## Subcommands

| Subcommand | Effect |
| --- | --- |
| `setup [name]` | Add an MCP entry under `name` (default `lpm-registry`) to every editor in the supported list. Idempotent — re-running overwrites the entry in place. |
...
| `status` | List every supported editor and which MCP servers (LPM or other) are configured in each. |
