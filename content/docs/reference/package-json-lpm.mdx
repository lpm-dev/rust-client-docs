...
Security-sensitive project keys such as `scriptPolicy`, `minimumReleaseAge`, and `minimumReleaseAgePolicy` are treated as project **proposals**, not machine authority. If a repo asks for a weaker posture than this machine currently allows, install or rebuild fails and points you at [`lpm security unlock`](/docs/infra/security#temporary-unlocks) or a persistent [`lpm config`](/docs/infra/config) change.
...
How [`lpm install`](/docs/packages/install) saves registry dependencies that match the root default catalog.
...
Automatic modes take precedence over explicit registry versions, save flags, and existing registry ranges. They preserve source dependencies, including workspace links, paths, aliases, Git sources, and tarball URLs. `--catalog` cannot replace these sources.
...
If the repo lowers the cooldown below the current approved machine floor, install fails with `security_approval_required` and points you at a temporary [`lpm security unlock`](/docs/infra/security#temporary-unlocks) or a persistent machine-level config change.
...
Plain selectors and unqualified `parent>child` selectors can rescue a dependency whose original range has no available candidate.
Range-qualified selectors require a natural candidate. They do not match a replacement version from another override.
If an override target is unavailable or rejected during override selection, LPM CLI warns and retains the natural selection.
Selection checks release age, publisher trust, and platform compatibility. Later engine checks can fail or omit the selected package.
A required dependency still fails if neither selection is available.

A rescued selection shows `unresolved` in text summaries and `from_version: null` in JSON output and saved override records.
See [Resolver](/docs/packages/resolver#overrides-and-aliases) for selection behavior.
...
Local patches apply after dependency resolution and linking, before lifecycle scripts. Each key has the format `"<name>@<exact-version>"`.
[`lpm patch`](/docs/packages/patch) and [`lpm patch-commit`](/docs/packages/patch) create and register patches automatically.
The `path` field is relative to the `package.json` that declares it.

`lpm.lock > [patches]` records each artifact's SHA-256 checksum.
Installs check this checksum and compare each selected destination's source integrity with `originalIntegrity`. A mismatch stops installation.
These checks apply to the selected package bytes. They do not detect an upstream republish until an install selects the changed source.

Installs skip patches for deliberately omitted dependencies and retain their registrations. A registration absent from the dependency graph produces an error.
...
| `passEnv` | string[] | Exact additional environment names to pass after approval. Reserved runtime variables are rejected. |
| `readProject` | `"narrow"` \| `"full"` | Narrow reads allow dependency directories and build metadata. Approved full reads include all project files and secrets. |
...
| `sandboxReadAllow` | string[] | Individual files inside the project that scripts can read after approval. The CLI combines these paths with user-level `script-read-allow`. Directories, symlinks, and paths outside the project are rejected. Changes require another approval. |
...
Widening any of `passEnv`, `readProject`, `sandboxReadAllow`, `sandboxWriteDirs`, or `sandboxLimits` beyond the approved baseline requires explicit approval through [`lpm approve-scripts`](/docs/packages/approve-scripts) — the system does not silently grant elevated capabilities from raw repo or user config edits.
