...
The suffix after `#` is an opaque 64-character lowercase hexadecimal identity token derived from the exact `source` and `integrity` pair. It lets two packages with the same `name@version` retain independent approvals. Copy exact selectors from the candidate list printed by [`lpm approve-scripts`](/docs/packages/approve-scripts); don't construct the token by hand.

Legacy rich-map keys in the `name@version` form remain readable. Re-approving one migrates it to the source-qualified key when source or integrity is available.
