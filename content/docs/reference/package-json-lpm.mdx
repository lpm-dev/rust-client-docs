...
Approvals are **ephemeral trust** — never written to `package.json` or another trust store. Each `lpm install` creates a new in-memory approval; an exact cached verdict may recreate it without invoking the advisor provider. Use [`lpm approve-scripts`](/docs/packages/approve-scripts) (which writes to `trustedDependencies`) for durable trust.

Precedence: `--advisor` > `package.json > lpm > triageAdvisor` > `~/.lpm/config.toml > triage-advisor` > default (`"none"`).
...
Lifecycle trust configuration for dependency packages under `scriptPolicy: "deny"`. Two shapes remain readable:
...
The array is migration input only. Because it has no version, source, integrity, or script hash, it cannot authorize lifecycle execution. The packages remain in `lpm approve-scripts` output until they are reviewed into exact rich bindings.

**Rich map form** — new approvals are bound to source + integrity + script hash, so a source or content change re-opens the package for review:
...
The suffix after `#` is an opaque 64-character lowercase hexadecimal identity token derived from the exact `source`, `integrity`, and `scriptHash`. It lets two packages with the same `name@version` retain independent approvals even when only their lifecycle content differs. Copy exact selectors from the candidate list printed by [`lpm approve-scripts`](/docs/packages/approve-scripts); don't construct the token by hand.

Legacy rich-map keys in the `name@version` form remain readable as migration input. A source-less legacy binding cannot authorize a source-qualified package; re-approving writes a new source-, integrity-, and script-hash-qualified key.
