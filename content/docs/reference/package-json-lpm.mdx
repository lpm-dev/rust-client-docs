...
Runtime / CLI version constraints. LPM CLI enforces both `engines.node` (against the effective Node version) and `engines.lpm` (against the running CLI version) by default — install / rebuild / add abort on mismatch. `lpm.json > runtime.node` takes precedence over `engines.node` as a **pin source** for runtime auto-install; the **enforcement** check still runs against the resolved Node version.
