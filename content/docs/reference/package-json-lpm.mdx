...
| `"prefer"` | Save `"catalog:"` when `catalogs.default[pkg]` satisfies the resolved version; warn and save the direct spec when it does not. |
| `"strict"` | Save `"catalog:"` when `catalogs.default[pkg]` satisfies the resolved version; fail before committing `package.json` on mismatch or missing default-catalog entry. |

This affects `lpm install <pkg>` save policy only. Hand-written `"catalog:"` and `"catalog:<name>"` dependencies still resolve through the `catalogs` field below, and `lpm install --catalog[=<name>] <pkg>` can force a catalog save for one invocation.
...
Set `package.json > lpm > catalogMode` to control whether `lpm install <pkg>` writes matching default-catalog entries back as `"catalog:"`.
