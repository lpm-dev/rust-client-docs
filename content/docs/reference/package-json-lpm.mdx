...
**Default:** `"hoisted"` for single-package projects. LPM auto-detects workspace roots (a `package.json > workspaces` glob or a `pnpm-workspace.yaml`) and defaults those projects to `"isolated"`. There is no auto-promotion from a single package to a workspace — declare workspaces explicitly to opt in.

CLI flag: `lpm install --linker <isolated\|hoisted>` overrides per-invocation. Precedence: `--linker` flag > `~/.lpm/config.toml > linker` > `LPM_LINKER` > this `package.json` value > built-in default (`"hoisted"`, or `"isolated"` if a workspace is detected). Unknown values fail loudly at install time — there is no silent fallback.
