...
CLI flag: `lpm install --linker <isolated\|hoisted>` overrides per-invocation. Precedence: `--linker` flag > `~/.lpm/config.toml > linker` > `LPM_LINKER` > this `package.json` value > default `"isolated"`. Unknown values fail loudly at install time — there is no silent fallback.
