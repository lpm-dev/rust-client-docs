...
| `triage-advisor` | `"none"` \| `"claude-cli"` \| `"codex"` \| `"ollama"` | `"none"` | Optional LLM advisor for the [triage gate](/docs/packages/security-audit#optional-llm-advisor). Only active when `script-policy = "triage"`. Execution approvals last for one install. Verdicts use a separate [persistent cache](/docs/packages/security-audit#verdict-cache). |
...
| `release-age-policy` | `"direct"` \| `"strict"` | `"direct"` | Cooldown scope. `"direct"` checks direct/root dependencies only; `"strict"` also checks transitive dependencies and lockfile replays using persisted publish timestamps. Set with [`lpm config release-age-policy`](/docs/infra/config#focused-settings). |
...
| `sandbox.mode` | `"default"` \| `"strict"` \| `"none"` | `"default"` | Default lifecycle-script sandbox posture for this machine. Set with [`lpm config sandbox`](/docs/infra/config#focused-settings). |
| `sandbox.allow-degraded` | bool | `false` | Allow degraded sandbox fallback for this machine when full containment is unavailable. |
| `sigstore.verify` | `"deny"` \| `"warn"` \| `"off"` | `"deny"` | Provenance verification posture used by installs and related flows. Set with [`lpm config sigstore`](/docs/infra/config#focused-settings). |
...
| `fetch-lpm-security-insights` | bool | `true` | Use Registry behavioral tags and lifecycle-script data in the normal install summary for `@lpm.dev/*` packages. For JavaScript packages, `lpm audit` and audit-after-install add Registry security findings and vulnerabilities. For explicit Swift packages, audit-after-install adds them. This setting is independent of local source analysis. Set with `lpm config lpm-insights` or manage it alongside package skills with [`lpm config lpm-dev`](/docs/infra/config#control-lpmdev-registry-package-features). |
...
| `script-read-allow` | array of strings | `[]` | Propose reads of individual project files. The CLI combines these paths with project `sandboxReadAllow`; script approval is required. Directory entries, symlinks, and paths outside the project are rejected. Changes require another approval. |
| `max-sandbox-write-roots` | array of absolute paths | `[]` | User-authorized roots that may cover absolute [`package.json > lpm > scripts > sandboxWriteDirs`](/docs/reference/package-json-lpm#the-lpmscripts-block) entries outside the project. LPM CLI resolves the root and candidate before granting access. A candidate cannot traverse a symlink, Windows junction, or reparse point below the authorized root, and dangerous system or credential roots remain denied even when listed here. Relative roots are ignored. Each requested directory also requires script approval. |
...
[`lpm policy list`](/docs/infra/policy#review-all-active-extensions) redacts recognized credential arguments and URL credentials. This applies to terminal and JSON output. LPM CLI stores and runs the original command arguments.

[`lpm policy test`](/docs/infra/policy#test-one-package-candidate) rejects malformed package names before it starts an extension. Accepted loose versions use their canonical form in the request.
