...
# Fetch server-side findings for installed @lpm.dev/* packages
...
| `fetch-lpm-security-insights` | bool | `true` | Fetch server-side AI findings, vulnerabilities, lifecycle-script data, and registry behavioral tags for installed `@lpm.dev/*` packages. This setting is independent of local source analysis. Set with `lpm config lpm-insights` or manage it alongside package skills with [`lpm config lpm-dev`](/docs/infra/config#lpmdev-settings). |
