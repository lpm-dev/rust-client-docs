...
| `firewall.mode` | `"off"` \| `"report"` \| `"enforce"` | `"off"` | firewall.lpm.dev verdict checks for selected public npm package versions. `report` warns and continues; `enforce` blocks packages marked malicious. npm metadata and tarballs still use the direct npm route. Set with [`lpm config firewall`](/docs/infra/config#setup-wizards). Per-process `LPM_NPM_FIREWALL` can only tighten this value. |
