...
LPM only honors a weaker value when it is already covered by the current approved machine posture. Otherwise:

- `lpm config ...` asks for confirmation before persisting the weaker posture in an interactive TTY
- install / rebuild / related commands fail with `security_approval_required` if the raw file value is weaker than the approved floor
- a managed machine policy can fail the change with `security_floor` instead

For short-lived exceptions, use [`lpm security unlock`](/docs/reference/security#unlock). For persistent changes, use [`lpm config`](/docs/reference/config) rather than relying on a hand edit alone.
...
On `lpm install -g` the `package.json > lpm` tier is N/A — global installs synthesize their own package.json that doesn't carry user-edited keys. The chain collapses to CLI flag > `~/.lpm/config.toml` > built-in default for `script-policy`, `triage-advisor`, `minimum-release-age-secs`, and the script-policy aliases (`--yolo` / `--triage`).
