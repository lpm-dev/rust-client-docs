...
# lpm.dev npm firewall verdict checks
[firewall]
mode = "enforce"
...
| `firewall.mode` | `"off"` \| `"report"` \| `"enforce"` | `"off"` | lpm.dev firewall verdict checks for selected public npm package versions. `report` warns and continues; `enforce` blocks packages marked malicious. npm metadata and tarballs still use the direct npm route. Set with [`lpm config firewall`](/docs/infra/config#setup-wizards). Per-process `LPM_NPM_FIREWALL` can only tighten this value. |
...
Both forms parse to the same value. For `typosquat-guard`, the `default` value is accepted when hand-edited, but the CLI removes the key instead of persisting `typosquat-guard = "default"`. For `firewall.mode`, boolean-like aliases are accepted when hand-edited or provided through `LPM_NPM_FIREWALL`, but `lpm config firewall` persists the canonical `off`, `report`, or `enforce` value. Invalid values (non-numeric strings, negative numbers, zero for positive-only keys, etc.) error at load time with the file path baked in.
...
The npm firewall uses `[firewall].mode`, defaulting to `"off"`. `LPM_NPM_FIREWALL=report|enforce` can tighten the mode for one process, but it cannot weaken a stricter config value. The legacy diagnostic `LPM_EXPERIMENT_NPM_FIREWALL` is accepted for local benchmark compatibility; prefer `lpm config firewall --set report|enforce` for persistent use.
