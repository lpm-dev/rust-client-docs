...
| `firewall.mode` | `"off"` \| `"report"` \| `"enforce"` | `"off"` | firewall.lpm.dev verdict checks for selected public npm package versions. The hosted verdict service is an LPM Pro/org feature, so active modes send LPM auth. `report` warns and continues; `enforce` blocks packages marked malicious or denied by entitlement. npm metadata and tarballs still use the direct npm route. Set with [`lpm config firewall`](/docs/infra/config#setup-wizards). Per-process `LPM_NPM_FIREWALL` can only tighten this value. |
...
[firewall]
mode = "report"                       # canonical string
```

Both forms parse to the same value. For `typosquat-guard`, the `default` value is accepted when hand-edited, but the CLI removes the key instead of persisting `typosquat-guard = "default"`. For `firewall.mode`, boolean-like aliases are accepted when hand-edited or provided through `LPM_NPM_FIREWALL`, but `lpm config firewall` persists the canonical `off`, `report`, or `enforce` value. For `integrity`, config accepts only `source` or `tree`; the wizard persists those canonical strings. Invalid values (non-numeric strings, negative numbers, zero for positive-only keys, etc.) error at load time with the file path baked in.
...
The npm firewall uses `[firewall].mode`, defaulting to `"off"`. `LPM_NPM_FIREWALL=report|enforce` can tighten the mode for one process, but it cannot weaken a stricter config value. Active modes send LPM auth because the hosted firewall.lpm.dev verdict service is an LPM Pro/org feature. The legacy diagnostic `LPM_EXPERIMENT_NPM_FIREWALL` is accepted for local benchmark compatibility; prefer `lpm config firewall --set report|enforce` for persistent use.
