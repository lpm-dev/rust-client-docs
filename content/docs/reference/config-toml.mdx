...
# Run `lpm audit` after every successful install and print a summary
...
| `fetch-lpm-security-insights` | bool | `true` | Use registry behavioral tags and lifecycle-script data in the normal install summary for `@lpm.dev/*` packages. `lpm audit` and audit-after-install add registry security findings and vulnerabilities. This setting is independent of local source analysis. Set with `lpm config lpm-insights` or manage it alongside package skills with [`lpm config lpm-dev`](/docs/infra/config#lpmdev-settings). |
