...
| `minimum-release-age-exclude` | string array | `[]` | Exact package names exempted from the cooldown. Merged with CLI and package.json excludes. Globs, version selectors, and protocol specifiers are rejected. |
...
| `signatures` | bool | `false` | Verify npm registry package signatures during install and fail closed when a registry package cannot be verified. Set with [`lpm config signatures`](/docs/reference/config#setup-wizards). Per-process override: `LPM_VERIFY_REGISTRY_SIGNATURES`. |
| `trust-policy` | `"off"` \| `"no-downgrade"` | `"off"` | When set to `"no-downgrade"`, the resolver refuses versions that drop npm trust evidence compared with an earlier published version. Ranges can fall back to an older allowed candidate; exact blocked pins fail. Set with [`lpm config trust-policy`](/docs/reference/config#setup-wizards). |
...
On `lpm install -g` the `package.json > lpm` tier is N/A — global installs synthesize their own package.json that doesn't carry user-edited keys. The chain collapses to CLI flag > `~/.lpm/config.toml` > built-in default for `script-policy`, `triage-advisor`, `minimum-release-age-secs`, `strict-peer-dependencies`, `trust-policy`, and the script-policy aliases (`--yolo` / `--triage`). `minimum-release-age-exclude` still merges with repeated CLI `--min-release-age-exclude` flags. Registry signature verification uses `LPM_VERIFY_REGISTRY_SIGNATURES` > `signatures` > default `false`. Sigstore verification uses `LPM_PROVENANCE_ENFORCE` > `[sigstore].verify` > built-in default, with weaker env/config values guarded by the security approval layer.
