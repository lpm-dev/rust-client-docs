...
For subprocess passthrough commands (`lpm run`, `lpm test`, etc.), `--json` does **not** emit a wrapping JSON envelope on failure — the wrapped command's own output (whatever it was) is the result. This preserves the "single JSON result" contract.
