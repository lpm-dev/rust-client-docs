---
title: Exit codes
description: "What each LPM CLI exit code means and how to handle them in CI."
---
...
For [lpm run](/docs/dev/run), a single requested task with no task dependencies preserves the child status. This includes pre/post hook failures.

Multiple tasks return the number of failed tasks. Workspace runs report failure for the selected operation.

Root install hooks also preserve the child exit code. In JSON mode, their error includes `phase` and `exit_code`. Dependency rebuilds report an aggregate result.
...
- **[lpm run](/docs/dev/run)** reports task status, duration, cache hits, and skipped tasks. Task failures include the child exit code and phase when available. Workspace runs report selected and successful package counts. An unmatched workspace filter produces a successful zero-package result unless `--fail-if-no-match` is set. Child output and cached logs go to stderr. Stdout contains one JSON envelope.
