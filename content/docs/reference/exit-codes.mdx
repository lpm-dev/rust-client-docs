...
| _passthrough_ | Forwarded subprocess exit code. Commands that wrap external processes (`lpm run`, `lpm exec`, `lpm dlx` / `lpx`, `lpm test`, `lpm bench`, `lpm lint`, `lpm fmt`, `lpm check`) exit with the wrapped command's exit code. |
