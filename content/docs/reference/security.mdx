...
| `provenance-unverified` | `LPM_PROVENANCE_ENFORCE=warn|off`, raw or persisted `[sigstore].verify = "warn"|"off"`, or `--unverified-provenance*` |
...
Every guarded attempt and granted unlock is appended to `~/.lpm/security/audit.jsonl` as a signed hash-chained entry. Production builds also store the audit head in the OS keyring so local truncation or rewrite attempts can be detected on the next append. This is local tamper evidence, not a remote immutable log.
