...
Use `--package <name>` to scope the unlock to one or more packages when the weakening is package-specific. Repeat `--package` to cover more than one package.
...
| `provenance-unverified` | `LPM_PROVENANCE_ENFORCE=warn|off`, `--unverified-provenance*`, or weaker persistent Sigstore verification settings |
...
Every guarded attempt and granted unlock is also appended as a signed entry in `~/.lpm/security/audit.jsonl`.
...
- the current project root when applicable
...
- any active runtime overrides such as `LPM_PROVENANCE_ENFORCE`
...
LPM may ask inline for confirmation in an interactive terminal. If you approve, it mints the same temporary unlock internally and continues the command.
...
When this file owns a control, weaker repo or user changes do not win. They fail with `error_code: "security_floor"` instead. In production, LPM only loads managed policy from the managed path itself and validates that the file and its parent directories are authority-owned rather than user-writable.
