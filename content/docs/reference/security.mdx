...
```bash
lpm security <unlock|lock|status>
```
...
```bash
lpm security unlock <scope|all|default> [--project PATH | --global] [--ttl 10m]
```
...
Use `--package <name>` only with a concrete scope when the weakening is package-specific. Repeat `--package` to cover more than one package. A package-scoped unlock only covers requests for those package names; it does not cover blanket all-package flags such as `--ignore-provenance-drift-all` or `--unverified-provenance-all`. Bundle selectors `all` and `default` reject `--package`.
...
```bash
lpm security lock <scope|all|default> [--project PATH | --global]
```
...
Use `--package <name>` only with a concrete scope. For `lock`, package filters match unlocks created with that exact package filter set.
