...
- `unlock` creates a short-lived, project-scoped approval for one guarded weakening.
- `status` shows the effective security floor for the current project root, where that floor came from, and which unlocks are still active.
...
| --- | --- |
| `lpm security unlock <scope>` | Create a temporary approval for one guarded weakening in one project |
| `lpm security status` | Print the effective floor, source attribution, and active unlocks for the current project |

## `unlock`

```bash
lpm security unlock <scope> --project . --ttl 10m
```

Creates a temporary approval under `~/.lpm/security/unlocks/` for one project root. The command does **not** run the risky action itself. It only creates the unlock so you can rerun the original install or rebuild command.
...
| `provenance-unverified` | weaker persistent Sigstore verification settings |

### Interactive vs automation

`lpm security unlock` is interactive today. In a TTY it asks for confirmation, then writes the signed unlock grant on success.
...
```bash
lpm security status [--project PATH]
```

Shows the effective security floor for the selected project root.
...
- the active unlocks for this project and when they expire
...
When this file owns a control, weaker repo or user changes do not win. They fail with `error_code: "security_floor"` instead.
