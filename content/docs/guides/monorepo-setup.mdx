...
- Workspace members referenced via `workspace:*` must be published to the registry. Local-only deps aren't injected into deploy outputs yet — publish internal members first.
...
- **`workspace:*` deps in `lpm deploy` output must be published.** Unpublished workspace dep injection isn't supported yet — the pre-deploy build needs to publish internal members first.
