...
`lpm dev` will use the pinned version, auto-installing it via [`lpm use`](/docs/dev/use) if missing. Fall-back chain: `lpm.json > runtime.node` → `package.json > engines.node` → `.nvmrc` → `.node-version`. See [Node version pinning](/docs/dev/node-version-pinning) for the full detection contract.
