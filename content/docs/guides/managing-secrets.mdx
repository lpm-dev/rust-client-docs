...
[`lpm env set`](/docs/dev/env) writes to the project's local vault. On macOS, LPM CLI and the [LPM Vault app](/docs/dev/lpm-vault) share Keychain storage.

The app can manage those values without plaintext dotenv files. See [Using LPM Vault](/docs/dev/lpm-vault) for the app-to-runtime workflow.
...
The supported CLI command for secrets is [`lpm env`](/docs/dev/env). [LPM Vault](/docs/dev/lpm-vault) is the macOS app that shares local storage with LPM CLI.
...
- [Using LPM Vault](/docs/dev/lpm-vault) — manage variables in the macOS app and load them at startup
