...
- **Rotation uses remote compare-and-swap.** `lpm env rotate-key` preserves every named personal environment and metadata boundary; if the remote version changes concurrently, resolve the conflict and retry.
