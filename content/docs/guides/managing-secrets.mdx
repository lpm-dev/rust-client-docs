...
The normal command exits non-zero when any environment is invalid. Under global `--json`, LPM CLI currently reports `"success": false` but exits with status 0, so test that field explicitly in automation.
...
Missing keys currently leave the process exit status at 0. JSON output sets `"valid": false` while retaining `"success": true`, so test `valid` explicitly if you use this command in automation. `--strict` lists extra local keys, but those extras do not make the result invalid.
...
- **Don't discard validation fields in automation.** `lpm env check --json` requires testing `success`; `lpm env validate --json` requires testing `valid`.
- **Don't run `lpm env rotate-key` on a project with named environments.** It currently force-pushes only the default local environment and can replace the remote multi-environment data.
