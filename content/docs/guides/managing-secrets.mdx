...
[`lpm env set`](/docs/dev/env) writes to protected local storage associated with the project.

See [local secrets](/docs/dev/lpm-vault) to load those values into an application without plaintext dotenv files.
...
[`lpm run start --env=staging`](/docs/dev/run) and [`lpm dev --env=staging`](/docs/dev/dev) load the staging vault after the applicable dotenv files.

If the staging vault is missing or empty, runtime loading falls back to `default`. A non-empty staging vault does not inherit keys from `default`.
...
For organization sharing, an owner or administrator first grants the contributor access to the env project and their registered sharing key.

```bash
git clone <repo>
cd <repo>
lpm login
lpm env pull --org=<slug>
lpm install
lpm dev
```

The `vault` field in [`lpm.json`](/docs/infra/secrets-vault#configure-a-project) identifies the cloud project. The identifier does not grant access.

Browser pairing transfers a key from an existing CLI machine to the browser. It does not install that key on a new CLI machine.
Personal cloud sync has no CLI-to-CLI key-transfer command. A new device cannot decrypt another device's project from account login alone.

For local-only or personal projects, transfer the required values through a trusted method, then import them:

```bash
lpm env import .env.shared
```

The file contains plaintext secrets. Exclude it from Git and delete it after import.
The environment schema detects missing required values. The `secret: true` field marks values for supported masking and redaction paths.
...
The supported CLI command for secrets is [`lpm env`](/docs/dev/env). Local use requires no desktop app.

See [Secrets vault](/docs/infra/secrets-vault) for the storage and encryption design.
...
- [Local secrets](/docs/dev/lpm-vault) — store values locally and load them at startup
