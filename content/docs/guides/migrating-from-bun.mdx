...
`lpm migrate` converts a Bun project to LPM CLI in one command. It detects `bun.lock` or `bun.lockb`, parses the lockfile, writes `lpm.lock` plus `lpm.lockb` when the graph fits the binary format, optionally configures `.npmrc`, runs `lpm install`, and verifies `build` + `test` when those scripts exist.
