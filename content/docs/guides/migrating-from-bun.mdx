...
description: "Convert a Bun bun.lock or bun.lockb project to LPM CLI with rollback safety."
---
...
`bun.lock` supports JSON comments and trailing commas. If `bun.lockb` is selected, LPM CLI converts a fixed copy with the `bun` executable on `PATH`.
The binary converter emits Yarn text, which LPM CLI converts to its staging lockfile.
An older sibling `bun.lock` does not replace the selected binary input. If Bun is unavailable, migration fails.
...
- `bun.lock` comments, trailing commas, dependency metadata, and integrity strings are supported.
- A selected `bun.lockb` needs the `bun` executable on `PATH`.
...
Restores the original managed files and removes files created by migration. Repeated migrations retain the first snapshots.
Rollback does not restore `node_modules` or undo arbitrary script output. Use Bun to restore its dependency layout after rollback.
