...
`lpm init --lpm` walks you through `owner` / `name` / `version` / `description`. The published name will be `@lpm.dev/<owner>.<name>` — pick an `owner` you've claimed on lpm.dev (or one your org owns). Plain `lpm init` asks for the target first; choose **lpm.dev package** for this guide.
...
Plain `lpm init -y` is still valid and defaults to an lpm.dev package, but the explicit flags avoid leaving placeholder owner/name values in automation.
...
The score is computed from readme presence/length, license declaration, types coverage, test signals, and maintenance heuristics. See [`lpm quality`](/docs/packages/quality) to view the report for an already-published lpm.dev package.
...
Generates a Sigstore-signed attestation that proves which workflow built the tarball and which commit it came from. With `--npm`, LPM attaches npm's expected `{name}-{version}.sigstore` bundle in the publish payload. Consumers can verify provenance via the audit pipeline or the registry UI.
...
For npm-compatible targets, provenance requires public access. LPM's npm default is public for scoped and unscoped packages. If repo config enables provenance but a one-off publish should skip it, pass `--no-provenance`.
