...
For a Pool or Marketplace version, use `--wait` when CI must wait for public availability:

```bash
lpm publish --wait
```

The command keeps upload success separate from the publication wait. Do not publish the same version again if the wait fails.
