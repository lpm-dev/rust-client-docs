...
Validates the package configuration and authored skills, packs and scans the final tarball, computes the normalized local LPM.dev quality score, resolves real LPM.dev authentication, and performs a non-mutating Registry permission/version preflight. It does not reserve or upload a version, create Registry state, generate requested provenance, or claim publication occurred.

```bash
lpm publish --check
```

Runs the network-free preparation path and stops before LPM.dev OIDC, skills, and publish-permission lookups. Both preview modes leave the working tree untouched.
