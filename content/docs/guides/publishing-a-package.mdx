...
Validates the package configuration and authored skills, packs each target artifact, scans the actual final artifacts for hardcoded secrets, and computes the local LPM.dev quality score. Explicitly included files are scanned even when Git ignores them. It does not verify that your account can publish this package, check whether the version is available, generate requested provenance, or upload.

```bash
lpm publish --check
```

Runs the network-free preparation path and stops before the optional LPM.dev OIDC and skills lookups. Both preview modes update only the effective manifest used in memory when authored skills require `.lpm/skills`; they do not modify `package.json`. A real publish may persist the entry.
