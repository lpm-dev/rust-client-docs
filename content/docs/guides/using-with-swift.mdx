...
Setup failures abort the install instead of leaving a partially configured trust path. When `Package.swift` has more than one eligible target, LPM CLI asks which target should receive the dependency.
