...
The LPM.dev Registry speaks the [SE-0292 Swift Package Registry API](https://github.com/swiftlang/swift-evolution/blob/main/proposals/0292-package-registry-service.md), so Swift Package Manager (SPM) can resolve LPM.dev Registry-hosted Swift packages natively. This guide walks through the full setup with LPM CLI: installing a Swift package, and (for authors) publishing one.
...
1. Logs SPM into the LPM.dev Registry (writes the `swift package-registry` config).
2. Downloads the LPM.dev Registry CMS signing certificate and writes it to `~/.swiftpm/security/trusted-root-certs/lpm.der`.
...
> See [Trust model](/docs/packages/swift-package-registry#trust-model) on the concepts page for what the signing posture actually attests to today (detached integrity + HTTPS authenticity) and what it does not (trusted signer identity, per-author attribution).
...
The flow is the same as any other package published to the LPM.dev Registry, plus two Swift-specific requirements:
...
After publish, SPM clients on any machine that's run `lpm swift-registry` can resolve the package by its LPM.dev Registry identity.
...
The `_` between owner and package name is the unambiguous separator: the LPM.dev Registry forbids `_` in both owner and package name (DB-level), so the boundary is always clear even when either half contains hyphens. You don't need to deal with the SPM identity by hand — LPM CLI's `lpm install` and `lpm swift-registry` commands handle the translation.

## Verifying signatures

The LPM.dev Registry serves the CMS signing cert at `https://lpm.dev/api/swift-registry/certificate` (DER). `lpm swift-registry` writes the cert and the matching `registries.json` scope override; SPM then verifies the CMS detached signature on every install.
