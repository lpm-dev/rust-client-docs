...
After migration, `lpm install` is silent for projects whose `lpm.overrides` covers every `pnpm.overrides` key. If they drift apart later (you add a new `pnpm.overrides` entry without re-running migrate), `lpm install` emits a stderr warning suggesting `lpm migrate`.

## pnpm-specific gotchas

- **`pnpm-workspace.yaml`** isn't read by LPM. Move workspace globs into `package.json > workspaces`.
- **`pnpm patch`** patches in `patches/` aren't auto-converted today. Re-author them via [`lpm patch`](/docs/packages/patch) — the format is similar but the integrity binding is stricter.
