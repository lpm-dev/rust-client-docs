...
- **Isolated `node_modules` layout for workspaces.** Workspace projects auto-flip to LPM's isolated layout (symlinks into a virtual store) — same shape pnpm uses, so your imports keep working unchanged and you get the same phantom-dep protection. Single-package projects default to hoisted; force isolated with `lpm install --linker=isolated` or `package.json > lpm > linker = "isolated"` if you want pnpm's shape there too.
...
- **Workspaces.** LPM reads `package.json > workspaces` first; if it's absent, it falls back to `pnpm-workspace.yaml`. Migration doesn't strictly require moving the globs, but `package.json > workspaces` is the cleaner long-term shape — `pnpm install` and `lpm install` both honor it, and you can delete `pnpm-workspace.yaml` once the team has cut over.
...
- **`pnpm-workspace.yaml`** is read as a fallback when `package.json > workspaces` is absent — `pnpm install` still works after migration. Moving globs into `package.json > workspaces` is the recommended long-term shape (one source of truth, npm + LPM + pnpm all read it), not a hard requirement.
