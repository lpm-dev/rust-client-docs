...
Restores `pnpm-lock.yaml`, `.npmrc`, and `package.json` to pre-migration state. Files that the migration newly created — `lpm.lock`, `lpm.lockb`, and any patch files copied to `patches/` — are removed.
