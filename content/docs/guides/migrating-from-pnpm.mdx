...
After migration, `lpm install` is silent for projects whose `lpm.overrides` and `lpm.patchedDependencies` cover every pnpm-side entry. If they drift apart later (you add a new `pnpm.overrides` or `pnpm.patchedDependencies` entry without re-running migrate, or change a target), `lpm install` emits a diff-aware stderr warning suggesting `lpm migrate`.
