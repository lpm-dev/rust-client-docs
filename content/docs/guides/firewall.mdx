...
The hosted verdict service is an LPM.dev Registry Pro/Org feature. Active modes send LPM.dev Registry auth; run `lpm login` locally, or use `LPM_TOKEN` / registry-audience OIDC in CI.
...
OIDC-capable CI can avoid storing `LPM_TOKEN`. When firewall mode is active and no existing bearer or refreshable stored session is available, LPM CLI exchanges a registry-audience OIDC token for a short-lived LPM.dev Registry token before calling the verdict API. If `LPM_TOKEN` or stored LPM.dev Registry auth is already available, the OIDC exchange is skipped.
...
- [CI/CD setup](/docs/guides/ci-cd-setup#5-npm-firewall-in-ci) — token and OIDC examples
