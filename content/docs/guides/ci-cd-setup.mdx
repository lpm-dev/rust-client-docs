...
```yaml
- run: npm install -g @lpm-registry/cli@0.41.0
```
...
Each step exits non-zero on failure — no special handling needed. See [Exit codes](/docs/reference/exit-codes). `lpm trust diff` itself is informational and always exits zero — pipe through `jq -e` (or another JSON asserter) to gate on `trustedDependencies` drift.
