...
### Share task outputs across runners

[Remote task caching](/docs/infra/remote-cache) reuses completed build outputs across CI jobs. Enable it in `lpm.json` for tasks with declared outputs and a deliberate `cacheEnv` list.

After CLI and dependency installation, use a cache-scoped token on the build step:

```yaml
- name: Build with shared task cache
  run: lpm run build
  env:
    NODE_ENV: production
...
Use `cache:write` for producers. For consumers, use `cache:read` and set `LPM_REMOTE_CACHE_READ_ONLY: "1"`.

Cache scopes do not grant access to private packages. Keep package-install credentials on the install step.

See [Remote task cache — Team and CI setup](/docs/infra/remote-cache#share-with-a-team-and-ci) for the project configuration and organization namespace rules.
...
- [Remote task cache](/docs/infra/remote-cache) — share task results across runners
