...
Create a read-scoped token at [lpm.dev/dashboard/settings/tokens](https://lpm.dev/dashboard/settings/tokens), then store it as `LPM_TOKEN` in your CI's secret store. The token is shown only once when created. LPM CLI reads it directly and routes `@lpm.dev/*` packages without an `.npmrc`; use [`lpm setup ci npmrc`](/docs/infra/setup) only when another npm-compatible client in the job needs Registry authentication.
