...
Each step exits non-zero on failure — no special handling needed. See [Exit codes](/docs/reference/exit-codes). `lpm trust diff` is informational by default; add `--assert-none` to gate on `trustedDependencies` drift. Use `lpm --json trust diff --assert-none` if you also want the structured diff envelope in CI logs.
...
- [`lpm setup ci npmrc`](/docs/reference/authentication#lpm-setup-ci) — generate `.npmrc` for CI
- [`lpm setup ci <platform>`](/docs/reference/authentication#lpm-setup-ci) — generate OIDC workflow snippets
- [`lpm ci`](/docs/packages/install#frozen-lockfile-and-ci) — frozen lockfile install
- [`lpm env print --ci`](/docs/reference/authentication#ci-env-helpers) — CI-native env output
