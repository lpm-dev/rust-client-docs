...
7. **CI hint** — prints a hint pointing at `lpm ci setup github-actions` (or generates a workflow with `--ci`).
8. **Summary** — what was done, where backups live, and how to roll back.

In a TTY, you'll be prompted before the install + verify steps. Skip the prompts with `-y` (implies `--force`).
...
Walks the backups created by the previous run and restores `package-lock.json`, `.npmrc`, and `package.json` to their pre-migration state. Safe to run repeatedly. Doesn't touch `lpm.lock` / `lpm.lockb` — you can delete those by hand.
