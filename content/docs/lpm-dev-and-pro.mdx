...
Organizations use separate org billing at $8 per seat. Each seat includes unlimited private packages, 5 GB package storage, and 25 GB remote cache. The organization shares a limit of 3 concurrent tunnels and 3 claimed tunnel domains across all members and base domains. Additional seats do not increase these two limits.

Each billed Stripe seat adds 100,000 included tunnel requests and 20,000 requests/minute to the organization account. The seat count is fixed when the tunnel usage period opens and applies for the rest of that period. Organization overage uses the same proportional $1/100,000 rate and default-on toggle.
