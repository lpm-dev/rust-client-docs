...
LPM.dev Registry implements the [SE-0292 Swift Package Registry API](https://github.com/swiftlang/swift-evolution/blob/main/proposals/0292-package-registry-service.md). Once you've run `lpm swift-registry` once, Swift Package Manager (SPM) on your machine can resolve LPM.dev Registry-hosted Swift packages natively — no manual fork-as-git-dep, no shim.
...
Four setup steps under the hood. Scope, certificate, and signing-trust failures abort the command. Authentication setup warns instead when no token is available or SPM rejects the login:
...
When `lpm install` first encounters a Swift package in a project that hasn't been set up, it runs the same four-step path (in `ensure_configured` mode) before resolving the dep. Scope, certificate, and signing-trust failures abort setup; a missing token warns during setup, but LPM.dev Registry returns `401` until you run `lpm login`.
...
- Today `lpm swift-registry` configures the `lpmdev` scope only. Custom-scope publishing (e.g., `acme` for a private LPM.dev Registry tenant) isn't surfaced in LPM CLI yet.
