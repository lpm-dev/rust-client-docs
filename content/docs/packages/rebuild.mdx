...
temporary writes use a private per-build directory. The dependency graph,
...
samples), a local hit completed in **29.45 ms median** versus **114.99 ms** with
the cache disabled: **3.90× faster** and **74.39% lower wall time**. A fresh
project sharing the warm LPM store completed in **29.19 ms median** (**3.94×
faster**, **74.62% lower**). The miss median was 134.58 ms and the artifact used
