---
title: lpm rebuild
description: "Run lifecycle scripts for installed packages — the second step of the install pipeline."
---
...
`lpm install` downloads and links packages without running any lifecycle scripts. `lpm rebuild` is the **second step** — it selectively runs lifecycle scripts based on the active [script policy](/docs/packages/install#lifecycle-scripts) and the project's `trustedDependencies`.
...
- [`lpm install`](/docs/packages/install#lifecycle-scripts) — the first step of the same pipeline; shares the `--policy` semantics
