...
| --- | --- | --- | --- | --- | --- |
| **Default** | Write-containment to the package's own directory + anything in `package.json > lpm > scripts.sandboxWriteDirs` | Minimal environment plus approved `passEnv` names | Allowed | Denied unless explicitly approved (see [below](#project-secret-files)) | Nothing — this is the default |
| **Strict** | Same as default | Same as default | **Denied** | Same as default | `--strict-sandbox` (or alias `--paranoid`), persistent via `[sandbox] mode = "strict"` in `~/.lpm/config.toml` / `./lpm.toml`, or env `LPM_STRICT_SANDBOX=1` |
...
- the complete environment exposed to the lifecycle script, including the
  baseline variables and LPM CLI's effective `PATH`, `INIT_CWD`, `TMPDIR`, `TMP`,
  and `TEMP` values
...
could contain the previous path. Changes to exposed baseline variables also
change the build key. Undeclared variables, including `CFLAGS` and
`GITHUB_RUN_ID`, do not reach dependency scripts and do not change the key.
Adding a `passEnv` grant requires approval and disables native artifact reuse
for that project, because the project has widened its lifecycle capabilities.
...
The default `readProject: "narrow"` allows dependency directories, `package.json`,
package-manager lockfiles, and root `tsconfig*.json` files. Other project source
files require an explicit read grant. The package can read its own files,
standard toolchains, caches, `.lpm`, `.husky`, and approved writable directories.

An approved `readProject: "full"` grants reads of the complete project tree,
including secret files. It does not grant access to credentials outside the project.

The baseline environment contains `PATH`, `HOME`, user names, locale and terminal
settings, `CI`, and required Windows system variables. Other values require an
explicit `passEnv` entry and script approval. For example:
...
```bash
lpm approve-scripts my-dependency
lpm rebuild my-dependency
```

Only the approved names reach the script. A similar name does not inherit approval.
Reserved runtime variables such as `NODE_OPTIONS`, `LD_PRELOAD`, and
`DYLD_INSERT_LIBRARIES` cannot use `passEnv`. LPM CLI also controls `INIT_CWD`,
temporary-directory variables, and Git/npm configuration paths.

### Project secret files

Narrow reads deny ordinary source directories and their secret files, including
nested `.env` files and custom names such as `.env.preview`.
Explicit read grants can allow individual files. Full project access also allows
all project secrets.

Additional protection covers conventional secrets inside readable project paths.
On Linux, files already denied by the read policy need no mount namespace.
Other protected files require a namespace overlay. If the host cannot create this
overlay, LPM CLI refuses to run the script. Windows uses file access rules.

The Linux and Windows secret scan captures existing files before the script starts.
It has depth limits and excludes generated directories. It cannot protect a new
secret placed inside an already readable directory after the scan.

#### Opting specific files back in

Some builds need a project secret, such as a database URL or CA bundle. These fields grant access to specific files:
...
The CLI combines `sandboxReadAllow` with the user-level `script-read-allow` list.
Each entry must name a file inside the project. Directories, symlinks, and paths
outside the project are rejected. Missing files grant no current access.

Both read grants and `sandboxWriteDirs` require script approval. Adding or changing
a path requires another approval, including paths from user configuration.
The approval prompt shows the resolved paths. Writable directories also permit reads.
Built-in writable directories cannot be symlinks or Windows reparse points.

An approved secret file remains readable. Linux needs no namespace overlay when
all otherwise-readable secret files have explicit approval.

### Timeouts, cancellation, and child processes

Ctrl+C and SIGTERM cancel active dependency scripts. LPM CLI waits for cleanup
before it returns an error. An interrupted script does not receive a successful
build marker. The `--timeout` limit applies to each lifecycle phase.

Linux scripts cannot create a separate process group or session. LPM CLI terminates
the group after success, failure, timeout, or cancellation. Windows uses a Job
Object to contain and terminate descendants.

**macOS limitation:** native scripts can detach or reparent child processes.
Those descendants can survive completion, timeout, or cancellation. Their filesystem
and network restrictions remain active, but cleanup cannot guarantee termination.
LPM CLI preserves native build compatibility instead of blocking all child processes.
For complete process-lifetime isolation, use a disposable virtual machine or container.

`--no-sandbox` removes the Linux process-group restriction along with filesystem,
