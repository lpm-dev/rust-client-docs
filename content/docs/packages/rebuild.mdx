...
- the complete environment exposed to the lifecycle script, including custom
  variables plus LPM's effective `PATH`, `INIT_CWD`, `TMPDIR`, `TMP`, and
  `TEMP` values
...
runs that restore the LPM store into the same workspace path; a differently
...
normalizes them before invoking LPM.
...
samples), a local hit completed in **32.33 ms median** versus a **138.03 ms**
strict cache miss: **4.27× faster** and **76.58% lower wall time**. Recreating a
CI checkout at the same stable path while retaining the warm LPM store
completed in **29.50 ms median** (**4.68× faster**, **78.63% lower**). Every
measured scenario used the same strict sandbox, and each artifact used 134,444
bytes. The benchmark and raw methodology live in the rust-client repository
under `bench/scripts/native-build-cache-benchmark.mjs` and
