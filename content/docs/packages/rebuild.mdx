...
- `package.json > lpm > trustedDependencies` (rich-map form, with an exact source/content identity key plus matching integrity + script hash), OR
- A scope glob in `package.json > lpm > scripts.trustedScopes` (e.g. `@myorg/*`)

Legacy name-only arrays remain visible to the review/migration path but do not authorize execution because they cannot identify reusable script bytes.
...
`.lpm-built` state. Before a v2 command runs, LPM CLI restores and revalidates
the immutable post-patch, pre-build lifecycle baseline whose hash passed the
trust gate, so stale outputs cannot leak into the replacement artifact and
patched bytes cannot differ from the reviewed bytes. Corrupt, incomplete, or
over-budget baselines and artifacts fail closed.

`--force` always bypasses lookup, rebuilds from the verified lifecycle baseline,
and replaces the artifact for the resulting key.
