...
Recursive installs report workspace-union convergence under each target's `timing.resolve.workspace_union` and aggregate each union group once at `timing.workspace_union`. `expansion_pass_count` counts complete re-resolutions after ambient peer roots are added; it does not include the initial resolution. `expansion_capped` becomes `true` after four expansion passes, when unconverged importers fall back to isolated resolution. `isolated_importer_count` gives the total fallback count, while `isolated_importer_reasons` separates ineligible root specifiers, release-age policy checks, projection failures, the expansion cap, and stalled expansion.
