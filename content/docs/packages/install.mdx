...
LPM applies a **24-hour** minimum release age by default. On project installs, the cooldown is enforced for direct/root dependencies: the package specs declared by the project, plus packages you pass to `lpm install <pkg>`. For ranges on those direct packages, the resolver skips candidates that are still inside the cooldown window and picks the newest older candidate that still satisfies the range. Exact direct pins to a too-new version still fail, because there is no older version that can satisfy the exact request.

Transitive dependencies of an allowed direct package are not separately halted by this cooldown. That keeps a mature direct package on its normal dependency graph instead of downgrading it only because one of its children was published recently.
