...
When `~/.lpm/config.toml > [firewall] mode` is set to `report` or `enforce`, install sends selected public npm package versions to firewall.lpm.dev in batches. Metadata and tarballs still use the direct npm route; the firewall request only asks for block/warn verdicts.
