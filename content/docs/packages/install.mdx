...
With `--json --timing`, source analysis appears under `timing.fetch_breakdown.source_scan` as `sum_ns` and `max_ns`. On the fused cold-install path this work happens while extraction is running, so it is already included in cold install wall time and `extract_ms`; do not add it to the total again.
