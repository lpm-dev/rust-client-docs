...
A normal install shows Critical, High, and Medium results under `Security summary`. These results need attention because they include obfuscation, shell execution, native bindings, and unsafe dependency declarations.
...
```bash
lpm --verbose install  # show Behavioral metadata after the install
