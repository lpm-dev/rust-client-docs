...
Package source is analyzed locally while its extracted bytes enter the store. The scanner records 22 behavioral, supply-chain, and manifest signals in `.lpm-security.json` next to the stored package bytes. This applies to npm, custom-registry, and `@lpm.dev/*` packages; it does not depend on an LPM.dev Registry package being present.
...
For `@lpm.dev/*` packages only, LPM CLI makes one batch metadata request to the configured LPM.dev Registry and merges registry behavioral tags, vulnerabilities, lifecycle-script metadata, and AI security findings into the local result. npm-only installs still receive the local security summary; they do not trigger this LPM.dev Registry enrichment request. The separately configured [LPM Firewall](/docs/guides/firewall) can make its own verdict requests for npm packages.

`--no-security-summary` skips the post-install cache aggregation and human report. It does not disable the extraction-time local scan or remove existing `.lpm-security.json` cache files.
