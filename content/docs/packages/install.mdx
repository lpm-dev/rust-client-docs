...
Before a new direct dependency enters the project, LPM checks the package name for common typosquatting patterns against popular packages. A likely typo such as `axois` fails in `--json`, CI, non-TTY, and `--yes` runs before `package.json` is changed; in an interactive terminal LPM defaults to cancel and only switches to the suggested package or writes a committed allow-list entry to [`lpm.toml`](/docs/reference/lpm-toml#typosquat-policy) when you choose that action. Existing lockfile replays are not blocked by this check.
...
In interactive terminals, a single suspicious CLI package can be replaced with the suggested package or committed to the project allow-list. In CI, `--json`, non-TTY, and `--yes` runs, LPM exits `1` with `error_code: "typosquat_suspected"` and leaves the manifest untouched.
...
When `~/.lpm/config.toml > [firewall] mode` is set to `report` or `enforce`, install sends selected public npm package versions to firewall.lpm.dev in batches. Metadata and tarballs still use the direct npm route; the firewall request only asks for block/warn verdicts. The hosted verdict service is an LPM Pro/org feature, so active modes send LPM auth; run `lpm login` before enabling `report` or `enforce`.
...
`report` prints malicious or warning verdicts and lets the install continue. If firewall.lpm.dev denies entitlement or is unreachable, report mode warns and continues. `enforce` blocks packages marked malicious before their tarballs are allowed through, and fails before package bytes are materialized when entitlement is denied. Configure it with:

```bash
lpm config firewall --set report
lpm config firewall --set enforce
```

Disabling or downshifting the firewall after it becomes part of the approved machine posture is guarded by [`lpm security`](/docs/infra/security).

## Policy extensions

Install can run local policy extensions from `~/.lpm/config.toml > [policy.extensions.<name>]`. Extensions receive the resolved package candidates as JSON on stdin and return verdicts as JSON on stdout. LPM spawns the configured command directly; it does not use `sh -c` or interpolate package data into a command string. The command's first entry must be an absolute path or a program name found on an absolute `PATH` directory; relative executable paths are rejected.
...
LPM applies a **24-hour** minimum release age by default. On project installs, the default policy checks direct/root dependencies: the package specs declared by the project, plus packages you pass to `lpm install <pkg>`. For ranges on checked packages, the resolver skips candidates that are still inside the cooldown window and picks the newest older candidate that still satisfies the range. Exact pins to a too-new version still fail, because there is no older version that can satisfy the exact request.
...
When enabled, LPM refuses npm versions whose trust evidence is weaker than an earlier published version of the same package. Ranges can fall back to an older allowed candidate; an exact pin to a blocked version fails with the trust-policy reason.
...
- [`lpm upgrade`](/docs/packages/upgrade) — bump eligible LPM and npm deps to their latest matching range
