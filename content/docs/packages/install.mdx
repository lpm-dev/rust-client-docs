...
Artifact availability does not weaken that contract. If a registry no longer serves an artifact pinned by an existing lockfile, mutable `lpm install`, `lpm install --frozen-lockfile`, and `lpm ci` all fail without deleting, truncating, rewriting, or replacing `lpm.lock` or `lpm.lockb`. A mutable install does not silently select a newer version or a different source. To change the pin, run the command reported by the error, such as:

```bash
lpm upgrade <package>
```

Run the upgrade in a mutable development environment, review the result, and commit the updated lockfiles before retrying a frozen or CI install.
