...
If the registry cannot confirm the report, the command returns `pool_attribution_unconfirmed`. Retry the same command to reuse cached packages and resend the report. If the error requests a CLI update, update LPM CLI first.
