...
Source identity is preserved during linking. A registry package, tarball, `file:` package, and `link:` package may share the same `name@version`; each dependency edge points at the source declared by that edge instead of guessing from name and version alone. Global v2 graph identity includes registry origin and content integrity, while local `file:` and `link:` identity is qualified by the resolved project path. Two projects can therefore use the same relative source path without sharing a link entry. Local source snapshots refresh when you run `lpm install` again.
...
Version-carrying peer specs are parsed once and checked consistently by every resolver. For example, `work:^18` is checked as `^18`, while `"react-compat": "npm:react@^17 || ^18"` checks both alternatives against the installed `react` version and preserves `react-compat` as the local module name. Repeating the alias in each union alternative is also supported. `workspace:^` and `workspace:~` are presence constraints; explicit `workspace:` ranges keep their semver constraint.

`file:`, `link:`, and URL peer specs require an explicitly declared provider installed under the peer's local dependency name with the same source identity. The fetched package's own manifest name may differ from that local name: for example, `"react-compat": "file:./react"` can provide a `react-compat` peer even when `./react/package.json` names the package `react`. File and link providers remain explicit providers when their paths also identify workspace members in both v1 and v2 store modes, and their manifest versions do not need to be valid semver because matching is source-based. A relative `file:` or `link:` peer declared by a workspace member resolves from that member's directory, then is compared with providers in project-relative coordinates.

Source identity is exact. A registry package with the same name or version is not equivalent, and a URL's declared SRI fragment participates in its identity; the same URL pinned to different hashes is different content. A verified declared SRI remains the stored source identity instead of being rewritten to another hash algorithm. Missing npm-alias peers can be auto-installed from their canonical registry target. While peer auto-installation is enabled, a missing required non-registry or named-registry peer fails with an explicit source-routing error; optional peers and installs with peer auto-installation disabled do not take that failure path. Unknown protocols and malformed source specs are rejected instead of becoming unconstrained presence checks. Git dependency installation is not currently supported, so a git peer cannot be supplied explicitly.
...
Store v2 binds lifecycle trust to an immutable post-patch, pre-build package baseline, not the mutable directory left behind by a prior build. Immediately before a trusted script runs, LPM restores that baseline and verifies its exact content hash again. Normal generated output therefore does not reopen an approval, while baseline drift or a package tree that exceeds the bounded hashing budget remains blocked.
...
- `--allow-snapshotless-lockfile` — with `--offline`, explicitly permits compatibility replay of a legacy lockfile without a root importer snapshot. Human-readable output warns that stale roots or peer-role transitions may remain.
...
Legacy or imported lockfiles that do not contain a root importer snapshot cannot prove that their package graph still matches `package.json`. A mutable online install re-resolves and rewrites that graph; frozen and offline installs fail and ask you to refresh the lockfile. If an online rebuild is impossible, combine `--offline --allow-snapshotless-lockfile` to opt into compatibility replay. The warning is load-bearing: without an importer snapshot, removed roots and direct-peer-to-ambient transitions cannot be validated and stale packages may remain.
...
Re-running scripts after `lpm approve-scripts --global` requires `lpm uninstall -g --preserve-trust <pkg> && lpm install -g <pkg>` for each affected top-level global. The approval output prints these commands. `lpm rebuild --global` is a planned follow-up.
...
| `--allow-snapshotless-lockfile` | With `--offline`, permit legacy replay without a root importer snapshot and warn about stale-graph risk |
