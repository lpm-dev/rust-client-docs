...
`file:`, `link:`, and URL peer specs require an explicitly declared provider installed under the peer's local dependency name with the same source identity. The fetched package's own manifest name may differ from that local name: for example, `"react-compat": "file:./react"` can provide a `react-compat` peer even when `./react/package.json` names the package `react`. File and link providers remain explicit providers when their paths also identify workspace members in both v1 and v2 store modes, and their manifest versions do not need to be valid semver because matching is source-based.
