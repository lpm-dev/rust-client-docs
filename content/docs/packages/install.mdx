...
Before a new direct dependency enters the project, LPM checks the package name for common typosquatting patterns against popular packages. A likely typo such as `axois` fails in `--json`, CI, non-TTY, and `--yes` runs before `package.json` is changed; in an interactive terminal LPM can switch to the suggested package or write a committed allow-list entry to [`lpm.toml`](/docs/reference/lpm-toml#typosquat-policy). Existing lockfile replays are not blocked by this check.
...
`LPM_TIMING_DETAIL=1` adds `timing.detail` with metadata request attribution by purpose, resolver wall/metadata/scheduler/CPU/policy counters, fetch-stage plan/cache/download attribution, cache-classification branch timing, registry-signature/provenance timing, link substages, and tail writes such as lockfile and build-state persistence. `LPM_TIMING_DETAIL=trace` also adds `timing.detail.trace.slow_packages` buckets for the slowest tarball HTTP, extract, security, finalize, and provenance verification work, plus per-purpose duplicate metadata request rankings.
