...
The normal summary does not use registry vulnerabilities or AI security findings. [`lpm audit`](/docs/packages/audit) and audit-after-install add those findings.
...
After a successful install, LPM CLI can run [`lpm audit`](/docs/packages/audit) and print a compact summary. This feature is optional.
...
The audit is **informational only**. Its findings never fail the install. Run [`lpm audit --fail-on=<level>`](/docs/packages/audit) for a gating audit. The feature is **disabled by default**.
...
Audit-after-install uses the same scan and severity policy as `lpm audit`. It counts each advisory separately and preserves Critical registry security findings.

The summary prints every Critical finding below the first line. Findings with lower severity remain available through `lpm audit`.
