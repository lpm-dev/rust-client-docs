...
3. **Link.** `node_modules/<pkg>` is created as a symlink into the global store at `~/.lpm/store/v2/links/<graph-key>/`. Single packages default to **hoisted** (npm-style flat); workspaces auto-default to **isolated** (pnpm-style strict-deps). Override per-invocation with `--linker=isolated` or `--linker=hoisted`.
...
- **The script-policy chain collapses.** With no project-level `package.json > lpm > scriptPolicy` to read, the precedence chain on `-g` is CLI flag > `~/.lpm/config.toml > script-policy` > default. The `~/.lpm/config.toml > minimum-release-age-secs` chain works the same way.
...
| `--linker <isolated\|hoisted>` | Linking layout (default: `hoisted`; workspaces auto-default to `isolated`) |
