...
`lpm install --json` always includes a compact `timing.waterfall` object for setup, resolve, fetch, link, and tail attribution. For deeper install profiling, opt in per run:
...
The default JSON envelope stays compact. Use detail mode for local performance investigations and cold/warm install comparisons, not as a stable production API.
...
When `~/.lpm/config.toml > [firewall] mode` is set to `report` or `enforce`, install sends selected public npm package versions to the lpm.dev firewall verdict endpoint in batches. Metadata and tarballs still use the direct npm route; the lpm.dev request only asks for block/warn verdicts.

`report` prints malicious or warning verdicts and lets the install continue. `enforce` blocks packages marked malicious before their tarballs are allowed through. Configure it with:
