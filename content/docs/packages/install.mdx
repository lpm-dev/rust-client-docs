...
LPM CLI discovers the workspace once and runs each selected project through the normal install and security pipeline. Member project lifecycles run in dependency order; the root lifecycle runs last. A failure stops later targets. Cyclic graphs fall back to stable path order.
