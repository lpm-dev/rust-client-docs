...
`file:`, `link:`, and URL peer specs require an explicitly declared provider installed under the peer's local dependency name with the same source identity. The fetched package's own manifest name may differ from that local name: for example, `"react-compat": "file:./react"` can provide a `react-compat` peer even when `./react/package.json` names the package `react`. File and link providers remain explicit providers when their paths also identify workspace members, and their manifest versions do not need to be valid semver because matching is source-based.
...
Legacy or imported lockfiles that do not contain a root importer snapshot cannot prove that their package graph still matches `package.json`. A mutable online install re-resolves and rewrites that graph; a frozen install fails and asks you to refresh the lockfile. `--offline` deliberately retains compatibility replay for a snapshotless lockfile because offline mode cannot re-resolve it. Run one mutable online install when possible to upgrade it to importer-validated state.
