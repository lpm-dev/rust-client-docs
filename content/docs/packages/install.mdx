...
Declare a local `.tgz` archive in `package.json`:
...
Then run `lpm install`.
LPM CLI installs the archive and its registry dependencies.
It preserves dependency aliases and optional dependencies, and excludes development dependencies from the archive manifest.
Nested `file:`, `link:`, workspace, Git, and archive dependencies inside the archive are unsupported.

LPM CLI records the archive content hash in `lpm.lock`.
If the archive changes, `lpm install` installs the new content and updates the lockfile.
[`lpm ci`](/docs/packages/ci) rejects changed archive content and preserves the lockfile.
The archive must remain available at its declared path, including during frozen installs with an empty store.
`lpm install --force` reads the local archive again.

Relative archive paths cannot contain `..` components.
Use an absolute path for an archive outside the project.
Local archive reads have a 500 MiB limit.

### Newly published versions

If cached metadata cannot satisfy a requested version or range, LPM CLI refreshes that package's metadata once before reporting it unavailable.
This applies to LPM.dev Registry, public npm, and custom registries from `.npmrc`.
A new publication does not require you to clear the entire cache.
Registry access checks and minimum release age still apply after the refresh.
If the version remains unavailable, the install preserves the previous manifest and lockfile.

### Windows paths

The package store supports file paths longer than the traditional Windows limit of 260 characters.
External tools can still impose their own path limits.
For projects with native build tools, a short checkout path reduces compatibility problems.
...
API capabilities appear separately under `Capabilities`. They do not increase security warning counts.
Runtime evaluation, child processes, and dynamic module loading can be legitimate package features.
[`lpm audit`](/docs/packages/audit#source-capabilities) includes source evidence and supports explicit policies that restrict these capabilities.

The Critical `:credential-exfiltration` finding reports supported secret flows into network payloads. See [source detection coverage](/docs/packages/audit#credential-exfiltration).
...
Verbose install output lists informational API use under `Capabilities` and artifact traits under `Behavioral metadata`.
Each query hint contains the tags in that section. Environment-variable access uses `:env`, and URL literals use `:url-strings`.
...
If an older registry explicitly rejects graph reports, LPM CLI retries with package roots in batches of 200. Successful installs show a warning about incomplete dependency credit. Other registry errors still return a failure.
