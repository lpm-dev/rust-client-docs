...
Source identity is exact. A registry package with the same name or version is not equivalent, and a URL's declared SRI fragment participates in its identity; the same URL pinned to different hashes is different content. Missing npm-alias peers can be auto-installed from their canonical registry target, but a missing non-registry or named-registry peer fails with an explicit source-routing error. Unknown protocols and malformed source specs are rejected instead of becoming unconstrained presence checks. Git dependency installation is not currently supported, so a git peer cannot be supplied explicitly.
