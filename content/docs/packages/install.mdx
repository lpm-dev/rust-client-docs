...
| `allow` (`--yolo`) | Run every dependency lifecycle script during install |
...
LPM applies a **24-hour** minimum release age by default. For version ranges, the resolver skips candidates that are still inside the cooldown window and picks the newest older candidate that still satisfies the range. Exact pins to a too-new version still fail, because there is no older version that can satisfy the exact request.
...
Set persistent defaults via `package.json` (`lpm.minimumReleaseAge`, `lpm.minimumReleaseAgeExclude`) or `~/.lpm/config.toml` (`minimum-release-age-secs`, `minimum-release-age-exclude`). Excludes are exact canonical package names only: for `"local": "npm:real-pkg@1.0.0"`, exclude `real-pkg`, not `local`. CLI, package, and global exclude lists merge in that order, with duplicates removed.
