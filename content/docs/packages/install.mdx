...
When `~/.lpm/config.toml > [firewall] mode` is set to `monitor` or `enforce`, install sends selected public npm package versions to firewall.lpm.dev in batches. Metadata and tarballs still use the direct npm route; the firewall request only asks for block/warn verdicts. The hosted verdict service is an LPM Pro/org feature, so active modes send LPM auth; run `lpm login` locally, or use `LPM_TOKEN` / registry-audience OIDC in CI.
...
`monitor` prints would-block and warning verdicts and lets the install continue. If firewall.lpm.dev denies entitlement or is unreachable, monitor mode warns and continues. `enforce` blocks packages whose effective firewall action is `block` before their tarballs are allowed through, and fails before package bytes are materialized when entitlement is denied. Configure it with:
