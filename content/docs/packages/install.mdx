...
Legacy or imported lockfiles that do not contain a root importer snapshot cannot prove that their package graph still matches `package.json`. A mutable online install re-resolves and rewrites that graph; a frozen install fails and asks you to refresh the lockfile. `--offline` continues to support compatibility replay of a snapshotless lockfile because it cannot re-resolve; run one mutable online install when possible to upgrade it to importer-validated state.
