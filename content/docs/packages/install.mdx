...
4. **Skip scripts.** Lifecycle scripts do **not** run by default. The install pipeline executes `preinstall`, `install`, and `postinstall`; other names like `prepare` and `prepublishOnly` are recognized for detection but never executed. See [lifecycle scripts](#lifecycle-scripts) below.
...
| `--auto-build` | Forces the rebuild attempt under triage with mixed-trust trees |
