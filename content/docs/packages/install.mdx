...
Scheme-carrying peer specs use their comparable version body during these checks. For example, `work:^18` is checked as `^18`, and `"react-compat": "npm:react@^18"` checks the installed `react` version while preserving `react-compat` as the local module name. `workspace:` ranges are normalized the same way, `||` unions normalize each alternative, and `file:`, `link:`, git, or URL specs without a version body compare as `*`.

Missing npm-alias peers can be auto-installed from their canonical registry target. A missing required peer whose original spec needs non-registry or named-registry source routing fails with an explicit source-routing error instead of silently installing a same-named package from the default registry; install a compatible provider explicitly in that case.
