...
Project approvals are bound to **`{name, version, source, integrity, scriptHash}`** — any change to the installed source, package content, or scripts re-opens the package for review on the next install. The strict binding is what makes the deny-by-default model usable: you grant trust to one install identity, not to a name.
...
lpm approve-scripts 'esbuild@0.25.1#<identity-token>' # disambiguate equal name/version sources
...
The destination depends on scope: project approvals land in `package.json`, global approvals land in `~/.lpm/global/trusted-dependencies.json`. Both stores use source-, integrity-, and lifecycle-content-qualified keys so equal-coordinate packages can retain independent approvals, including two revisions that differ only in their lifecycle content hash.
...
```json title="package.json"
{
  "lpm": {
    "trustedDependencies": {
      "esbuild@0.25.1#0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef": {
...
The `#` suffix is an opaque 64-character lowercase hexadecimal token derived from the exact `source`, `integrity`, and `scriptHash`. Source-less coordinate-only rich keys remain readable as migration input, but cannot authorize a modern source-qualified package; approving that package writes a new exact key.

### Global scope (`--global`)

Approvals land in `~/.lpm/global/trusted-dependencies.json` — a separate per-machine trust file. Global bindings store source, integrity, and script hash plus a `provenanceAtApproval` snapshot used by the install-time drift gate:
...
The global `#` suffix follows the same source, integrity, and lifecycle-content identity contract as project selectors. Schema-1 `name@version` entries remain readable for migration, but they do not grant strict lifecycle trust; approving the package again writes a schema-3 exact binding.
...
For project and global trust, a different `source`, `integrity`, or `scriptHash` does not match the approved identity and the package re-enters the blocked set. (See [`lpm trust diff`](/docs/packages/trust) to inspect drift events for project trust.)

## Modes

| Mode | Behavior |
| --- | --- |
| (default — interactive at TTY) | Walk the blocked set one package at a time. Cards show `name@version`, source, an exact copyable selector, integrity, script hash, the lifecycle phases the package declares, the [static tier](/docs/packages/security-audit#layer-5-triage) (green / amber / red), and a binding-drift warning when this package was previously approved and the script content has changed since. Approve / skip / inspect per package. |
| `--list` | Read-only — print the blocked set, including source and exact selector for disambiguating equal-coordinate rows. No prompts or mutations. Mutually exclusive with `--yes`. |
...
| `<pkg>` | Approve a specific package directly. Skips the interactive walk and auto-confirms (no TTY prompt). Accepts `name`, `name@version`, or the exact `name@version#identity-token` selector. If a shorter selector matches multiple blocked rows, LPM CLI exits non-zero and lists exact copyable selectors. This is the correct non-interactive path for single-package approvals. |
...
After a successful global approval, the banner enumerates the affected top-level globals and prints `lpm uninstall -g --preserve-trust <pkg> && lpm install -g <pkg>` for each. `--preserve-trust` is deliberately limited to this immediate approved-reinstall workflow; an ordinary global uninstall prunes approvals that are no longer reachable. The `--json` envelope carries the same information as `next_step.origins` and includes the exact commands in `next_steps[]`. `--dry-run` omits both follow-up fields since no mutation occurred. Until `lpm rebuild --global` ships, reinstalling each affected origin is the only way to execute the newly approved scripts.
...
The envelope carries `schema_version`, `success`, `command`, `dry_run`, `blocked_count`, `approved_count`, and `skipped_count`. In project scope, `blocked[]` rows contain `name`, `version`, `source`, `identity_selector`, `integrity`, `script_hash`, `phases_present`, `static_tier`, and `binding_drift`. `identity_selector` is the exact value automation can pass back as `<pkg>` when equal-coordinate rows coexist. Successful project approvals that actually write trust include `next_steps: [{description, command}]` with `command: "lpm rebuild"`. Under `--global`, rows carry the same `source` and `identity_selector` fields plus `origins` (the top-level globals that pulled the package in); list/empty-set envelopes include `unreadable_origins`, and successful `--global --yes` runs add both `next_step: {kind: "reinstall_globals", origins: [...]}` and `next_steps[]` command objects containing `lpm uninstall -g --preserve-trust … && lpm install -g …`. `--dry-run` omits follow-up commands since no mutation happened.

## Flags

| Flag | Effect |
| --- | --- |
| `<package>` | Approve a specific package directly (`name`, `name@version`, or `name@version#identity-token`). Auto-confirms in non-TTY / JSON mode. |
