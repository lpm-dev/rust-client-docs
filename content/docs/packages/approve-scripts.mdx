...
The destination depends on scope: project approvals land in `package.json`, global approvals land in `~/.lpm/global/trusted-dependencies.json`. Project keys include a source/content identity token; global keys remain `name@version` and bind integrity, script hash, and approval-time provenance.
...
Approvals land in `~/.lpm/global/trusted-dependencies.json` — a separate per-machine trust file. Global bindings store integrity and script hash plus a `provenanceAtApproval` snapshot used by the install-time drift gate:
...
For project trust, a different `source`, `integrity`, or `scriptHash` does not match the approved identity and the package re-enters the blocked set. Global trust applies the same rule to `integrity` and `scriptHash`. (See [`lpm trust diff`](/docs/packages/trust) to inspect drift events for project trust.)

## Modes

| Mode | Behavior |
| --- | --- |
| (default — interactive at TTY) | Walk the blocked set one package at a time. Project cards show `name@version`, source, an exact copyable selector, integrity, script hash, the lifecycle phases the package declares, the [static tier](/docs/packages/security-audit#layer-5-triage) (green / amber / red), and a binding-drift warning when this package was previously approved and the script content has changed since. Approve / skip / inspect per package. |
...
The envelope carries `schema_version`, `success`, `command`, `dry_run`, `blocked_count`, `approved_count`, and `skipped_count`. In project scope, `blocked[]` rows contain `name`, `version`, `source`, `identity_selector`, `integrity`, `script_hash`, `phases_present`, `static_tier`, and `binding_drift`. `identity_selector` is the exact value automation can pass back as `<pkg>` when equal-coordinate rows coexist. Successful project approvals that actually write trust include `next_steps: [{description, command}]` with `command: "lpm rebuild"`. Under `--global`, rows remain coordinate-based and carry `name`, `version`, `integrity`, `script_hash`, `phases_present`, `binding_drift`, and `origins` (the top-level globals that pulled the package in); list/empty-set envelopes include `unreadable_origins`, and successful `--global --yes` runs add both `next_step: {kind: "reinstall_globals", origins: [...]}` and `next_steps[]` command objects telling the caller which `lpm uninstall -g … && lpm install -g …` pairs to run. `--dry-run` omits follow-up commands since no mutation happened.
