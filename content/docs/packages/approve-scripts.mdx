...
On Windows, script approval does not replace [administrator sandbox setup](/docs/infra/doctor#windows-sandbox-permission-setup).
Protected parent directories or system-installed tools can require that setup before an approved script runs.
Later installs and scripts run from your normal terminal.
