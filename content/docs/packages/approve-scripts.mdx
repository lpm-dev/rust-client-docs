...
The destination depends on scope: project approvals land in `package.json`, global approvals land in `~/.lpm/global/trusted-dependencies.json`. Both stores use source/content-qualified keys so equal-coordinate packages can retain independent approvals.
...
The `#` suffix is an opaque 64-character lowercase hexadecimal token derived from the exact `source` and `integrity`. Legacy coordinate-only rich keys remain readable and are migrated when that package is approved again.
...
The global `#` suffix follows the same opaque source/content identity contract as project selectors. Schema-1 `name@version` entries remain readable for migration, but they do not grant strict lifecycle trust; approving the package again writes a schema-2 exact binding.
