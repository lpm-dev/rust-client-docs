...
`trust diff` is informational — it always exits zero and prints whatever it found. To turn it into a CI gate, parse `--json`:

```bash
lpm trust diff --json | jq -e '(.added | length) + (.removed | length) + (.changed | length) == 0'
```

`jq -e` exits non-zero when the boolean evaluates to false, so any non-empty diff fails the step.

The JSON envelope: `schema_version`, `command`, `snapshot_captured_at`, `current_binding_count`, `added[]`, `removed[]`, `changed[]`. Each entry carries `key` (e.g., `esbuild@0.25.1`), `previous`, and `current` (the integrity / scriptHash before and after).
...
The JSON envelope: `schema_version`, `command`, `dry_run`, `mutated` (the actual final state — `false` on `--dry-run` or no-op), `stale_count`, `stale[]` (the keys removed).
...
`trust diff` itself doesn't fail on drift; pipe through `jq -e` (or any tool that asserts on the JSON shape) to turn it into a gate. The intent is "fail the build if `trustedDependencies` changed since the last reviewed install."
