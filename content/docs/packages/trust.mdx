...
- **Same-key binding changes** — same approval key, but a different `integrity` or `scriptHash`. A source/content identity change normally appears as one removed key plus one added key because the identity token changes.
...
The JSON envelope: `success`, `schema_version`, `command`, `assertion_failed`, `diff_count`, `snapshot_captured_at`, `current_binding_count`, `added[]`, `removed[]`, `changed[]`. Each entry carries `key` (for example, `esbuild@0.25.1#<identity-token>`), `previous`, and `current` (the binding before and after). When `--assert-none` fails in JSON mode, LPM CLI emits this same single diff envelope with `success: false`, `error_code: "trust_diff_assert_none"`, and exits non-zero.
