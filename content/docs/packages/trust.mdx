...
Mutating policy commands use atomic file replacement. A duplicate addition does not rewrite the destination file.
...
`remove` narrows the security policy and does not need an unlock. It also removes the signed authorization for that scope.
...
The JSON envelope contains `success`, `schema_version`, `command`, `scope`, `action`, `changed`, and `scopes`. The `add` and `remove` actions also contain `selector`.
...
`add` is idempotent. If the selector exists, the command does not rewrite `package.json`.
...
The JSON envelope contains `success`, `schema_version`, `command`, `scope`, `action`, `changed`, and `exclusions`. The `add` and `remove` actions also contain `selector`.
