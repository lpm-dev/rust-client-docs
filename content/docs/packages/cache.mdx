...
If the project registry is corrupt, pending tombstones cannot be inspected, or the `--apply` tombstone sweep fails, prune exits non-zero. In JSON mode, `success` is `false` and the matching diagnostic field (`registry_corrupt_reason`, `tombstone_count_error`, or `tombstone_sweep_error`) carries the reason.
