...
`lpm cache status` reports the local task-cache path and size. With remote caching enabled through `lpm.json` or `LPM_REMOTE_CACHE`, it also reports hosted status, usage, and quota.

See [Remote task cache](/docs/infra/remote-cache) for setup, namespace selection, and CI examples. `lpm cache clean tasks` clears local task results without deleting hosted artifacts.
...
- **In-flight installs are respected.** Prune coordinates with concurrent `lpm install`, `lpm audit`, and `lpm query` invocations through the store reader/writer lock — dry-run takes the shared half, `--apply` takes the exclusive half. See [`lpm store` — Locking model](/docs/infra/store#concurrent-operations).
