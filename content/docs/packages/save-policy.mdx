...
| `lpm install --catalog zod` | `"zod": "catalog:"` | Explicit default-catalog save when `catalogs.default.zod` matches the resolved version |
...
`--catalog` is a save policy, not a catalog editor. The target entry must already exist in root `package.json > catalogs`, and the resolved version must satisfy that catalog range. On mismatch or missing entry, LPM rolls `package.json` back and exits with an error.
