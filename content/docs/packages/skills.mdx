...
An explicit package version selects that release's skills. Use an exact version, such as `@1.0.0`; ranges and tags are not accepted.

Updates and package removal stop if the installed skill directory contains edited files, untracked files, or an invalid ownership manifest. Your files remain intact. Back up and move `.lpm/skills/owner.package/`, then retry the command. An unchanged package set can be replaced when a release removes or renames skills.

If skills are pending review or unavailable after review, `lpm skills add` reports an error and preserves the installed set. Automatic package installation also reports the unavailable review. Use `--no-skills` to install the package independently, then retry the skill command after approval.
...
| Frontmatter | Valid YAML with unique keys. `name` and `description` are required strings. `version` is an optional string; `globs` is an optional array of strings. |
...
Duplicate keys, unclosed quotes, and non-string glob entries fail validation. YAML folded and literal descriptions are supported. Quote numeric-looking names and versions to keep them strings.
