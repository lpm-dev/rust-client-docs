...
When multiple agent directories link to the same external skill directory, the dashboard shows one skill with every agent target instead of duplicate rows. It resolves the top-level agent alias for inspection but still rejects symlinks inside the skill tree. Broken agent aliases remain visible as `broken-link`, use explicit missing-target language, and appear under **needs attention**; `lpm skills doctor --global` reports their exact paths.
