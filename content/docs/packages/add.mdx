...
When npm firewall mode is `report` or `enforce`, `lpm add` checks the selected source package when it is a routed public npm package. `enforce` blocks packages marked malicious before the source tarball is downloaded or copied. lpm.dev packages and other `.npmrc`-declared private/custom registry packages are not sent to the npm firewall verdict API. Dependencies installed after a config-aware source copy use the normal install pipeline and its firewall checks.
