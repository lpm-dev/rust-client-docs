...
When npm firewall mode is `report` or `enforce`, `lpm add` checks the selected source package with firewall.lpm.dev when it is a routed public npm package. The hosted verdict service is an LPM Pro/org feature, so active modes send LPM auth; run `lpm login` before enabling it. `report` warns and continues when entitlement is denied, while `enforce` blocks packages marked malicious or denied by entitlement before the source tarball is downloaded or copied. In human output, the source-package download line shows `🔥 LPM Firewall active` when that check is active. lpm.dev packages and other `.npmrc`-declared private/custom registry packages are not sent to the npm firewall verdict API. Dependencies installed after a config-aware source copy use the normal install pipeline and its firewall checks.
