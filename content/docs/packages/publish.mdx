...
Auth uses the same npm token sources as `lpm publish --npm`:
...
Publishing to staging does not prompt for OTP. npm defers proof-of-presence to `lpm stage approve` and `lpm stage reject`, where `--otp` or the usual npm web-auth retry flow is used.
...
Every `lpm stage` subcommand supports `--json`. JSON output uses LPM envelopes with `success`, `target: "npm"`, `registry`, `stageId` when a single staged package is involved, and `data` for the registry response.
