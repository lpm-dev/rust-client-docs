...
lpm publish --wait                           # wait for public LPM.dev availability
lpm publish --wait --wait-timeout 900        # wait for up to 15 minutes
...
Private LPM.dev Registry versions are active after upload. Pool and Marketplace versions can require publication review before they become public.

For a pending version, LPM CLI shows these separate values:

- The uploaded version.
- The publication state.
- The current latest public version, if one exists.

Use `--wait` when CI must wait for public availability:

```bash
lpm publish --wait
lpm publish --wait --wait-timeout 900
```

LPM CLI polls every three seconds. The default timeout is 600 seconds. You can set a timeout from 10 through 3600 seconds.
...
These results do not undo the upload. Do not publish the same version again after a wait failure.
...
| `--wait` | Wait until an LPM.dev Registry upload becomes publicly available |
| `--wait-timeout <SECONDS>` | Set the wait limit from 10 through 3600 seconds. The default is 600 seconds |
