...
Every target is packed and scanned for hardcoded secrets. When LPM.dev Registry is one of the targets, LPM CLI also computes the local quality score and enforces `--min-score`. Normal npm, GitHub Packages, GitLab Packages, and custom-registry publishes do not run that quality gate.
...
lpm publish --min-score 60                   # gate an LPM.dev publish on local quality
...
2. **Pack** — assemble an in-memory tarball from the package manifest. LPM CLI rejects compressed tarballs over **500 MiB** during local preparation. LPM.dev Registry currently enforces a smaller **100 MiB** upload cap; other targets can impose their own limits.
...
5. **Secret scan** — scan supported text files in the project tree, excluding `.git`, `node_modules`, and paths ignored by Git. Skip with `--allow-secrets` only when you have reviewed the package carefully.
6. **Quality score** (LPM.dev Registry target only) — compute the local readme/license/types/tests/maintenance signals. With `--min-score N`, abort if the local score is below the threshold.
...
`--dry-run` runs the same local preparation. For an LPM.dev Registry target, it may also attempt an available CI OIDC exchange and, when authored skills exist, fetch the previously published skills for a staleness warning. Failures from those two optional reads do not fail the dry run. It does not resolve npm/GitHub/GitLab upload credentials, check publish permission or version availability, generate `--provenance`, or upload anything. A supplied `--provenance-file` is still validated locally.

Neither mode is currently guaranteed to leave the working tree untouched. When authored skills exist and `package.json > files` does not include `.lpm/skills`, publish preparation adds that entry before packing. Review the resulting manifest change and commit it when appropriate.
...
| Flag | Effect |
| --- | --- |
| `--dry-run` | Prepare and show the publish plan without resolving upload auth or uploading |
