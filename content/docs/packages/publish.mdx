...
You must be logged in to the chosen target — see [Authentication](/docs/reference/authentication) for `lpm login --npm`, `--github`, `--gitlab`, or `lpm login --login-registry <URL>` for custom.
