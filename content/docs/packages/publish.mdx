...
If published skills have the same semantic content as the previous release, LPM.dev Registry returns a review reminder. Line endings and trailing whitespace do not count as content changes. Text output displays the warning. JSON output includes it in the affected registry result's `warnings` array.

## Package lifecycle scripts

Publishing can run six scripts from `package.json`, in this order:
`prepublishOnly`, `prepack`, `prepare`, `postpack`, `publish`, and `postpublish`.
The first three run before packing. `postpack` runs after packing.
The last two run after a successful publish or dry run.

In non-interactive mode, use `--yes` to run these scripts or `--ignore-scripts` to skip them:

```bash
lpm publish --npm --dry-run --yes
lpm publish --npm --ignore-scripts
```

Each script runs in the sandbox with a five-minute timeout.
Windows lifecycle scripts require a project on a local drive.
Protected parent directories and system-installed tools may need [administrator sandbox setup](/docs/infra/doctor#windows-sandbox-permission-setup).
Preview the permissions with `lpm doctor sandbox-setup` before applying them.
After setup, publish from your normal terminal.
Approved tool directories avoid repeated permission walks during hooks.
...
`--dry-run` runs local preparation and checks the selected registry before an upload.
For LPM.dev Registry, it resolves authentication, checks authored-skills staleness, and validates publish permissions, account limits, and version availability.
For npm, GitHub Packages, and GitLab Packages, it resolves registry-specific credentials and checks version availability remotely.
These checks do not upload or reserve a version.
Any failure exits non-zero. JSON output reports the failure.
A supplied `--provenance-file` is validated locally.

Both preview modes leave LPM CLI's manifest edits in memory. Lifecycle scripts can change project files during a dry run.
When authored skills exist and `package.json > files` excludes `.lpm/skills`, preparation adds that entry to the in-memory manifest.
A real publish can persist the entry.
...
| `--publish-registry <URL>` | Any npm-compatible registry | HTTPS, or HTTP for a loopback development registry |
...
Custom publish registries require HTTPS, except for HTTP loopback development registries such as `http://127.0.0.1:4873`. LPM CLI refuses other HTTP publish destinations before upload.
...
Custom publishing reuses the saved login whether the registry URL ends with a slash or not. Different registry paths retain separate credentials. Registry URLs cannot contain a username or password.

Publish redirects can replay the package body only within the original registry directory and origin. A body-preserving redirect outside that directory is refused, even on the same host. A `303` redirect drops the body and can continue without credentials outside that directory. See [redirect transport policy](/docs/registries#redirect-transport-policy).
...
| Flag | Effect |
| --- | --- |
| `--dry-run` | Prepare the package, run consented hooks, and check registry authentication and version availability without uploading |
...
| `--publish-registry <URL>` | Custom npm-compatible registry (HTTPS, or HTTP loopback for development) |
