...
2. **Pack** — assemble an in-memory tarball from the package manifest. When `files` is omitted, project-local `.lpm` state is excluded except for validated publisher-authored `.lpm/skills/*.md` files. Declared `bundledDependencies` (and the legacy `bundleDependencies` alias) are copied from verified hoisted or isolated install layouts, including required transitives. Bundled bytes are part of the same size, secret-scan, digest, and provenance boundaries. LPM CLI rejects compressed tarballs over **500 MiB** during local preparation. LPM.dev Registry currently enforces a smaller **100 MiB** upload cap; other targets can impose their own limits.
...
5. **Secret scan** — scan the final packed artifact for every target, after name and workspace-protocol rewrites. The scan therefore covers exactly what would upload, including bundled dependency bytes, rather than a Git-filtered view of the project tree. Skip with `--allow-secrets` only when you have reviewed every final artifact carefully.
...
Both preview modes leave the working tree untouched. For a real publish, when authored skills exist and `package.json > files` does not include `.lpm/skills`, LPM CLI adds that entry before packing.
