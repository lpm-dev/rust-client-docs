...
`--query-verbose` adds per-match tag detail — the full set of tags carried by each matched package, not just the ones in the selector.
...
| `--query-verbose` | Per-match tag details |
