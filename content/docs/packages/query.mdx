...
The `:eval` tag also covers aliases and supported Node `vm` evaluation or compilation APIs.
...
| Tag | Expands to |
| --- | --- |
| `:critical` | `:obfuscated` OR `:protestware` OR `:credential-exfiltration` OR `:destructive-filesystem` |
| `:high` | `:eval` OR `:child-process` OR `:shell` OR `:dynamic-require` OR `:encrypted-execution` OR `:downloaded-execution` OR `:scripts` OR `:vulnerable` |
