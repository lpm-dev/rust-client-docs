...
Drops one or more packages from `package.json`, removes them from `node_modules`, and rewrites the local `lpm.lock` / `lpm.lockb` pair to the post-uninstall graph.
