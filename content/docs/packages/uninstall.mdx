...
Drops one or more packages from `package.json`, removes them from `node_modules`, and rewrites the local `lpm.lock` / `lpm.lockb` pair to the post-uninstall graph. When a removed package makes transitive packages unreachable, LPM prunes those orphaned entries from both the lockfile and `node_modules`, then cleans up empty scoped-package directories such as `node_modules/@scope/`.
