...
lpm uninstall -g --preserve-trust eslint # approved-script reinstall only
...
Ordinary global uninstall removes exact lifecycle approvals that are unique to the removed install, while preserving approvals still reachable through another global install with the same source and integrity. After [`lpm approve-scripts --global`](/docs/packages/approve-scripts#global-vs-project), use the emitted `--preserve-trust` uninstall command so the immediate reinstall can consume the new approval. The flag requires `--global`; don't use it for permanent removal because it intentionally leaves those trust rows in place.
...
| `--preserve-trust` | With `--global`, keep exact lifecycle approvals for an immediate post-approval reinstall |
