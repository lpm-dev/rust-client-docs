...
Fetches an `@lpm.dev/*` package tarball and extracts it into a directory you choose. **No lockfile**, **no `node_modules`**, **no script execution**, **no `package.json` mutation**. Just the bytes.
...
**Scope: `@lpm.dev/*` packages only.** Bare names like `zod` or `react` are rejected at parse time. For npm tarballs, use `npm pack <pkg>` then `tar -xf`.

## Examples

```bash
lpm download @lpm.dev/owner.pkg                      # latest, extracted into the current directory
lpm download @lpm.dev/owner.pkg --version 2.1.0      # a specific version
lpm download owner.pkg --output /tmp/inspect         # short form + `-o` alias
lpm download @lpm.dev/owner.pkg --json               # structured output (URL, integrity, paths)
```

## How it works

1. Fetches the package metadata from lpm.dev to find the tarball URL and SRI integrity hash for the requested (or latest) version.
2. Downloads the tarball.
3. Verifies the SRI hash when the registry ships one; emits a `No integrity hash available — skipping verification` warning and continues when it doesn't.
