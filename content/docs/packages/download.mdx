...
When npm firewall mode is `report` or `enforce`, `lpm download` checks routed public npm packages with firewall.lpm.dev after selecting the version and before downloading the tarball. `enforce` blocks packages marked malicious before any package bytes are extracted. In human output, the download line shows `🔥 LPM Firewall active` when that check is active. lpm.dev packages and other `.npmrc`-declared private/custom registry packages are not sent to the npm firewall verdict API.
