...
When npm firewall mode is `report` or `enforce`, `lpm download` checks routed public npm packages with lpm.dev after selecting the version and before downloading the tarball. `enforce` blocks packages marked malicious before any package bytes are extracted. lpm.dev packages and other `.npmrc`-declared private/custom registry packages are not sent to the npm firewall verdict API.
