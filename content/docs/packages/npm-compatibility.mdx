...
### `node_modules` layout: hoisted by default, isolated for workspaces

LPM matches npm's flat `node_modules/` layout for single-package projects. **Workspaces auto-flip to isolated** (pnpm-style symlinks into the global virtual store) — phantom-dep bugs are most expensive in monorepos, so the safer layout becomes the default there. Force a specific mode:
...
Or persistent in `package.json > lpm > linker` / `~/.lpm/config.toml > linker`.
