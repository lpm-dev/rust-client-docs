...
| Lifecycle script names | `preinstall`, `install`, `postinstall` execute (see [Differences](#differences)). `prepare`, `prepublishOnly`, `preuninstall`, `uninstall`, `postuninstall` are recognized for detection — declaring them puts the package in [`lpm audit`](/docs/packages/audit) and the [`lpm approve-scripts`](/docs/packages/approve-scripts) review queue, but the install pipeline never runs them. |
