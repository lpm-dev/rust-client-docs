...
description: "What works the same as npm, what's different, and what LPM CLI adds on top."
---
...
npm reads `engines.node` / `engines.npm` and only enforces them when `engine-strict=true` is set in `.npmrc`. LPM CLI enforces by default — install / rebuild / add abort when the workspace root's `engines.lpm` constraint doesn't match the running CLI version, or `engines.node` doesn't match the effective Node runtime.
