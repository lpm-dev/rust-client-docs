...
LPM blocks direct/root dependency versions published less than 24h ago by default. npm doesn't. Override with `lpm install --allow-new`, or opt into strict transitive cooldown with `minimumReleaseAgePolicy: "strict"`. See [Recently published packages](/docs/packages/install#recently-published-packages).
...
| npm trust no-downgrade policy | [`lpm config trust-policy`](/docs/infra/config#setup-wizards) |
