...
When npm firewall mode is `report` or `enforce`, `lpm fetch` batches eligible public npm package-version rows from the lockfile with firewall.lpm.dev before starting tarball downloads. That includes current public npm registry rows, legacy source-less rows whose tarball URL is a canonical `registry.npmjs.org` package tarball for the same name/version, and canonical public npm `tarball+https:` rows for the same name/version. The hosted verdict service is an LPM Pro/org feature, so active modes send LPM auth; run `lpm login` before enabling it. `report` warns and continues when entitlement is denied, while `enforce` blocks packages marked malicious or denied by entitlement before the store is populated. In human output, the lockfile-fetch line shows `🔥 LPM Firewall active` when that batch check is active. Private/custom registry sources, arbitrary remote tarball URL sources, local sources, and lpm.dev packages are not sent to the npm firewall verdict API.
