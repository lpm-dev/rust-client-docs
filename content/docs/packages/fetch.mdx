...
When npm firewall mode is `report` or `enforce`, `lpm fetch` batches eligible public npm package-version rows from the lockfile before starting tarball downloads. `enforce` blocks packages marked malicious before the store is populated. Private/custom registry sources, remote tarball URL sources, local sources, and lpm.dev packages are not sent to the npm firewall verdict API.
