...
The cache key folds in `(name, version, source, integrity)`, every amber phase and referenced file shown to the advisor, the repository hint, prompt template hash, provider slug, and model version. A registry verdict therefore cannot cross to a file or workspace source, and an integrity change cannot reuse the old verdict. Any key input changing causes the next invocation to re-classify. Cached verdicts expire after 30 days by default. Hits return the cached `Approve` / `Manual` / `Abstain` value verbatim; the cache does not promote anything.
