...
- **Source- and script-aware identity.** Approvals key on `(name, version, source, integrity, script_bundle_hash)`. Equal-coordinate registry, workspace, file, and link packages remain independent even when a local source has no integrity, and a verdict cannot cross to different lifecycle-script bytes.
