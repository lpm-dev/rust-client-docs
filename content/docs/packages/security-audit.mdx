...
The npm firewall is an opt-in firewall.lpm.dev verdict check for selected public npm package versions. It does not proxy npm metadata or tarballs: installs still fetch from the normal npm registry path, then batch selected `(name, version)` checks to firewall.lpm.dev before tarballs are allowed through.

The hosted verdict service is an LPM Pro/org feature. Active `monitor` and `enforce` modes send LPM auth, so run `lpm login` first. Entitlement failures are warnings in `monitor` mode and blocking failures in `enforce` mode.
...
The firewall also carries policy metadata for configurable enforcement. Recommended enforcement blocks trusted public malicious advisories, LPM AI-confirmed malware, and LPM AI-agent control-surface policy blocks; it warns for critical vulnerabilities and static-only suspicious signals. See the [Firewall for npm guide](/docs/guides/firewall) for the full policy profile.
