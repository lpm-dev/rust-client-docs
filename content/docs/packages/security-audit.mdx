...
Project approvals are **strict**: the key combines `name`, `version`, and an opaque identity token derived from the exact `source` and `integrity`; the binding also records `source`, `integrity`, and `scriptHash`. Two equal-coordinate packages from different sources remain independently reviewable, even when their bytes share an integrity value. A source, content, or script change invalidates the approval and re-opens that identity for review. Global approvals remain keyed by `name@version` and bind integrity, script hash, and approval-time provenance.
...
- **Source- and script-aware identity.** Approvals key on `(name, version, source, integrity, script_bundle_hash)`. The script hash covers every install lifecycle phase and its reachable in-package delegated file graph. If that graph is missing, escapes the package, or exceeds the safety limits, LPM CLI withholds a reusable hash and keeps the package blocked. Equal-coordinate registry, workspace, file, and link packages remain independent even when a local source has no integrity, and a verdict cannot cross to changed executable bytes.
