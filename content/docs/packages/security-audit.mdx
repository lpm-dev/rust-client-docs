...
- **Approve them explicitly** via [`lpm approve-scripts`](/docs/packages/approve-scripts) — project and global trust record the exact source/content identity and script hash; global bindings also retain approval-time provenance.
...
Approvals are **strict**: project and global keys combine `name`, `version`, and an opaque identity token derived from the exact `source`, `integrity`, and `scriptHash`; each binding also records those fields. The script hash covers the executed lifecycle phase bodies and the package-owned content tree, including the raw `package.json` bytes. Two equal-coordinate packages from different sources—or two lifecycle-content revisions sharing source and integrity—remain independently reviewable. A source, content, or script change invalidates the approval and re-opens that identity for review. Bindings without a reusable script hash never grant durable trust. Source-less coordinate-only legacy entries cannot authorize source-qualified packages and remain migration input.

Under store v2, lifecycle review uses an immutable post-patch, pre-build baseline. Build-generated output does not alter that durable identity. Before execution, LPM restores the exact authorized baseline and recalculates its content hash; missing, changed, malformed, or over-budget baselines fail closed before a process is spawned. Whole-tree and delegate hashing are bounded by entry-count, total-byte, per-delegate-file, and depth limits so hostile package trees cannot force unbounded trust/advisor I/O.
...
A drift event blocks the install. Re-approve via [`lpm approve-scripts`](/docs/packages/approve-scripts) (or `lpm approve-scripts --global`) to capture the new identity, or opt out per-package with `--ignore-provenance-drift <pkg>` / blanket-waive with `--ignore-provenance-drift-all`. For project trust, the reference snapshot lives under the exact `package.json > lpm > trustedDependencies > <pkg>@<ver>#<identity-token>` binding; global trust stores it under the equivalent exact key in `~/.lpm/global/trusted-dependencies.json`. Registry provenance requests follow the installed package's resolved registry route. File, link, git, and direct URL sources are not substituted with same-name public-registry provenance.
...
- **Source- and content-aware identity.** Approvals key on `(name, version, source, integrity, script_bundle_hash)`. The script hash covers every executed install lifecycle phase plus the bounded package-owned file tree, including raw `package.json` bytes and package-owned `.lpm-build-tmp` or `.lpm-build-complete` entries. Dependency directories and LPM CLI's internal store sidecars are excluded. Statically recognized delegate graphs are checked as an additional fail-closed guard. If package content is unreadable or the tree/delegate graph exceeds a safety limit, LPM CLI withholds a reusable hash and keeps the package blocked. Equal-coordinate registry, workspace, file, and link packages remain independent even when a local source has no integrity, and a verdict cannot cross to changed package executable bytes.
