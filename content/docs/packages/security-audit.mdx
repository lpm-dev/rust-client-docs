...
- **Ephemeral.** Approvals never write to disk. A second `lpm install` invokes the advisor again — the verdict is non-deterministic and LPM does not pretend otherwise. Strict bindings via [`lpm approve-scripts`](/docs/packages/approve-scripts) exist for the "approve once, remember forever" use case.
