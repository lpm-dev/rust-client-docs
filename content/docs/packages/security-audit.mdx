...
The npm firewall is an opt-in lpm.dev verdict check for selected public npm package versions. It does not proxy npm metadata or tarballs: installs still fetch from the normal npm registry path, then batch selected `(name, version)` checks to lpm.dev before tarballs are allowed through.
