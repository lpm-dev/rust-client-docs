...
| **Behavioral analysis**   | Risky API usage, obfuscation, telemetry, license issues | At extract time by default, with schema-versioned local caching |
...
The `lpm-security` crate's `behavioral` module performs static analysis while package bytes are extracted into the store by default. Results are cached in a schema-versioned `.lpm-security.json` next to the package files. Cache identity includes package source and integrity, so the same `name@version` from different registries does not share findings.

Use `lpm config source-analysis` to manage install-time analysis. Disabling it is an approval-gated security weakening; it skips new install-time scans and cache writes without deleting existing caches. [`lpm audit`](/docs/packages/audit) still scans installed source when it cannot use a cache. After analysis is re-enabled, the next install backfills any missing, malformed, or outdated store cache from the already-extracted bytes without downloading the tarball again.
...
Known vulnerabilities and LPM.dev Registry security findings use their published severity. A Critical advisory or registry finding remains Critical in `lpm audit` and audit-after-install.
...
**Minimum release age** — LPM CLI avoids new dependency versions for 24 hours by default. Project installs check direct dependencies by default. Strict mode also checks transitive dependencies and lockfile replays.
