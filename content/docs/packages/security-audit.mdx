...
Approvals are **strict**: project and global keys combine `name`, `version`, and an opaque identity token derived from the exact `source` and `integrity`; each binding also records `source`, `integrity`, and `scriptHash`. The script hash covers the executed lifecycle phase bodies and the package-owned content tree. Two equal-coordinate packages from different sources remain independently reviewable, even when their bytes share an integrity value. A source, content, or script change invalidates the approval and re-opens that identity for review. Bindings without a reusable script hash never grant durable trust. Coordinate-only legacy entries remain migration input rather than executable trust.
...
- **Source- and content-aware identity.** Approvals key on `(name, version, source, integrity, script_bundle_hash)`. The script hash covers every executed install lifecycle phase plus the bounded package-owned file tree; dependency directories and LPM-generated store/build markers are excluded. Statically recognized delegate graphs are checked as an additional fail-closed guard. If package content is unreadable or the tree/delegate graph exceeds a safety limit, LPM withholds a reusable hash and keeps the package blocked. Equal-coordinate registry, workspace, file, and link packages remain independent even when a local source has no integrity, and a verdict cannot cross to changed package executable bytes.
