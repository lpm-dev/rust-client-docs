...
The hosted verdict service is an LPM Pro/org feature. Active `report` and `enforce` modes send LPM auth, so run `lpm login` first. Entitlement failures are warnings in `report` mode and blocking failures in `enforce` mode.

Configure it globally:

```bash
lpm config firewall --set report   # warn and continue
lpm config firewall --set enforce  # block malicious verdicts
```

`off` is the default. `report` is useful while evaluating coverage; `enforce` is the blocking mode. Once report or enforce mode is part of the approved machine posture, disabling or downshifting it is a guarded weakening through [`lpm security`](/docs/infra/security).
