...
| High | `:eval`, `:child-process`, `:shell`, `:dynamic-require`, `:encrypted-execution`, `:downloaded-execution` |
...
The local source scanner assigns Critical to high-confidence obfuscation, protestware, credential exfiltration, and broad filesystem deletion. Obfuscation uses these confidence bands:
...
Behavioral tags report code patterns and package capabilities. They do not prove user intent or malicious execution.

The `:credential-exfiltration` finding traces supported secret flows into upload calls. See [detection coverage](/docs/packages/audit#credential-exfiltration) for its limits.
...
Source analysis covers JavaScript and TypeScript without executing package code. An API capability does not prove malicious intent or execution.

Full file analysis stops at 4 MiB. Larger files receive samples. Each package has a 50 MiB scan budget and a 5,000-file limit.

Missing files, unreadable inputs, unsupported syntax, and exceeded limits prevent a clean audit. See [source coverage](/docs/packages/audit#source-coverage) for recovery steps and JSON fields.

Local analyses can be cached. Scanner rule changes invalidate older cached results.
...
lpm audit --level high          # filter findings and the failure decision
lpm audit --fail-on vuln        # CI: fail on confirmed vulns only (not behavior)
```

The vulnerability check requires network access. OSV requests run on each invocation. Local source analysis can use cached results.

OSV receives names and versions outside `@lpm.dev/*`, including private-registry packages. See [inventory and network requests](/docs/packages/audit#inventory-and-network-requests).
