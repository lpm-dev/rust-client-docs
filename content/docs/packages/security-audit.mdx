...
- run: lpm trust diff --json | jq -e '(.added | length) + (.removed | length) + (.changed | length) == 0'
...
`lpm audit --fail-on` covers the broad-strokes gate; `lpm query --assert-none` covers your project's specific concerns. `lpm trust diff` itself is informational — pipe through `jq -e` to make it gate on `trustedDependencies` drift. Together they're cheap and they catch most real classes of supply-chain trouble.
