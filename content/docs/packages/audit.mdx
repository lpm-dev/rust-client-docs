...
2. **Source capabilities and security findings** — source analysis lists API capabilities separately from suspicious patterns, such as obfuscation and protestware. Capabilities include runtime evaluation, child processes, network access, filesystem access, and native code. See the full tag list under [`lpm query`](/docs/packages/query).
...
- Full analysis stops at 2 MiB for one file. Larger files receive a sample from each end.
