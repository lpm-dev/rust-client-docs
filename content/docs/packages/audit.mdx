...
The Critical `:credential-exfiltration` finding identifies supported flows of private-key material or the complete process environment into network payloads.
It checks `fetch` request headers and bodies with fixed HTTP URLs, plus supported repository-content uploads.
