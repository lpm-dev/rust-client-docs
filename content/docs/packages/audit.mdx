...
2. **Source capabilities and security findings** — source analysis lists API capabilities separately from suspicious patterns, such as obfuscation, protestware, and credential exfiltration. Capabilities include runtime evaluation, child processes, network access, filesystem access, and native code. See the full tag list under [`lpm query`](/docs/packages/query).
...
`lpm audit` exits non-zero when findings meet the active `--fail-on` policy.
High-severity capabilities, such as `eval`, fail CI only with explicit `--fail-on=behavior` or `--fail-on=all`.

An incomplete vulnerability or source scan always exits non-zero, regardless of `--fail-on`. A network or advisory-data failure is not evidence that the installed tree is clean.

| Policy                      | Exit code 1 when                                                                                                                                                                     |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| (no flag, implicit default) | Vulnerabilities **or** critical behavioral flags (obfuscation, protestware, and credential exfiltration). Excludes `high` behaviors so existing pipelines that tolerate `eval` do not break. |
...
Critical source results include high-confidence obfuscation, protestware, and credential exfiltration. Scores above `0.3` through `0.7` are Info-level possible obfuscation. Scores above `0.7` are Critical.

Capability tags do not establish data flow or command intent. Environment access plus network access does not prove exfiltration. The separate credential-exfiltration finding requires a supported flow into an upload call. A detected `rm -rf` shell command remains a High shell and child-process signal.
