...
The root importer snapshot is what proves that the lockfile matches `package.json`. Legacy or imported lockfiles without that snapshot are not eligible for mutable or frozen warm replay: a mutable online install re-resolves and rewrites the graph, while a frozen install fails. This prevents removed dependencies or changed direct-peer roles from surviving in a stale graph. `--offline` deliberately retains compatibility replay for snapshotless lockfiles because offline mode cannot re-resolve them. Run one mutable online install when possible to write a current importer snapshot.
