---
title: Lockfile
description: "Why LPM CLI ships two lockfile files, when each is read, and how the warm-install fast path works."
---
...
- **SRI integrity hash** (`sha512-…`, `sha256-…`, or legacy `sha1-…`) — downloads are verified against the declared SRI, and explicitly declared algorithms remain exact. Warm/offline v1 reuse compares the stored `.integrity` marker with the lockfile; it does not rehash the extracted tree on every install.
...
The root importer snapshot is what proves that the lockfile matches `package.json`. Legacy or imported lockfiles without that snapshot are not eligible for mutable, frozen, or default offline replay: a mutable online install re-resolves and rewrites the graph, while frozen and offline installs fail. This prevents removed dependencies or changed direct-peer roles from surviving in a stale graph. If an online rebuild is impossible, `lpm install --offline --allow-snapshotless-lockfile` explicitly enables compatibility replay and warns that stale packages may remain. Run one mutable online install when possible to write a current importer snapshot.
...
The TOML lockfile is **schema-versioned** (current: `6`), not tool-versioned. A version bump only happens when the on-disk shape changes. Older clients reading a lockfile with a higher version refuse to install rather than silently misinterpreting fields — fail-fast over silent breakage. Version `5` added importer snapshots for frozen-lockfile installs and patch checksum records. Version `6` added persisted `engines.node` constraints so warm and frozen installs can revalidate dependency compatibility.
