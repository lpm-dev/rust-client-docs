...
The TOML lockfile is the canonical source — sorted, deterministic, git-friendly. The binary companion is intentionally smaller: it can only represent fields in its fixed-width v2 wire format. If the TOML lockfile contains aliases, peer pinning, platform metadata, optional-reachability state, catalogs, or registry signatures, LPM skips `lpm.lockb` and removes any stale binary file.
...
The TOML lockfile is **schema-versioned** (current: `4`), not tool-versioned. A version bump only happens when the on-disk shape changes. Older clients reading a lockfile with a higher version refuse to install rather than silently misinterpreting fields — fail-fast over silent breakage.
...
`lpm.lockb` is intentionally smaller than `lpm.lock`. It doesn't carry every field — npm-alias metadata, ambient peer installs, per-package peers, platform metadata, optional reachability, catalogs, and registry signatures live only in TOML. Projects with any of those fields write **only `lpm.lock`**.
