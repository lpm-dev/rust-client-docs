...
No metadata fetches. No tarball downloads. Just symlinks (isolated layout) or hardlinks (hoisted layout) into already-extracted store entries. The wrapper tree lives at `.lpm/wrappers/` (a project-root sibling, not under `node_modules/`), so `rm -rf node_modules` between iterations doesn't destroy the wrapper tree — warm install becomes a symlink rebuild from already-materialized wrappers.
