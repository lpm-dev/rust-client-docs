...
Automatic setup during `lpm install` remains best-effort for authentication. If session refresh or SPM login fails, the install flow can continue to the Registry request and report its result.
