...
Fetches the lpm.dev quality report for an `@lpm.dev/*` package and prints it: a score out of 100, a tier label, and a per-check breakdown grouped by category (readme, license, types, tests, maintenance, etc.).

**Scope: `@lpm.dev/*` packages only.** The quality endpoint lives on lpm.dev; npm-registry packages aren't covered. For broad quality-shaped signals on installed npm packages, [`lpm audit`](/docs/packages/audit) covers vulnerabilities + behavioral analysis.
