...
[`lpm upgrade`](/docs/packages/upgrade) applies the report for `@lpm.dev/*` packages and for npm packages whose existing `lpm.lock` entry records a public npm source. That keeps the common flow as:
...
Packages without a recorded npm-public source are still skipped instead of being queried against `registry.npmjs.org`. Run `lpm install` first so the source is captured in `lpm.lock`, then rerun `lpm outdated` or `lpm upgrade`.
...
When `lpm outdated` skips non-`@lpm.dev/*` packages because they do not have a recorded npm-public source in `lpm.lock`, the envelope also includes `skipped_private` and `skipped_private_reason`.
