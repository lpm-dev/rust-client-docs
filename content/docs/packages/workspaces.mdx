...
> **The legacy substring matcher is gone.** Phase 32 removed it per design decision D2. `--filter core` does NOT match `@babel/core` anymore — write `--filter '*/core'` for that. This is the most common surprise after a Phase 32 upgrade.
...
Tells the resolver "use the local workspace member, not a published version." The linker symlinks the member into `node_modules/` (isolated layout) instead of pulling from the registry. Topological order is respected for tasks (`dependsOn: ["^build"]` waits for upstream member's build to finish first).
...
- Workspace members referenced via `workspace:*` must be **published to the registry** today. The resolver has no local-package handling for the deploy output's tree. Phase 12+ will add unpublished workspace dep injection.
