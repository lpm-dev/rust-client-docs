...
LPM CLI installs members in dependency order and installs the root last. Workspace discovery is shared across the operation instead of repeated in one process per member. Each target still gets its own lockfiles, `node_modules`, lifecycle, engine checks, and security-policy evaluation. A member failure stops later targets.
...
Workspace members can also satisfy compatible plain semver edges that appear transitively through registry packages. If a registry package depends on `@my-co/shared` and the current workspace has a matching local member, LPM CLI resolves that edge to the local member instead of leaking the lookup to the registry.

Self-dependencies are invalid: a workspace member that declares itself via `workspace:*` in `dependencies`, `devDependencies`, `peerDependencies`, or `optionalDependencies` fails before LPM CLI writes lockfiles or `node_modules`.
