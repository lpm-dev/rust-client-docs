...
Release and version operations scan workspace members before planning. These scans allow at most 10,000 patterns, 100,000 directory entries, and a 16 MiB path budget. Use specific member directories instead of a broad recursive pattern.
...
### Refreshing shared versions

After you change an override or catalog in the root `package.json`, run [lpm install](/docs/packages/install) from the workspace root:

```bash
lpm install
```

LPM CLI refreshes affected members even when their own manifests do not change. Member-specific overrides still take precedence over matching root overrides.

Commit the updated root manifest and `lpm.lock` together. Use a frozen install in CI to reject configuration changes that the lockfile does not record:

```bash
lpm install --frozen-lockfile
```

Catalog references can coexist with local `file:` dependencies. Repeated installs preserve each member's dependency versions and compatible peers supplied by the root. An unchanged frozen install preserves the lockfile.
