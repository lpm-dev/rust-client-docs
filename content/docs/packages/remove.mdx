...
Cleans up the on-disk artifacts of an [`lpm add`](/docs/packages/add) invocation by reading the project-local `.lpm/added-sources.json` manifest that `lpm add` writes. Exact tracked files are removed from whatever path they were copied to, then LPM cleans up any package skills it installed.

This is the **reverse of [`lpm add`](/docs/packages/add)** for source-delivered packages on lpm.dev, npm, or a private registry routed via `.npmrc`. To remove a runtime dependency from `package.json` and `node_modules`, use [`lpm uninstall`](/docs/packages/uninstall).

## Scope

`lpm remove` accepts the same package identity you passed to [`lpm add`](/docs/packages/add): bare npm names (`lodash.merge`), scoped registry packages (`@scope/private-pkg`), and lpm.dev packages (`@lpm.dev/owner.ui-kit`). For legacy lpm.dev cleanup, the short remove-only form (`owner.ui-kit`) still works too.
...
3. Removes `.lpm/skills/<owner>.<name>/` if the add installed LPM agent skills, plus any editor integrations LPM set up alongside (for example Cursor symlinks).
