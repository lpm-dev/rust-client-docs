...
`lpm licenses` lists the licenses declared by installed packages in the current `lpm.lock` graph. It reads the same installed package manifests as [lpm sbom](/docs/packages/sbom), including aliases and separate dependency instances. Missing or invalid required manifests stop the command.
...
LPM CLI trims license text and removes empty or duplicate declarations. Multiple declarations join with `AND`, with parentheses that preserve compound expressions.
...
`scope` is derived from root reachability in `lpm.lock`, including aliases and peer relationships. Required paths win over optional paths, and optional paths win over excluded/dev-only paths, so a dev-only transitive package is marked `excluded` unless a production or optional root also reaches it.
