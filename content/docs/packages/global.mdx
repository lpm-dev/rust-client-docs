...
| Flag | Effect |
| --- | --- |
| `--outdated` | Resolve each install's saved spec against its registry and active cooldown policy; flag packages with newer installable matching versions. `@lpm.dev/*` globals use LPM batch metadata, while npm globals use the same npm metadata path as global install/update. |
...
Global update uses the global minimum release age chain: `~/.lpm/config.toml`, then the 24h default. Versions still inside the cooldown window are skipped during planning, including `--dry-run`, so the plan only shows versions the nested install would accept.
...
- **No project-config tier.** Globals use a synthesized package.json that doesn't carry `lpm > scriptPolicy`, `lpm > minimumReleaseAge`, `lpm > minimumReleaseAgeExclude`, or `lpm > scripts.autoBuild`. The script-policy and cooldown precedence chains collapse to CLI flag > `~/.lpm/config.toml` > default. Triage on `-g` requires the explicit `--auto-build` flag for mixed-trust trees — there's no per-project knob.
