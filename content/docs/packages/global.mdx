...
- **Reinstall to rerun blocked scripts.** After `lpm approve-scripts --global`, the trust file is updated but the install root isn't re-executed. Run the emitted `lpm uninstall -g --preserve-trust <pkg> && lpm install -g <pkg>` command for each affected top-level global. `lpm rebuild --global` is a planned follow-up.
...
Approvals write to `~/.lpm/global/trusted-dependencies.json` (separate from any project's `package.json`). The success banner enumerates the affected top-level globals and prints the matching `lpm uninstall -g --preserve-trust … && lpm install -g …` command for each.
