...
When a global install would expose a bin name that another global package already owns, LPM refuses to silently take it over. In an interactive terminal, LPM prompts per colliding command: replace the existing shim, install the new command under an alias, or cancel. In JSON and non-TTY runs, it exits with a collision error and shows copy-pasteable `--replace-bin` / `--alias` examples.
