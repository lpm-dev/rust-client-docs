...
On Windows, global installs create command shims for Command Prompt, PowerShell, and Git Bash.
These shims forward arguments and the exit code to the installed package command.
Updates and aliases use the same command forwarding.
...
An alias also works when no command name collides. Uninstalling the package removes its alias command.
