...
With `--outdated`, the human table shows `Package`, `Current`, `Wanted`, `Latest`, and `Bins`. `Wanted` is the newest version that still satisfies the global install's saved spec; `Latest` is the registry's absolute latest version, so major-version jumps are visible even when the saved range would not select them.
