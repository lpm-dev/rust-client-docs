...
**Default scope: `@lpm.dev/*` plus public npm packages already recorded as public npm in `lpm.lock`.** That means a normal `react`, `lodash`, or `@scope/pkg` install that currently resolves through `registry.npmjs.org` is eligible for `lpm upgrade` too. Packages without a recorded public-npm source are skipped instead of being queried against registry.npmjs.org — this avoids leaking private/custom-registry package names. If you added a public npm package before LPM recorded source metadata, run [`lpm install`](/docs/packages/install) once to refresh `lpm.lock`, then rerun `lpm upgrade`.
...
If your `package.json` says `"react": "^18.0.0"` and the lockfile records `react` as coming from the public npm registry, `lpm upgrade` will land `^18.3.1` when 18.3.1 is the newest matching 18.x release. With `--major` and react 19 available, it would also offer `^19.0.0`.
