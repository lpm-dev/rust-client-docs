...
`lpm upgrade` honors the same [minimum release age](/docs/packages/install#minimum-release-age) policy as install. If the registry's latest version is still inside the cooldown window, upgrade selects the newest mature candidate instead of rewriting `package.json` to a version the install step would reject.
