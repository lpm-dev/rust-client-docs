...
3. **Write** — emit `lpm.lock` + `lpm.lockb`. Back up the source lockfile and `.npmrc`.
4. **`.npmrc`** — optionally configure `.npmrc` for LPM-aware tooling. Skip with `--no-npmrc`.
5. **Install** — run `lpm install` against the new lockfile (lockfile fast-path, no re-resolution). Skip with `--no-install`.
6. **Verify** — run `build` + `test` scripts to confirm nothing broke. Skip with `--skip-verify`.
7. **CI template** — print a hint pointing at a CI workflow template, or generate one with `--ci`. Suppress with `--no-ci`.
8. **Summary** — print what was done, where backups live, and how to roll back.
