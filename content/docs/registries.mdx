...
The `@lpm.dev/*` scope is the **only** scope that always goes to lpm.dev. JSR packages use JSR's npm-compatible registry. Everything else is npm by default.

## JSR packages

LPM accepts `jsr:` dependency specs in `package.json` and resolves them through JSR's npm-compatible registry. The JSR package name stays as the local import name, while the metadata fetch uses JSR's folded npm package name under `@jsr`.
