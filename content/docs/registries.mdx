...
LPM CLI merges global, user (`~/.npmrc`), then project or workspace-root `.npmrc` configuration. Later assignments take precedence. Keep committed project `.npmrc` files to literal routing data, and put env-backed secrets in user or CI-owned config.
...
### Keep credentials outside registry URLs

LPM CLI rejects registry URLs that contain a username or password. The error appears before registry requests, manifest changes, or lockfile writes. Use a registry-scoped token as shown above, or scoped Basic authentication:

```ini title="~/.npmrc"
//npm.my-company.com/team/:username=${REGISTRY_USER}
//npm.my-company.com/team/:_password=${REGISTRY_PASSWORD_BASE64}
```

`REGISTRY_PASSWORD_BASE64` contains the Base64-encoded password. Base64 is encoding, not encryption. Keep the corresponding route, `registry=https://npm.my-company.com/team/`, in the project configuration.

On Unix, protect credential files with `chmod 600 ~/.npmrc`. Files readable by other users cannot supply authentication fields. See [authentication permissions](/docs/infra/authentication).

If an older CLI wrote a password into `lpm.lock`, move the credential to scoped configuration. Run `lpm install --force` to regenerate the lockfile, then run `lpm install --frozen-lockfile`. The forced install also resolves versions again and downloads packages. If you committed or shared the old password, rotate it at the registry.
...
Registry credentials apply only to their configured host, port, and path. Read redirects outside that scope continue without the credential. A later redirect back does not restore it. Registry redirects with a username or password in the destination URL are refused.

For publishing, a redirect cannot replay the package body outside the original registry directory, even on the same host. A body-dropping `303` redirect can continue as an anonymous GET outside that directory. Redirects within the registry directory retain the applicable credentials and body.

Registry read errors remove known request credentials from reflected response text in terminal and JSON output. This includes Bearer tokens, Basic authentication values, and decoded Basic passwords.
...
When a publisher changes a private package to public distribution, only the latest eligible release and future releases enter review. Earlier private history stays hidden. See [LPM.dev package distribution](/docs/packages/package-distribution).
