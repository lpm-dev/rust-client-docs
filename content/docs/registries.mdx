...
Disables hostname, expiry, and CA validation **process-wide**. LPM CLI logs a loud warning at install start whenever `strict-ssl=false` is in effect — this is the kind of setting that should never accidentally land in CI.
