...
To generate `.npmrc` for an npm-compatible client in CI without relying on `lpm login` in the runner:

```bash
lpm setup ci npmrc # generates an .npmrc for CI/CD use
```

For LPM CLI local development, create a read-scoped token at [lpm.dev/dashboard/settings/tokens](https://lpm.dev/dashboard/settings/tokens) and expose it as `LPM_TOKEN`; the built-in `@lpm.dev/*` route needs no `.npmrc`. If another npm-compatible client also needs access, place its env-backed auth in user-level `~/.npmrc`. See [`lpm setup`](/docs/infra/setup#local-authentication).
