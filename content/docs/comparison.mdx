...
| [Minimum release age (cooldown)](/docs/packages/install#recently-published-packages)                                 | `min-release-age` flag, off by default | 1-day cooldown by default                                   | Off by default                                                                 | 1-day cooldown by default                                                                                                                                                            |
...
| [Approval-gated weakening of security posture](/docs/reference/security)                                             | —                                      | —                                                           | —                                                                              | Weaker CLI flags can require native approval; pulled repo or config-file weakenings are treated as proposals until a short-lived unlock or persistent signed machine approval exists |
...
| [Dependency-graph visualization](/docs/infra/graph)                                           | `npm ls` (text)        | `pnpm list` (text)              | `bun pm ls` (text)                | `lpm graph` — HTML / JSON / dot output, depth filter, `--why <pkg>` reverse-lookup                                |
...
| [Auth-token storage](/docs/reference/authentication#token-storage) | `.npmrc` plain text                       | `.npmrc` plain text            | `.npmrc` / `bunfig.toml` plain text  | OS keychain (macOS Keychain / Linux Secret Service / Windows Credential Manager); `.npmrc` honored for tooling compat                                     |
...
| FreeBSD                       | Yes                  | Yes                             | —                        | —                                                                             |
| [Self-update](/docs/reference/self-update) | `npm install -g npm` | `pnpm self-update`              | `bun upgrade`            | `lpm self-update` — install-method-aware (npm / Homebrew / Cargo / standalone GitHub Releases); 24-hour banner check |
