...
| Provenance drift detection on republish                                                                             | —                                      | Resolve-time trust-rank downgrade refusal (trustedPublisher → provenance → none)                                                           | —                                                                              | Trust binding per package + drift detection when a publisher identity changes; optional `trust-policy = "no-downgrade"` refuses weaker npm trust evidence during resolution          |
