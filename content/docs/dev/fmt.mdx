...
A member inherits the workspace root pin unless its own `lpm.json` overrides it. Malformed configuration fails the affected member. Invalid root configuration stops the workspace run.

Run [`lpm plugin update biome`](/docs/dev/plugin) to pull the latest release into the cache.
...
Single-package mode (`lpm fmt`) preserves Biome's stdout — LPM CLI does not wrap it. Workspace mode emits a single LPM CLI envelope on stdout; per-member stdout/stderr is captured and surfaced inside the envelope only on failure. See [`lpm lint`](/docs/dev/lint#json-and-exit-codes) for the exact envelope shape.
