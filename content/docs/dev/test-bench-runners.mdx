...
`lpm test` and `lpm bench` accept `--all`, `--filter`, `--affected`, `--base`, `--fail-if-no-match`, and `--workspace-concurrency` — same grammar as [`lpm run`](/docs/dev/run) and the lint/fmt/check tools.

```bash
lpm test --all
lpm test --filter web --filter api
lpm test --filter './apps/*' --fail-if-no-match
lpm bench --affected --base develop --workspace-concurrency 2
```
