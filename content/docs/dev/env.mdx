...
preview value; that pre-existing value is never changed or deleted. The CLI
removes only a preview guard whose UUID or unique ownership value proves that
the current operation created it.
