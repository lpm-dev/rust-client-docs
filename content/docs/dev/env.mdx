...
The supported OIDC pull path is a **personal vault on a Pro account from GitHub Actions or GitLab CI**. No long-lived LPM.dev Registry token is stored in CI. Organization-vault OIDC policy creation is rejected until organization escrow and CI pull work end to end.
...
GitLab uses the stable numeric `project_id` claim and has no GitHub workflow/event/fork fields:
...
`--repo`, `--workflow`, `--events`, and `--allow-forks` are rejected for GitLab policies. The canonical subject is `project:12345`.
...
GitLab CI supplies `LPM_OIDC_TOKEN` with audience `https://lpm.dev`; LPM CLI binds it to the exact numeric project ID. Organization-vault OIDC pulls are not supported.
...
| `--oidc` | Pull a personal Pro vault through GitHub Actions or GitLab CI OIDC |
