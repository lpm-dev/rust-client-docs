...
When Coolify automatically copies a newly created production value into
preview, the CLI compares preview state before and after the create and deletes
only the newly created counterpart. It does not overwrite or remove a preview
value that existed before the push.
