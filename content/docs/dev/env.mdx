...
`lpm env rotate-key` pulls the authoritative personal cloud payload, validates every environment boundary, re-encrypts the complete payload with a fresh data key, and pushes it with the current remote version as a compare-and-swap. Named, empty, and remote-only environments plus alias, inheritance, and schema metadata are preserved. A compare-and-swap conflict or an error before upload leaves the previous remote payload unchanged. A timeout or disconnect after the request is sent is necessarily ambiguous: the Registry may have committed even though the CLI did not receive the response. In that case the CLI leaves its local version metadata unchanged; rerun `lpm env rotate-key` to pull the authoritative remote version and rotate again safely. JSON and human output report only an acknowledged new version and preserved environment names/count. Organization rotation is rejected; this command does not rotate the X25519 organization-sharing key.
...
The supported OIDC pull path is a **personal vault on a Pro account from GitHub Actions or GitLab.com CI**. Self-managed GitLab issuers are not supported. No long-lived LPM.dev Registry token is stored in CI. Organization-vault OIDC policy creation is rejected until organization escrow and CI pull work end to end.
...
`--repo`, `--workflow`, `--events`, and `--allow-forks` are rejected for GitLab.com policies. The canonical subject is `project:12345`. Exchanges require `ref_type: "branch"`; a tag whose name matches an allowed branch is still rejected.
...
GitLab.com CI supplies `LPM_OIDC_TOKEN` with audience `https://lpm.dev`; LPM CLI binds it to the exact numeric project ID. Self-managed GitLab issuers and organization-vault OIDC pulls are not supported.
...
| `--oidc` | Pull a personal Pro vault through GitHub Actions or GitLab.com CI OIDC |
