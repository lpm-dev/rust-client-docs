...
`lpm env` manages encrypted environment variables for the project. Local secrets live in an OS-keychain-backed vault; cloud sync persists them to the LPM.dev Registry and platform integrations push them to Vercel, Netlify, Railway, and other connected providers.
...
`lpm env pull` and `lpm env push` round-trip the local vault to the LPM.dev Registry's encrypted storage. The server only stores ciphertext — it never sees plaintext values.
