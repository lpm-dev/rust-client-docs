...
`lpm env check` loads `lpm.json > envSchema`, discovers the project's environments from its configuration and local vault, then validates each resolved environment. In normal output mode it exits non-zero when any environment is invalid.

```bash
lpm env check
lpm env check --json | jq -e '.success'
```

The explicit JSON test matters in automation: `lpm env check --json` reports `"success": false` for invalid environments but currently exits with status 0.
...
lpm env validate --json | jq -e '.valid'
```

Missing values do not currently make `lpm env validate` exit non-zero. Its JSON output keeps `"success": true` and sets `"valid": false`, so parse `valid` if you use it in automation. `--strict` reports local keys that are absent from `.env.example`, but those extras do not make the result invalid.
