...
`lpm env` manages encrypted environment variables for the project. Local values use OS-keychain-backed storage, cloud sync persists only ciphertext to lpm.dev, and the Vercel integration sends application env values directly from the CLI to Vercel.
...
Vercel is the only currently supported env platform. Application env values travel directly between the Rust CLI and Vercel; they never pass through lpm.dev. lpm.dev stores the Vercel credential encrypted at rest, non-secret connection configuration, authorization scope, and bounded operation counts.
...
The connect command prompts for the Vercel token and verifies it directly with Vercel before saving the encrypted credential to lpm.dev. Optional connection flags are `--team`, `--label`, `--linked-env`, and comma-separated `--target` values (`production`, `preview`, `development`). Omitting `--target` selects all three.

Normal pushes preserve values that exist only in Vercel. `--clean` removes those values in the selected targets. Vercel-managed variables are always excluded. Targetless values, duplicate relevant keys, and variables shared with an unselected target fail closed before mutation.
...
| `--to vercel` | Push directly from the CLI to Vercel |
| `--from vercel` | Pull directly from Vercel into the selected local env |
