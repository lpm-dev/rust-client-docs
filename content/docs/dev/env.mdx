...
Direct platform pushes record exact failed-operation counts only when acknowledged responses or authoritative final-state evidence prove them. Readable values can be reconciled by value, while GitHub secret creation or removal can be reconciled by name. An ambiguous existing-secret update remains unprovable because GitHub never returns the value; in that case, or when reconciliation itself fails, the CLI returns the provider error without sending a falsely exact `push_failed` audit.
