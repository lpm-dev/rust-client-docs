...
`lpm env` manages encrypted environment variables for the project. Local values use OS-keychain-backed storage, cloud sync persists only ciphertext to lpm.dev, and the Vercel, Coolify, and Railway integrations send application env values directly from the CLI to the selected platform.
...
| `--to <vercel\|coolify\|railway>` | Push directly from the CLI to the connected platform |
| `--from <vercel\|coolify\|railway>` | Pull directly from the platform into the selected local env |
