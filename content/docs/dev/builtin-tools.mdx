...
description: "How LPM ships Oxlint and Biome, plus how `lpm check` mixes local tsc parity with a managed tsgo engine."
---

LPM ships a small set of built-in tools — [Oxlint](https://oxc.rs) for [`lpm lint`](/docs/dev/lint), [Biome](https://biomejs.dev) for [`lpm fmt`](/docs/dev/fmt), and a type-check wrapper for [`lpm check`](/docs/dev/check). Oxlint and Biome are **plugin-backed**: lazy-downloaded on first use, cached, and pinned per-project. `lpm check` now straddles two models: default `tsc` remains project-local, while `--engine tsgo` uses a managed engine cache under `~/.lpm/engines`.
...
`lpm check` is **not** a plugin. It has two distinct execution paths:
...
- `tsgo` is a managed engine. LPM downloads the pinned platform tarball on first use, verifies its SRI integrity, preserves the full extracted layout, and reuses it from `~/.lpm/engines/tsgo/<version>/<platform>/` on later runs.

That split is intentional: `tsc` preserves editor and CI parity, while `tsgo` needs a full package layout (`lib/tsgo` plus the bundled `lib/*.d.ts` files) that does not fit the single-binary plugin model.
...
The engine sidecar records the engine identity, platform, entry path, tarball integrity, tarball SHA-256, and a hash of the extracted layout. Reuse re-hashes the installed layout before `tsgo` runs, so a stray binary without a sidecar or a tampered `lib/*.d.ts` file is treated as a cache miss instead of trusted state.
...
- [`lpm check`](/docs/dev/check) — NOT a plugin — uses project-local `tsc` by default and a managed `tsgo` engine when selected
