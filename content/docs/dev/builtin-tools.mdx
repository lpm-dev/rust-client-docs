...
LPM ships a small set of built-in tools — [Oxlint](https://oxc.rs) for [`lpm lint`](/docs/dev/lint), [Biome](https://biomejs.dev) for [`lpm fmt`](/docs/dev/fmt), and a type-check wrapper for [`lpm check`](/docs/dev/check). The first two are **lazy-downloaded** on first use, cached, and pinned per-project. `lpm check` is **not** lazy-downloaded — it shells out to the selected project-local engine (`tsc` by default, `tsgo` when you pass `--engine tsgo`).
...
`lpm check` is **not** a plugin — the wrapper shells out through the same PATH-injection mechanism `lpm run` uses. PATH-injection prepends `node_modules/.bin` and falls back to the system `PATH`, so a globally-installed engine can run too — but the project-local install is the right answer. For `tsc`, editor IntelliSense uses workspace `node_modules/typescript`, and a system `tsc` may be a different version. [`lpm doctor`](/docs/infra/doctor) flags the system-only case as `typescript_missing_for_tsconfig` (warn). For `tsgo`, install `@typescript/native-preview` in the project so the native binary and its bundled `lib/*.d.ts` files stay together.
...
- [`lpm check`](/docs/dev/check) — NOT a plugin — uses project-local `tsc` by default or `tsgo` when selected
