...
description: "How LPM ships Oxlint and Biome, plus how `lpm check` and `lpm bundle` use managed engines."
---

LPM ships a small set of built-in tools — [Oxlint](https://oxc.rs) for [`lpm lint`](/docs/dev/lint), [Biome](https://biomejs.dev) for [`lpm fmt`](/docs/dev/fmt), a type-check wrapper for [`lpm check`](/docs/dev/check), and a bundler wrapper for [`lpm bundle`](/docs/dev/bundle). Oxlint and Biome are **plugin-backed**: lazy-downloaded on first use, cached, and pinned per-project. `lpm check --engine tsgo` and `lpm bundle` use a separate **managed engine** cache under `~/.lpm/engines`.
...
That split is intentional: `tsc` preserves editor and CI parity, while `tsgo` and `rolldown` need full package layouts that do not fit the single-binary plugin model.
