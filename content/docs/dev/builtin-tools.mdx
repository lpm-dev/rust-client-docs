...
description: "How LPM ships Oxlint and Biome — lazy-downloaded, version-pinned, checksum-verified. Plus how `lpm check` integrates with the project's TypeScript install."
---
...
`lpm check` is **not** a plugin — `tsc` lives in `node_modules/.bin` and the wrapper just shells out via the same PATH-injection mechanism `lpm run` uses. PATH-injection prepends `node_modules/.bin` and falls back to the system `PATH`, so a globally-installed `tsc` will run too — but you don't want that. Editor IntelliSense uses workspace `node_modules/typescript`, and a system `tsc` may be a different version. [`lpm doctor`](/docs/infra/doctor) flags the system-only case as `typescript_missing_for_tsconfig` (warn). The fix is always `lpm install -D typescript`.
