...
`lpm dlx <pkg>@<version>` pins the requested version. `lpm dlx <pkg>` reuses the cached entry while it is fresh; once the TTL expires, the next run reinstalls against the current resolution of that spec.
