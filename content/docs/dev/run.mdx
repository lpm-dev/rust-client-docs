...
`lpm` falls through to `lpm run` when the first argument isn't a known subcommand. So `lpm build`, `lpm test:unit`, `lpm whatever` all resolve as `lpm run <name>` if `<name>` exists in `package.json > scripts`. Matches `npm`, `pnpm`, and `bun` conventions.

```bash
lpm build              # equivalent to lpm run build
lpm typecheck          # equivalent to lpm run typecheck
lpm db:migrate         # equivalent to lpm run db:migrate
```
...
`--filter` accepts the [filter grammar](/docs/packages/workspaces): exact name (`foo`), glob (`@scope/*`), path glob (`./apps/*`), forward closure (`foo...`), reverse closure (`...foo`), exclusion (`!foo`).
...
| `--all`               | Run in every workspace member (topological order); mutually exclusive with `--filter` / `--affected` |
