...
## Shorthand: `lpm <script-or-bin>` without `run`
...
## Hooks, environment, and failures

Package scripts run in this order: `pre<script>`, `<script>`, then `post<script>`. A failed phase stops the remaining phases.

The same order applies to sequential, parallel, streamed, and cached execution.

Captured logs share a 10 MiB limit per stream across all phases. Live output continues after that limit.

Explicit `lpm.json` task commands run directly. They do not use npm pre/post hook lookup.

Each phase receives [npm script context](/docs/packages/npm-compatibility#script-execution), including its command, package identity, manifest path, and original invocation directory.

Extra CLI arguments go to the main script. They do not change `npm_lifecycle_script`, which contains the declared command.

A single requested task with no task dependencies preserves its child exit code. Multiple tasks return the number of failed tasks.

Workspace runs exit nonzero if a selected member fails. Failure reports identify the child status and failed phase when available.

With `--json`, child output and cached logs go to stderr. Stdout contains one result envelope.
...
Manage cache state with [`lpm cache`](/docs/packages/cache). See [Remote task cache](/docs/infra/remote-cache) for setup and CI examples.
...
On macOS, [LPM Vault](/docs/dev/lpm-vault) can manage these values in the app. The `vault` field in `lpm.json` connects the project to the same local storage.
