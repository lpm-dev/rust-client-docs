...
| `lpm test` / `lpm bench`       | passthrough only | passthrough only | passthrough only |

For `lpm test` and `lpm bench`, workspace-scoped runs go through [`lpm run`](/docs/dev/run) today: `lpm run test --filter web --affected`.
