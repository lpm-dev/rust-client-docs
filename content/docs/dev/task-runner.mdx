...
Use `LPM_REMOTE_CACHE_TOKEN` in CI, or rely on the token from `lpm login`. When `signature: true`, set `LPM_REMOTE_CACHE_SIGNATURE_KEY` on every machine that reads or writes the cache. A missing or invalid signature is a cache miss.
