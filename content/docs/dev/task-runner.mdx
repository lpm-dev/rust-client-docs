...
`--no-bail` continues independent tasks after a failure. A failed explicit `^task` prerequisite still prevents its dependent task from running.
Without this flag, a failure stops later batches. Tasks that already started finish.
...
The runner adds project and workspace executables from `node_modules/.bin` to `PATH`.
Package scripts also run their `pre<name>` and `post<name>` hooks. Explicit task commands do not use package hooks.
