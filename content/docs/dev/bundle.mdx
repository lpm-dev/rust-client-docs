---
title: lpm bundle
description: "Bundle the project with Rolldown through an LPM-managed plugin."
---
...
`lpm bundle` does not rely on a project-local `rolldown` dependency. On first use, LPM installs its approved Rolldown version into `~/.lpm/engines/rolldown/<version>/<platform>/`, verifies every tarball's SRI integrity, preserves the full extracted package layout, and reuses that verified cache on later runs.
