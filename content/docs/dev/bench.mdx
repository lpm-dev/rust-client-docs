...
Both single-package and workspace runs emit one LPM CLI envelope with `--json`. Failed members include bounded captured output or a launch error. Successful members omit their captured output. See [test and bench runners](/docs/dev/test-bench-runners) for environment, runtime, hook, and cleanup behavior.
