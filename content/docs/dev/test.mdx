...
The workspace flags (`--all`, `--filter`, `--affected`, `--base`, `--fail-if-no-match`, `--workspace-concurrency`) are claimed by LPM. To pass any of them through to the underlying runner (e.g. bun's `--filter`, jest's `--all`), put them after `--`:
