...
4. **HTTPS** — if `lpm.json` has `https: true` (top-level) or you pass `--https`, serves over `https://localhost` using the local CA. See [`lpm cert`](/docs/infra/cert) for the trust store install.
...
For every other service with a declared `port`, LPM also injects `{SERVICE}_URL` and `{SERVICE}_PORT` into the child environment. For example, an `api` service can read `DB_URL=http://localhost:5432` and `DB_PORT=5432`, while `web` can read `API_URL` and `API_PORT`.
...
| `-y`, `--yes`              | Pre-approve the trust-store install for `--https`                                                            |
...
<Cards>
  ...
  <Card
    title="lpm use"
    href="/docs/dev/use"
    description="Pin a Node.js version, with auto-install on use."
  />
  <Card
    title="lpm plugin"
