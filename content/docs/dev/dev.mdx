...
5. **Tunnel** — if `lpm.json` has `tunnel.domain` set or you pass `--tunnel`, exposes the dev server through an LPM CLI tunnel.
...
Exposes the dev server through the LPM CLI tunnel network. Webhooks land on the public URL, get logged to disk, and are visible via `lpm tunnel inspect` / `lpm tunnel replay`. See [Tunneling](/docs/infra/tunneling).
