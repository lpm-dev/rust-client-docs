...
3. **Env** — if `.env` is missing but `.env.example` exists, copies it once, including in projects that use a vault. The mode selection order is `--env`, `tasks.dev.env`, `env.dev`, then the default environment. Mode files load after `.env` and `.env.local`. Vault values load last and override matching file values.
...
## Secrets from LPM Vault

[LPM Vault](/docs/dev/lpm-vault) stores project variables in the macOS Keychain. The `vault` field in `lpm.json` connects the project to those values.

```bash
lpm dev --env=development
```

The selected vault values enter the application environment at startup. After a value changes, restart the development server.

If `.env` is missing and `.env.example` exists, the CLI still creates `.env` from that example. This copy does not export vault values.

Keep `.env.example` free of real secrets. See [Using LPM Vault](/docs/dev/lpm-vault) for setup, environment selection, and migration from dotenv files.
