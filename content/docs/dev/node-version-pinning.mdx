...
For workspace runs, LPM CLI resolves every runnable selected member's local Node selector and validates the first Node on that member's script `PATH` against the root constraint before scheduling any member scripts. Members without the requested script or task are skipped without runtime preparation. A runnable member cannot pass the root check under one Node version and then execute under an incompatible runtime.
