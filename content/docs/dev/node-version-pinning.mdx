...
The workspace root's `engines.node` is a compatibility constraint, never a runtime selector. LPM CLI first applies `lpm.json > runtime.node`, `.nvmrc`, or `.node-version`; without a selector it preserves the current `node` from `PATH`. It then validates that effective Node against `engines.node`.

For workspace runs, LPM CLI resolves every selected member's local Node selector and validates that member's effective runtime against the root constraint before scheduling any member scripts. A member cannot pass the root check under one Node version and then execute under an incompatible managed runtime.
