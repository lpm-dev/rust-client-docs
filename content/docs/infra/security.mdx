...
`enable` writes `[firewall].mode` into the managed policy path and then reloads it through the same authority checks LPM uses at runtime. The default is `--firewall enforce`; `--firewall monitor` is available for admins who want managed visibility without blocking. `off` is intentionally not accepted by `protect enable`.
