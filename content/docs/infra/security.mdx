...
- `default` expands to the common install/runtime weakeners: `cooldown-bypass`, `cooldown-window`, `provenance-ignore-drift`, `provenance-unverified`, `scripts-triage`, `scripts-allow`, `sandbox-default`, `sandbox-none`, `sandbox-allow-degraded`, and `typosquat-disable`.
...
- the effective values for script policy, release-age floor, release-age policy, sandbox mode, sandbox degraded mode, and Sigstore verification
