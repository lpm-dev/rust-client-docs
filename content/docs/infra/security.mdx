...
<figure className="not-prose my-8 border-0 bg-transparent">
  <Image
    src="/images/security-approval-macos-light.jpg"
    alt="macOS asks for Touch ID or a password to approve a persistent machine-level security change in LPM CLI."
    width={618}
    height={638}
    sizes="(max-width: 412px) calc(100vw - 32px), 380px"
    className="mx-auto block h-auto w-full max-w-[380px] rounded-[40px] dark:hidden"
  />
  <Image
    src="/images/security-approval-macos-dark.jpg"
    alt="macOS asks for Touch ID or a password to approve a persistent machine-level security change in LPM CLI."
    width={618}
    height={638}
    sizes="(max-width: 412px) calc(100vw - 32px), 380px"
    className="mx-auto block h-auto w-full max-w-[380px] rounded-[40px] not-dark:hidden"
  />
  <figcaption className="mt-3 text-center text-sm text-fd-muted-foreground">
    macOS approval prompt for a persistent machine-level security change.
  </figcaption>
</figure>
...
The grant expires, but the approved configuration changes remain. The grant cannot override managed policy.
While `force-security-floor` is active, lower controls remain protected.
The grant permits an explicit change to that switch through [`lpm config`](/docs/reference/config-toml#security-sensitive-keys-are-proposals).
