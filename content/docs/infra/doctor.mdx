...
`--fix` only repairs what was actually checked — running `--fix` without `--all` cannot fix rows that the fast preset never ran. Pair `--all --fix` for the full repair pass.
