...
The full fix mode can claim a configured tunnel domain. It can also update `.gitattributes` and project formatting.
...
`-y` and `--yes` imply `--fix`. They currently have the same effect as `--fix` because doctor does not show confirmation prompts.
...
| `--fix` | Apply supported fixes immediately |
| `-y`, `--yes` | Imply `--fix` and currently have the same behavior as `--fix` |
