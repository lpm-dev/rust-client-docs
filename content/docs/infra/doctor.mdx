...
Without repairs, fast mode does not contact a registry or tunnel service. It does not start background update requests or run lint or format checks.
...
Add `--all` for tool, machine, and connected-service checks. Full mode requires registry credentials for a passing authentication result.
...
Save your project changes before you use `--fix`. Some fixes can change project files or installed dependencies. Repairs can download packages and runtimes, even without `--all`.
...
The JSON field `fixes_applied` lists completed fixes. `fixes_failed` lists unsuccessful actions and their errors. Targeted actions include the plugin name, tunnel domain, or runtime version selector.
After repairs, doctor reloads root and member manifests before it reruns the same check set. A failed repair appears in the final terminal summary and keeps the exit code at `1`.

Repairs apply once to the initial report. The final report can show remaining or newly discovered problems.
...
| `vault_storage_keychain` | pass | macOS Keychain is the selected backend for local secrets. This result does not probe Keychain access or secret decryption. |
| `vault_storage_native` | pass | A native data key exists, or the OS secure store is the preferred backend for new local secrets. Read the detail to distinguish these states. |
| `vault_storage_fallback` | warn | Local secrets use the fallback key at `~/.lpm/.vault-fallback-key`. Processes from the same user can read this key. **Fix:** Unlock or repair the OS secure store. |
| `vault_storage_unavailable` | fail | Encrypted local secret files exist, but LPM CLI cannot access either local key. **Fix:** Unlock the secure store. If necessary, restore the fallback key from backup. |
| `auth_unverified` | warn | The credential could not be checked because the registry request failed. |
| `vault_storage_native_with_fallback` | warn | A native data key and a local fallback key both exist. The fallback key still permits offline decryption. |
...
| `gitattributes_lockb_unmarked` | warn | `.gitattributes` does not mark `lpm.lockb` as binary. **Fix:** Run `lpm doctor --all --fix` or add `lpm.lockb binary`. |
| `gitattributes_missing` | warn | No `.gitattributes` file exists, so Git can treat `lpm.lockb` as text. **Fix:** Run `lpm doctor --all --fix` or add `lpm.lockb binary`. |
...
| Code | Severity | Description |
| --------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sigstore_verify_enforced` | pass | When verification runs, a rejected attestation stops the install or approval. This result does not examine verification scope or fetch availability. |
...
Use `checks[].code` from `lpm doctor --json`:
