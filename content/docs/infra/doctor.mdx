...
- **`lpm doctor`** — checks the **project + environment**: Node version detection, `lpm install` state, global store accessibility, vault-storage and Sigstore posture, plus the broader `--all` sweep for registry/auth/tunnel/tooling/sandbox state. The big "is everything OK here?" command.
...
| `vault_storage_fallback` | warn     | On Linux/Windows, vault secrets fall back to the encrypted-file backend at `~/.lpm/.vault-fallback-key`; any same-UID process can still read that key file. **Fix:** Treat the host as the trust boundary and keep untrusted lifecycle scripts sandboxed. |
