...
- Replace a project-level `node_modules` link with a real directory
...
The fix removes only the link entry and creates a real `node_modules` directory. The linked target and its contents stay unchanged.

You can also remove the link entry manually. Then run `lpm install` to create and populate the directory.
...
| `engines_bun_ignored`         | warn     | `engines.bun` is declared but LPM CLI does not enforce bun's version constraint. **Fix:** Use `lpm.json > runtime.bun` when the project needs managed Bun on `PATH`. LPM CLI enforces `engines.node` and `engines.lpm`.                                                               |
