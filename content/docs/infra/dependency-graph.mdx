...
description: "How LPM CLI models the installed dep tree — node identity, duplicates, registry attribution, paths, statistics."
---

[`lpm graph`](/docs/packages/graph) renders the installed dependency tree in tree, DOT, Mermaid, JSON, stats, or HTML form. Behind it is a small data model — `DepNode` per `(name, version)` pair, with depth, registry attribution, duplicate detection, and path-finding — that several other commands also read from.
...
| `lpm_packages`   | Nodes with `Registry::Lpm` (sourced from the LPM.dev Registry)                                                                                                                                                                                       |
...
Useful for high-level audits — "how many of my deps are from the LPM.dev Registry vs npm", "do I have ten copies of `loose-envify`", "how deep does my tree actually go".
...
- [`lpm graph`](/docs/packages/graph) — CLI command reference
