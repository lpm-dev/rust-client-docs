...
Webhooks that arrive at the public URL are captured in the current project's `.lpm/inspector.db`. Terminal inspection, replay, sessions, and the browser inspector all use that same project-local history.
...
lpm tunnel inspect --ui           # open this project's history in a browser
...
Captured request and response bodies, headers, statuses, and session metadata live in `.lpm/inspector.db`. The terminal `inspect` / `log` views, replay, and `inspect --ui` select the database from the current project, so one project cannot inspect or replay another project's captures. The browser view also refreshes when a running tunnel process adds records.

Legacy `.lpm/webhook-log*.jsonl` and `.lpm/webhooks/*.json` files are preserved if they already exist, but current LPM CLI versions no longer read, write, merge, or delete them. `lpm tunnel log --clear` clears captured requests and completed sessions from the active SQLite history. A currently open session row remains so its next capture stays linked and persistence can continue.

Replays use an explicit `--port`, or the single active `lpm dev` endpoint when no replay port is given. Ports must be in `1..=65535`; zero, negative, and overflowing values fail instead of wrapping.

The standalone browser inspector also adopts the single active `lpm dev` endpoint as its default replay target. If there is no active endpoint, or more than one, enter the intended port in the replay panel.
...
`lpm dev --tunnel` accepts the same `--inspect-port` and `--no-inspect` flags and surfaces the inspector URL in the startup banner. `--no-inspect` skips only the browser server; capture still persists to `.lpm/inspector.db`. During multi-service startup the inspector reports that it is waiting for the dev server instead of showing a guessed port; after ownership verification its status includes the resolved `local_url`. The tunnel footer's `o` key opens the inspector in a browser.
...
Inspect, replay, and log options can follow the action directly (for example: `lpm tunnel inspect --last 10`, `lpm tunnel replay 3 --port 4000`).
