...
After a network interruption, an established tunnel also retries a temporary capacity rejection while its old connection expires.
A new tunnel still stops when active tunnels fill the account limit.

After reconnection, use the latest `tunnel_url`. Free sessions can receive a different random URL.
Applications must reconnect their SSE and WebSocket streams. See [Reliability and recovery](/docs/infra/tunneling#reliability-and-recovery).
