...
Legacy `.lpm/webhook-log*.jsonl` and `.lpm/webhooks/*.json` files are preserved if they already exist, but current LPM CLI versions no longer read, write, merge, or delete them. `lpm tunnel log --clear` clears the active SQLite history only.
