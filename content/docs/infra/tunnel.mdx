...
Returns an `https://` URL that proxies to `localhost:3000`. Requires an LPM login — `lpm tunnel` errors with `authentication required. Run \`lpm login\` first` if no token is stored. Free users get an ephemeral random domain on every run; Pro and Org users can claim a stable one (see below).
