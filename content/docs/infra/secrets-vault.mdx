...
For `lpm env pull --oidc`, [`LPM_VAULT_ID`](/docs/reference/env-vars#auth-and-routing) can replace the resolved vault ID. The current implementation still reads `lpm.json > vault` first, so the field must exist locally before the environment-variable override is applied. Other `lpm env` operations do not read this variable. The CLI also tracks per-vault sync state in `lpm.json > vaultSync.personalVersion` and `vaultSync.orgVersions.<slug>` for push conflict detection; those are written by `lpm env push` / `lpm env pull` and shouldn't be edited by hand.
...
Run these commands on a logged-in development machine. The personal vault must already exist on the Registry before `oidc allow` can enable escrow. The command must finish without an escrow warning; a policy without escrow material cannot serve CI pulls.
...
The `--oidc` flag requests a GitHub Actions OIDC token, exchanges it through `POST /api/vault/oidc`, and uses the resulting short-lived token with `GET /api/vaults/{id}/ci-pull`. The checkout must contain its committed `lpm.json > vault` value.
