...
Local CLI surface: [`lpm use vars`](/docs/dev/use#env-vars-lpm-use-vars). Web dashboard: [lpm.dev/dashboard/secrets](https://lpm.dev/dashboard/secrets).
...
The local vault is unconditional — every plan can store secrets in the keychain. **Cloud sync is plan-gated** — `lpm use vars sync` against a free account returns `403 "Vault sync requires a Pro or Org plan"`.
...
```bash
LPM_FORCE_FILE_VAULT=1 lpm use vars list
```
...
You don't usually deal with the vault-id by hand — `lpm use vars` operations resolve it transparently.
...
```bash
lpm use vars set DATABASE_URL=...                          # → "default" environment
lpm use vars set --env=staging DATABASE_URL=...            # → "staging" environment
lpm use vars set --env=production DATABASE_URL=...         # → "production" environment
...
```bash
lpm use vars sync push          # encrypt + push to server
lpm use vars sync pull          # pull + decrypt
lpm use vars sync               # push or pull based on which side is newer
```
...
The pull response is **digitally signed** (`signedJsonResponse` on the server side). The CLI verifies the signature before trusting the encrypted blob, so a MITM that rewrites the response can't trick the CLI into decrypting attacker-controlled bytes.
...
The **schema** is interesting — your `lpm.json > envSchema` (`{required, format, pattern, default, secret}` per var) travels alongside the encrypted blob. The dashboard renders it read-only so a teammate viewing the vault sees which keys are required, which are secret, etc., without seeing values.

The **name** field is the project's directory name, captured by the CLI. Helps the dashboard render `apps/web` instead of `vault-7f3a-...-9b1d`.
...
3. You run `lpm use vars pair <code>` on a machine that already has the wrapping key.
...
```bash
lpm use vars audit                            # last 50 entries for this project's vault
lpm use vars audit --cursor <id>              # paginate
```
...
Pre-Phase-X versions derived the wrapping key from `SHA256("lpm-vault-wrap:" + auth_token)`. That tied the vault to your auth token — a token rotation broke decrypt.

Current versions use an independent random wrapping key in the keychain. On decrypt failure with the new key, LPM tries the legacy key. If the legacy key works, the blob is re-encrypted under the new key transparently and pushed back. Migration completes on the next push.

If you've rotated tokens AND the data was last encrypted under the old token-derived key AND you no longer have the old token, the legacy path can't recover. Practical workaround: pair a still-paired machine, transfer the wrapping key, re-encrypt.
...
- [`lpm use vars`](/docs/dev/use#env-vars-lpm-use-vars) — CLI surface for everyday use
