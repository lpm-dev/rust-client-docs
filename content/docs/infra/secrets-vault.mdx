...
CI is the one case where **server-side decryption is allowed** — by design. The supported path is a personal vault owned by a Pro account and pulled from GitHub Actions or GitLab CI. CI runs in an environment that has no keychain, no persistent state between runs, and no interactive way to pair a wrapping key, so vault sync supports an opt-in escrow:
...
The `--oidc` flag requests a GitHub Actions OIDC token, exchanges it through `POST /api/vault/oidc`, and uses the resulting short-lived token with `GET /api/vaults/{id}/ci-pull`. Set a non-empty `LPM_VAULT_ID` to bootstrap a checkout without a local vault field; otherwise LPM CLI uses `lpm.json > vault`.
...
GitLab policies instead use `subject: "project:<numeric-project-id>"`, require no workflow or event list, and reject GitHub-only fork settings. Create one with `lpm env oidc allow --provider=gitlab --project-id=12345 --branch=main --env=production`. Organization-vault OIDC policies remain unsupported and cannot be created.
