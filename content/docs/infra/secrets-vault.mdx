...
3. Sends the plaintext env values **directly** to the Vercel or Coolify API.

The LPM.dev Registry stores the platform credential encrypted with AES-256-GCM plus a bounded connection config (project or application ID, selected target, and linked environment). It handles the credential during authenticated connect and credential retrieval, but it never receives the application env values.
...
Connect, push, pull, clean, and status are CLI operations. The dashboard lists connection metadata and can disconnect a credential; it never exposes tokens or plaintext env values.
