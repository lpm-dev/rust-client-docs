...
Linux default      → ~/.lpm/vaults/<vault-id>.enc  (AES-encrypted; Unix perms tightened to owner-only)
Windows default    → ~/.lpm/vaults/<vault-id>.enc  (AES-encrypted local file)
macOS debug/test   → Same encrypted file backend when LPM_FORCE_FILE_VAULT=1
```

On macOS, the CLI reads and writes Keychain items via service `dev.lpm.vault`, and the internal LPM Vault app uses the same layout. On Linux and Windows, the CLI uses the encrypted file backend instead.
...
On macOS, this is stored as `{environment_name: {key: value}}` JSON inside one Keychain item per project. On Linux and Windows, the same environments map lives in the encrypted local vault file. `lpm dev --env=staging` (or `lpm run dev --env=staging`) injects the staging-scoped vars into the script.
...
The `--oidc` flag routes through the OIDC exchange (`POST /api/vault/oidc`) and then through the CI-escrow path (`GET /api/vaults/{id}/ci-pull`). [`lpm setup ci --oidc`](/docs/reference/authentication#lpm-setup-ci) generates the matching `.npmrc` and is usually run as a separate prior step in the same workflow.
...
- [`lpm setup ci --oidc`](/docs/reference/authentication#lpm-setup-ci) — CI auth for `ci-pull`
