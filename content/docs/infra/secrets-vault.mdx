---
title: Secrets vault
description: "How LPM CLI stores per-project secrets and uses the LPM.dev Registry to sync and share them."
---

The vault is LPM CLI's internal secrets system. Locally, it stores per-project env vars in the macOS Keychain on macOS. On Linux and Windows, it keeps encrypted vault blobs on disk and protects the local data key with the OS secure store when available: Secret Service-compatible storage on Linux, Credential Manager on Windows. With a Pro or Org plan, those vars **sync to the LPM.dev Registry** end-to-end encrypted. Org sync makes them shareable across team members. Personal Pro vaults can opt into GitHub Actions or GitLab.com CI OIDC pulls, which allow the Registry to decrypt one environment for an authorized CI identity without a long-lived Registry token. Self-managed GitLab and organization-vault OIDC are not supported. You can also push the same env project directly to Vercel, Coolify, Fly.io, Railway, or GitHub Actions without transmitting plaintext env values through the server.

Local CLI surface: [`lpm env`](/docs/dev/env). Web dashboard: [lpm.dev/dashboard/secrets](https://lpm.dev/dashboard/secrets). Today, [`lpm env`](/docs/dev/env) is the documented local surface.
...
| **Pro**  | Yes         | Yes        | No          | Personal vault via GitHub Actions or GitLab.com CI | Yes              |
| **Org**  | Yes         | Yes        | Yes         | Not for organization vaults | Yes              |
...
debug/test builds  → encrypted-file fallback when LPM_FORCE_FILE_VAULT=1
```

On macOS, LPM CLI reads and writes Keychain items via service `dev.lpm.vault`. On Linux and Windows, the encrypted vault blob stays at `~/.lpm/vaults/<vault-id>.enc`, while the local data key lives in the native secure store under service `dev.lpm.vault-local-key`, account `data-key`.

Existing Linux/Windows vaults that used `~/.lpm/.vault-fallback-key` are promoted automatically the first time the native store is reachable: LPM CLI stores the derived data key in the OS secure store and removes the old fallback key. If the native store is unavailable before promotion, LPM CLI can still use the encrypted-file fallback. After promotion, a locked or broken native store is a loud error, not a silent downgrade.

Maintainer debug/test binaries can force the file backend with `LPM_FORCE_FILE_VAULT=1` to exercise fallback behavior. Installed release builds ignore that override and keep using the platform default.
...
For `lpm env pull --oidc`, a non-empty, trimmed [`LPM_VAULT_ID`](/docs/reference/env-vars#auth-and-routing) is resolved first and can bootstrap a checkout without `lpm.json > vault`. If the variable is unset, empty, or whitespace-only, LPM CLI falls back to the local field. If neither source is available, the pull fails with a configuration error. Other `lpm env` operations do not read this variable. The CLI also tracks per-vault sync state in `lpm.json > vaultSync.personalVersion` and `vaultSync.orgVersions.<slug>` for push conflict detection; those are written by `lpm env push` / `lpm env pull` and shouldn't be edited by hand.
...
```text
[your machine]             [LPM.dev Registry]              [teammate's machine]
                                                                   ▼
...
1. **Wrapping key** — a 256-bit random key, generated **once per machine**, stored in the local key store: system keyring when available, otherwise `~/.lpm/.vault-key`. It leaves the machine only when you explicitly enable CI escrow for a personal vault; that exception is described below.
...
Without CI escrow, the server holds **encrypted blob + wrapped key** and can't read either — decryption requires wrapping-key material held by a paired machine. Enabling CI escrow deliberately gives the Registry an encrypted server-side copy of the wrapping key so an authorized workflow can request one plaintext environment.
...
```text
vault_sync row:
├── encryptedBlob          ← AES-256-GCM ciphertext
...
The **schema** is interesting — your `lpm.json > envSchema` (`{required, format, pattern, default, secret}` per var) travels alongside the encrypted blob. The dashboard renders it read-only so a teammate viewing the vault sees which keys are required, which are secret, etc., without seeing values. `schema` is rebuilt from `lpm.json` on every push; if `lpm.json` is missing the CLI sends no `schema` and the server keeps the last-known-good value. If `lpm.json` is malformed, oversized, or unreadable, push fails before upload because aliases, inheritance, and schema metadata are required to preserve environment boundaries.
...
LPM CLI uses an **X25519 + AES** scheme similar to ECIES:

1. Each org member registers an **X25519 public key** with the LPM.dev Registry (the server stores the public key in `user_public_keys`; the private key stays on the member's machine).
...
Your **sharing key** is the X25519 keypair the org-sync diagram above relies on. It's per-account, not per-device: the public half is registered on the LPM.dev Registry under `user_public_keys`, while the private half stays only on the local machine in LPM CLI's key storage.
...
- **Personal vault ciphertext or the per-machine wrapping key.** `rotate-sharing-key` affects only the account-level X25519 sharing key. [`lpm env rotate-key`](/docs/dev/env#re-encrypting-a-personal-cloud-vault) pulls and re-encrypts the complete authoritative personal payload with a fresh data key, preserving named environments and metadata under version-conflict protection.
...
- **Auth token / registry session.** Separate primitive again; rotate with [`lpm token-rotate`](/docs/infra/token-rotate).
...
CI is the one case where **server-side decryption is allowed** — by design. The supported path is a personal vault owned by a Pro account and pulled from GitHub Actions or GitLab.com CI. Self-managed GitLab issuers are not supported. CI runs in an environment that has no keychain, no persistent state between runs, and no interactive way to pair a wrapping key, so vault sync supports an opt-in escrow:

1. After creating the OIDC policy, LPM CLI sends the wrapping key to the Registry over authenticated TLS.
2. The Registry encrypts it with `VAULT_CI_ESCROW_KEY` and stores the result in `vault_sync.ciEscrowWrappingKey`.
3. CI hits `GET /api/vaults/{id}/ci-pull?env=production`.
4. The server verifies the request is from a valid OIDC exchange (token name prefix `oidc:{provider}:{subject}:{branch}`).
5. The server unwraps the escrowed wrapping key, decrypts the blob, filters by environment, and returns plaintext secrets.
...
Run these commands on a logged-in development machine. The personal vault must already exist on the Registry before `oidc allow` can enable escrow. The command succeeds only after policy creation and the required wrapping-key retrieval and escrow upload have all completed. If escrow setup fails, it exits non-zero without reporting policy success and explains that the policy may already exist while CI pulls remain unavailable. Fix the escrow problem and rerun the same command; rerunning safely updates the matching policy.

The authorized GitHub Actions workflow then pulls the selected environment:

```bash
lpm env pull --oidc --env=production --output=.env
```

The `--oidc` flag resolves an audience-`https://lpm.dev` OIDC token from the CI provider, exchanges it through `POST /api/vault/oidc`, and uses the resulting short-lived token with `GET /api/vaults/{id}/ci-pull`. In GitHub Actions, LPM CLI requests the runtime token directly. In GitLab.com CI, provide the job token as `LPM_OIDC_TOKEN` through an `id_tokens` block. Set a non-empty `LPM_VAULT_ID` to bootstrap a checkout without a local vault field; otherwise LPM CLI uses `lpm.json > vault`.
...
The workflow path is mandatory and must be a `.yml` or `.yaml` file directly under `.github/workflows/`. The exchange validates the token's repository, branch, workflow file, event, fork status, and requested environment against the policy. A mismatch returns `403`.

GitLab.com policies instead use `subject: "project:<numeric-project-id>"`, require no workflow or event list, and reject GitHub-only fork settings. The exchange requires the token's `ref_type` to be `branch`, so a tag with the same name as an allowed branch is not authorized. Create a policy with `lpm env oidc allow --provider=gitlab --project-id=12345 --branch=main --env=production`. Organization-vault OIDC policies remain unsupported and cannot be created.

`lpm env oidc allow` is a complete-policy upsert: its branch, environment, workflow, event, and fork values replace the corresponding allowlists instead of merging one denied value into them. Inspect the current policy with `lpm env oidc list` and review `lpm env oidc allow --help` before changing an existing policy.
...
Push your local env project to an external platform without the LPM.dev Registry ever transmitting plaintext env values. LPM CLI:

1. Loads the selected local environment from secure local storage.
2. Retrieves the encrypted-at-rest platform credential through the authenticated, signed LPM.dev Registry API.
3. Sends values **directly** to the Vercel, Coolify, Fly.io, Railway, or GitHub API. GitHub secret values are first sealed locally with GitHub's Actions public key.

The LPM.dev Registry stores the platform credential encrypted with AES-256-GCM plus a bounded connection config (project, repository and immutable repository or app ID, organization identity, environment, service, or application ID, selected target, token mode, and linked environment). It handles the credential during authenticated connect and credential retrieval, but it never receives the application env values.

Supported platforms:

| Platform  | Use case |
| --------- | -------- |
| `vercel`  | Merge, compare, clean, and pull env vars for selected Vercel deployment targets |
| `coolify` | Merge, compare, clean, and pull readable production or preview application env vars; requires a team-administrator `root` token or `read` + `write` + `read:sensitive` token |
| `fly` | Refresh, compare names, clean, and report skipped write-only secrets for one identity-bound Fly.io app |
| `railway` | Verified bulk merge, exact clean, compare, and pull for Railway service or shared environment variables |
| `github-actions` | Exact readable-variable sync plus sealed write-only secrets at repository or environment scope |

Endpoints:

| Endpoint                                | Purpose |
| --------------------------------------- | ------- |
| `POST /api/vault/platforms/connect`     | Store a CLI-verified encrypted platform credential and validated config |
| `GET /api/vault/platforms/connect`      | List connection metadata for the dashboard |
| `DELETE /api/vault/platforms/connect`   | Disconnect a stored platform credential |
| `POST /api/vault/platforms/credentials` | Return authorized connection data to the CLI in a signed response |
| `POST /api/vault/platforms/audit`       | Record bounded push or pull counts after a client-direct operation |

Connect, push, pull, clean, and status are CLI operations. The dashboard lists connection metadata and can disconnect a credential; it never exposes tokens or plaintext env values. Railway uses the fixed public GraphQL endpoint, disables redirects, reads unrendered references, and verifies every bulk mutation with an authoritative reread before recording success.

Fly.io binds a connection to the canonical app name, immutable app ID, and immutable organization ID. It disables redirects, rejects GraphQL errors even with HTTP 200, verifies identity and current secret names immediately before mutation, and rereads names before reporting success. Every local value is a write-only Fly secret. Push refreshes every expected value through `setSecrets`; clean removes extra names through `unsetSecrets`. Both mutations create Fly releases. Status verifies names only, and pull reports skipped names without replacing local values.

GitHub Actions binds a connection to the canonical repository name and immutable repository ID. It disables redirects, bounds pagination, and verifies that binding before every operation. Schema keys marked `client: true` without `secret: true` are readable Actions variables; every other key defaults to an Actions secret. Status and post-push verification compare readable values exactly and secret names only. Pull skips write-only secrets, while clean removes extra names from both namespaces. Failed multi-request pushes record only bounded counts for mutations GitHub had already accepted.

Cloud storage and platform deployment are intentionally separate. `lpm env push` encrypts locally and stores ciphertext plus wrapped key material on LPM.dev; `lpm env push --to <platform>` sends plaintext directly to that platform. Neither operation claims atomic rollback across the two independent systems.
...
Action set includes `push`, `pull`, `share`, `unshare`, `rotate_key`, `member_added`, `member_removed`, `platform_connect`, `platform_push`, `platform_push_failed`, and `platform_pull`. Each entry records the user, optional org, action, bounded metadata (version numbers, key fingerprint diffs, or platform operation counts), and timestamp.
...
Current versions use an independent random wrapping key in the keychain. On decrypt failure with the stored key, LPM CLI tries the legacy key as a fallback. If the legacy key works, the blob is re-encrypted under the stored key and pushed back inside the same `pull` — migration is automatic and converges without an extra command. The migration push is best-effort: if the network blips or the version races, the failure is logged at `warn` (not silent), and the next successful pull retries. A push from a machine that already has intact local secrets also implicitly migrates the cloud blob, since push always re-encrypts under the current stored key.
...
- [GitHub Actions OIDC setup](/docs/dev/env#oidc-policies-ci) — authorize and pull a personal Pro vault
- [`lpm.json` envSchema](/docs/reference/lpm-json#envschema) — what gets synced alongside the blob
- [Authentication](/docs/infra/authentication) — how the OS keychain backs LPM CLI auth too
