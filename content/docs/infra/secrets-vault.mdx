...
Action set includes `push`, `pull`, `share`, `unshare`, `rotate_key`, `member_added`, `member_removed`, `platform_connect`, `platform_push`, and `platform_pull`. Each entry records the user, optional org, action, bounded metadata (version numbers, key fingerprint diffs, or platform operation counts), and timestamp.
