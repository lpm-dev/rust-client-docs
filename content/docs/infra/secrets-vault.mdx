...
1. When you enable CI for a vault, the CLI re-wraps your wrapping key with `VAULT_CI_ESCROW_KEY` (a key the LPM.dev Registry holds) and stores the result in `vault_sync.ciEscrowWrappingKey`.
...
The `--oidc` flag routes through the OIDC exchange (`POST /api/vault/oidc`) and then through the CI-escrow path (`GET /api/vaults/{id}/ci-pull`). [`lpm setup ci npmrc --oidc`](/docs/infra/setup) generates the matching `.npmrc` and is usually run as a separate prior step in the same workflow.
...
2. Retrieves the encrypted-at-rest platform credential through the authenticated, signed LPM API.
...
- [`lpm setup ci npmrc --oidc`](/docs/infra/setup) — CI auth for `ci-pull`
