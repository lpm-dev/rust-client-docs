...
- **Personal vault ciphertext or the per-machine wrapping key.** `rotate-sharing-key` affects only the account-level X25519 sharing key. [`lpm env rotate-key`](/docs/dev/env#re-encrypting-a-personal-cloud-vault) does not replace the per-machine wrapping key; it re-encrypts the default local environment with a fresh data key and force-pushes it. Do not use that command on projects with named environments because it can replace the remote multi-environment data with default-environment values only.
...
CI is the one case where **server-side decryption is allowed** — by design. The currently supported path is a personal vault owned by a Pro account and pulled from GitHub Actions. CI runs in an environment that has no keychain, no persistent state between runs, and no interactive way to pair a wrapping key, so vault sync supports an opt-in escrow:
