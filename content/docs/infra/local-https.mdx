...
LPM CLI detects which framework `lpm dev` is starting and injects the right env vars:
...
If your framework needs different vars, you can read the cert paths yourself from the env LPM CLI injects and configure the server manually.
