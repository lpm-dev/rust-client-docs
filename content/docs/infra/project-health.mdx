...
The authoritative live catalog is [`lpm doctor list`](/docs/infra/doctor#lpm-doctor-list) (or `--json` for machine-readable). Highlights:
...
| --- | --- |
| Detected managed runtime not installed | Run [`lpm use node@<version>` or `lpm use bun@<version>`](/docs/dev/use) |
...
Full envelope shape (per-check fields) is documented on the [`lpm doctor`](/docs/infra/doctor#machine-readable-output---json) page. Useful for CI dashboards or agents that want to dispatch on specific failures rather than parse human output.
...
- **After upgrading LPM CLI**: confirms nothing in the new version broke your local state.
