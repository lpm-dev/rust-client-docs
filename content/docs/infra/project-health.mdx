...
| Runtime and security | Node.js, Bun, policy extensions, auth storage, local secret storage, and Sigstore posture | Sandbox and lifecycle-script policy |
...
Secret-storage results describe the selected backend or available local keys. A passing result does not prove that LPM CLI can decrypt every secret.

The Sigstore result reports the verification mode. Verification scope and fetch availability remain separate controls. See [security controls](/docs/infra/security).
...
The report separates completed actions in `fixes_applied` from failed actions in `fixes_failed`. Failed actions also appear in the final terminal summary. A failed action keeps the exit status unsuccessful, even if later checks pass.
