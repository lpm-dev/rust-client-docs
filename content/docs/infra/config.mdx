...
`lpm config firewall` accepts `off`, `report`, or `enforce`. `off` keeps the default direct npm metadata and tarball path with no firewall.lpm.dev verdict check. `report` sends selected public npm package versions to firewall.lpm.dev in batches, prints block/warn verdicts, and lets the command continue. `enforce` uses the same batched check and blocks packages marked malicious before package bytes are installed, fetched, downloaded, or copied through `lpm add`. When a human-output materialization command has eligible public npm packages to check, its progress line carries the `🔥 LPM Firewall active` badge. Private/custom registry packages and lpm.dev packages are not sent to the npm firewall verdict API. JSON output for materialization commands includes a `firewall` object when the firewall is active. Turning the firewall on is hardening and does not require an unlock; after `report` or `enforce` is approved as the machine floor, moving to a weaker mode is guarded.
