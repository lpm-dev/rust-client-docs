...
Generic `set` writes ordinary values as TOML strings. Known typed keys can preserve their native type; today `signatures` and `auto-install-lpm-skills` are written as booleans. The reader accepts both string and native forms for typed keys — see [String coercion](/docs/reference/config-toml#string-coercion) for the rationale.
...
| `engine-strict` | Enforce workspace-root `engines.lpm` / `engines.node` and dependency `engines.node` constraints. | `true` |
...
│  ○ LPM.dev package skills current: enabled (default)
```
...
Twelve of the keys above have dedicated interactive wizards that handle picking + validation for you:
...
`lpm config lpm-skills` controls only automatic package-published skill reconciliation for `@lpm.dev/*` packages. The default is enabled. Setting it to `false` keeps existing `.lpm/skills/` files and skips future automatic fetches; use [`lpm skills clean`](/docs/packages/skills) when you also want to remove existing package-owned skill content. Explicit [`lpm skills add`](/docs/packages/skills) commands are unaffected.
...
Bare `lpm config` and all twelve wizards write to the same `~/.lpm/config.toml` that `lpm config set` would. They require a TTY when invoked without `--set`; non-interactive callers must use an explicit action such as `lpm config list` or pass `--set <value>` to a specific wizard.
