...
To view only the values that are saved in the file, run:
...
| `lpm config set <key> <value>` | Save one top-level value | The key has no focused editor |
...
| `lpm config list` | Print all saved values | You want to inspect the configuration file |
...
`get` and `list` show saved values. They do not calculate built-in defaults or project overrides.
...
Use `lpm config set` for user defaults that have no focused editor.
...
Use the focused command for `sandbox`, `sigstore`, and `firewall`. These settings use nested TOML tables.

If a key requires an array or nested table, edit the file directly. For example, `script-read-allow` requires a TOML array.
...
For a saved key, `get --json` returns the key and its value:
...
If the key is not saved, the current command exits successfully without JSON output.
