...
Use `lpm config` to inspect active settings or save defaults for every project you use with LPM CLI.

```bash
lpm config [ACTION] [KEY] [VALUE] [--set <VALUE>] [--json]

lpm config
lpm config get <KEY>
lpm config set <KEY> <VALUE>
lpm config delete <KEY>
lpm config unset <KEY>
lpm config list
lpm config ls
lpm config <SETTING> [--set <VALUE>]
lpm config release-age-exclude [list]
lpm config release-age-exclude add <SELECTOR>
lpm config release-age-exclude remove <SELECTOR>
```
...
| `lpm config get <key>` | Print one saved value, including a dotted path | You know the configuration key |
| `lpm config set <key> <value>` | Save one top-level value | The key is a scalar or a supported nested alias |
| `lpm config delete <key>` | Remove one saved value or nested field | You want the built-in or project value again |
...
`get` reads a saved value from `~/.lpm/config.toml`. It does not calculate defaults or overrides. Use a dotted path for a nested field:

```bash
lpm config get sandbox.mode
lpm config get policy.extensions.local-feed.command
```

Explicit `get` returns the saved value, including secrets. The `list` and `set` output redact recognized secret values and credential arguments.
...
Omit `--set` to select a value in an interactive terminal. JSON mode and non-interactive shells require `--set`.
...
If the triage editor offers to change script policy, it saves that change with the advisor choice. Cancellation leaves both saved values unchanged.
...
The generic `set` action validates known scalar keys before it writes the file. It stores most values as TOML strings.

For `save-exact`, use `true`, `false`, `1`, `0`, `yes`, or `no`. Other Boolean keys also accept `on`, `off`, `enabled`, and `disabled`. Use lowercase values.

Unknown top-level keys remain available for custom configuration. Empty keys, dotted setters, and scalar writes to known array settings fail without changing the file.
...
The generic action rejects `policy`, `tunnel`, and every dotted key, including `sandbox.mode`. It does not change the file after this error. Edit `~/.lpm/config.toml` directly when you must change an unsupported nested value.
...
If another key requires an array or nested table, edit the file directly. For example, `script-read-allow` and `max-sandbox-write-roots` require TOML arrays.
...
This value applies to workspace mode for [`lpm run`](/docs/dev/run), [`lpm test`](/docs/dev/test), and [`lpm bench`](/docs/dev/bench).
...
A new exclusion requires security approval, even when the cooldown is off. Run this recipe in an interactive terminal.
...
See the [`lpm trust` storage contract](/docs/packages/trust#project-scope-and-storage) for the merge order and workspace behavior.
...
In JSON mode, CI, or a non-interactive shell, the command requires an active global `floor-edit` grant. Otherwise, it fails with `security_approval_required`.

See [`lpm security`](/docs/infra/security#approve-a-short-configuration-session) for a short, approved configuration session.

A managed policy or `force-security-floor` can prohibit changes to lower controls. In this case, the command fails with `security_floor`.

To disable or unset `force-security-floor` itself, obtain an active global `floor-edit` grant first.
See [user configuration](/docs/reference/config-toml#security-sensitive-keys-are-proposals).

Deleting a nested field follows the same checks as changing it. For example, deleting `sandbox.mode` cannot remove a forced strict sandbox setting.

Use [`lpm security status`](/docs/infra/security#status-output) to inspect the active security floor and temporary approvals.
...
lpm config set workspace-concurrency 4 --json
```
...
An active project `scripts-allow` or `scripts-triage` unlock can authorize a weaker project script policy. Its source includes `temporary project unlock`.
...
Remove a saved user value to use the next source in its precedence chain:

```bash
lpm config delete save-prefix
lpm config delete sandbox.mode
```
...
| --- | --- |
| `[ACTION]` | Select a generic action, focused editor, or exclusion command. Omit for the guided editor |
| `[KEY]` | Select the saved key for a generic action, or the exclusion operation |
| `[VALUE]` | Supply a scalar value or exclusion selector |
