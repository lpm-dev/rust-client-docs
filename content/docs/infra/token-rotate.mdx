...
## Do not use an external token for rotation

Do not authenticate this command with `LPM_TOKEN` or `--token`. The command cannot update an environment variable or an external secret store.

It also does not print the replacement token. The external source will contain the invalid old value after rotation.

For CI, rotate the token in the LPM.dev Registry dashboard. Then update `LPM_TOKEN` in the CI secret store.

If you already rotated an active `LPM_TOKEN`, remove it before you verify the stored replacement:
...
| `--token <TOKEN>` | Supply the active token. Do not use this flag for rotation because LPM CLI cannot return the replacement |
