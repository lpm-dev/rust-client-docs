...
Generates a fresh project certificate whose SAN includes the defaults — `localhost`, `127.0.0.1`, `::1`, and `*.local` (the wildcard covers mDNS-style hostnames like `my-app.local`) — plus every hostname passed via `--host`. Pair with a hosts-file entry to develop against a custom name.
