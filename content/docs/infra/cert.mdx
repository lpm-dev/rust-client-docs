...
Manages the local certificate authority (CA) and per-project HTTPS certificates that back [`lpm dev --https`](/docs/dev/dev). One-time `lpm cert trust` installs LPM's CA into your OS trust store; everything else is automated.
