...
```text
[your machine]        [LPM.dev Registry relay — Cloudflare]          [internet]
                                                                          ▼
...
| Per-tunnel rate limit   | **100 req/min** | unlimited | unlimited |
| Global per-IP cap       | 600 req/min/IP  | 600 req/min/IP | 600 req/min/IP |
...
| Relay → client | `hello`         | Handshake response — assigned domain, full `tunnel_url`, `session_id`, plan, expiry, and limits |
...
When the CLI's WebSocket connects to the relay, the `hello` message includes the LPM.dev Registry auth token. The relay:

1. Hashes the token (so it can be cached safely).
2. Looks up `tunnel_auth:{tokenHash}` in KV (60s TTL).
3. On miss: calls back to the LPM.dev Registry origin's `/api/registry/-/worker/validate-token` and `/api/registry/-/whoami` to fetch user identity, plan tier, and org memberships.
