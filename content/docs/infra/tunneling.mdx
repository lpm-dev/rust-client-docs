...
Free-tier tunnels get an **8-character lowercase alphanumeric** subdomain on `lpm.fyi`, like:
...
Generated cryptographically per session via `crypto.getRandomValues(8 bytes) % 36`. Different on every reconnect — the free tier has no reconnect-grace window, so each restart hands you a fresh URL.

These domains are **not stored in any database** — they exist for the lifetime of the WebSocket connection (max 2 hours under the free session cap) and then they're gone.
...
The relay-side validator is broader — it accepts any RFC 1123 hostname label (1–63 chars). The 3–32 cap exists at the claim API to keep human-typed subdomains short and consistent. Random free-tier subdomains use the relay's looser rules and land at exactly 8 chars.
...
```text
First request:
  Browser → k8f2m9x1.lpm.fyi/foo  →  HTML interstitial with "Visit Site" button
...
lpm tunnel inspect --ui        # browser inspector at http://localhost:4400
```
...
`lpm tunnel inspect --ui` starts a small web UI at `http://localhost:4400` (override with `--inspect-port`). It shows captured events with full headers, body, response status, and replay controls. Same data as the terminal `inspect`, just easier to navigate for big captures.

The inspector is auto-started alongside the tunnel by default. Disable with `--no-inspect`.
