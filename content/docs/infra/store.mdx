...
The v2 layout is the default. The file-level CAS in v3 is experimental and is used only when explicitly selected with `LPM_STORE_VERSION=v3`; LPM CLI never selects it automatically from the filesystem or execution environment. Store maintenance commands inspect both layouts regardless of which one the current install process selects.
...
lpm store verify                        # check structure and CAS metadata (fast)
lpm store verify --deep                 # also parse manifests and rehash v3 blobs
...
| `verify` | Integrity-check v1/v2 entries and v3 CAS metadata |
...
```bash
lpm store verify          # fast structural and metadata pass
lpm store verify --deep   # also rehash v3 blobs + parse package.json + check lpm.lock
lpm store verify --fix    # regenerate stale behavioral-analysis caches
```

The fast pass walks every v1, v2, and v3 store entry and confirms that package directories exist, contain `package.json`, and are non-empty. For v2 and v3 link entries, it also checks that each sidecar's source hash maps to the recorded object path and that the referenced object is present. The v3 pass additionally validates source records, tree manifests, blob metadata, materialized-tree caches, and reachability between those CAS tiers without recomputing blob content hashes. Mismatches surface as `corrupted` entries with a remediation hint.

`--deep` extends the pass with these checks:
...
- **Rehashes every v3 CAS blob** and validates its content-plus-normalized-mode identity. Legacy v1/v2 extracted directories retain marker-consistency verification rather than a full expanded-tree content hash.
...
The output distinguishes **entries** (one per directory on disk; may double-count during a store-version migration or under cross-project graph-key splits where the same `name@version` legitimately lives at multiple paths) from **unique coordinates** (deduped on `name@version`). Both counts appear in human + JSON output so an in-flight migration doesn't look like an inflated package count. CAS source, tree, blob, and materialization counts are reported separately.
...
Removes `~/.lpm/store/v1/`, `~/.lpm/store/v2/`, and `~/.lpm/store/v3/` in their entirety — every extracted package, link entry, CAS blob, manifest, materialized tree, and cached integrity sidecar. The outer `~/.lpm/store/` directory plus the `.gc.lock*` control files stay so the next operation doesn't have to reinitialize them. The next `lpm install` rebuilds whatever it needs.
...
lpm store clean --json    # bytes freed across v1 + v2 + v3 subdirectories
```
...
| `legacy_check_kind` | Scope for v1/v2 extracted entries: `presence` for the fast pass, `lockfile_marker_consistency` for `--deep` or `--fix` |
| `check_kind` | Compatibility alias of `legacy_check_kind` |
| `bytes_integrity_recomputed` | `true` when a deep pass rehashed at least one v3 blob; otherwise `false` |
...
| `unique_coords` | Deduped count over `name@version` — different from `entries_verified` during store-version migration |
...
| `cas.check_kind` | `metadata` for the fast pass; `content_hash` for `--deep` or `--fix` |
| `cas.sources` / `trees` / `blobs` / `materialized` | Number of v3 CAS entries inspected in each tier |
| `cas.blobs_rehashed` / `blob_integrity_recomputed` | Blob content-hash work performed by this pass |
| `cas.orphaned_sources` / `orphaned_trees` / `orphaned_blobs` / `orphaned_materialized` | Structurally valid v3 entries not reachable from a live source/tree relationship |

`clean` envelope:

| Field | Meaning |
| --- | --- |
| `removed_bytes` / `removed` | Total bytes / formatted total across `v1` + `v2` + `v3` |
| `v1_removed_bytes` / `v2_removed_bytes` / `v3_removed_bytes` | Per-subdirectory breakdown |
| `v1_path` / `v2_path` / `v3_path` | Absolute paths that were removed |
| `path` | Legacy alias of `v1_path` retained for older consumers — prefer the versioned path fields |

## Flags

| Flag | Applies to | Effect |
| --- | --- | --- |
| `--deep` | `verify` | Also parse package.json, validate name/version, cross-check lockfile integrity markers, and rehash v3 CAS blobs |
