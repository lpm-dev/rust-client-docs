...
invocations from performing the same native build twice. See
...
Native build-cache work adds a narrower lock beneath the shared store lock:
`build-locks/<build-key>.lock`. Different keys can build concurrently. Two
processes requesting the same key serialize, and the waiter checks the
artifact again after acquiring the lock before deciding whether to execute.
