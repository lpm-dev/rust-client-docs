...
LPM CLI cannot unset environment variables in its parent shell. If a non-empty `LPM_TOKEN` remains active, logout still clears the stored LPM.dev session but exits non-zero, reports `success: false` and `local_cleared: false`, and tells you to unset `LPM_TOKEN` in that shell or CI environment.
