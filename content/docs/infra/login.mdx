...
| GitHub Packages | `lpm login --github` | You use GitHub CLI authentication or need a saved fallback token |
| GitLab Packages | `lpm login --gitlab` | You use GitLab CLI authentication or need a saved fallback token |
...
LPM CLI can use an existing `gh` or `glab` session:
...
These login commands check the host CLI session. LPM CLI does not save another token when the check succeeds.

Save a fallback token when `gh` or `glab` is not available:
...
In CI, package commands can use environment tokens directly:
...
These variables authenticate package operations. They do not satisfy `lpm login --github` or `lpm login --gitlab`.
