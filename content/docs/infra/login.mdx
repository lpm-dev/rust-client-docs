...
When the current LPM.dev Registry login has a refresh-backed CLI session, `lpm logout` also makes a best-effort request to revoke its browser vault pairings. A pairing cleanup failure does not prevent local logout.

Local logout does not invalidate the server-side CLI session. To revoke that session and its refresh capability, open [Security settings](https://lpm.dev/dashboard/settings/security) and remove the device from the active sessions list.
