...
| Custom npm-compatible registry | `lpm login --login-registry <URL>` | You need a token for one registry URL and path |
...
Keep usernames and passwords outside the URL. For install authentication, use [registry-scoped `.npmrc` fields](/docs/registries#keep-credentials-outside-registry-urls).
