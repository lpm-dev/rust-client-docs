...
A headless Linux upgrade preserves an existing encrypted-file login when its saved key can decrypt the credential store. Keep both `~/.lpm/.credentials` and `~/.lpm/.key`. A keyring outage does not authorize a stale file token or restore a revoked login.
