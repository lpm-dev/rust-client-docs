...
Publication uses the first available source in each row.

| Registry or target | Credential priority |
| --- | --- |
| LPM.dev Registry | `--token` → `LPM_TOKEN` → stored access token |
| npm (`registry.npmjs.org`) | `NPM_TOKEN` → token from `lpm login --npm` → effective npm configuration |
...
Npm-compatible package reads, including search and info, use matching `.npmrc` credentials. Stored publish logins and bare `NPM_TOKEN` do not supply these credentials.
...
For self-managed GitLab, store the token for the full package endpoint, such as `https://gitlab.example.com/api/v4/projects/12345/packages/npm`. Use `lpm login --login-registry <URL> --token <token>`.
...
| --- | --- |
| macOS | Data Protection Keychain through official signed LPM CLI builds |
...
## macOS protected credentials

Official signed LPM CLI releases use a restricted Keychain access group. Public source code does not grant access to that group.
Unsigned builds and third-party builds cannot read its credentials.

An upgrade migrates each older credential when LPM CLI first needs it. The migration checks the recorded credential identity before it copies the value. After the shared copy passes verification, the migration removes the old copy. Interrupted cleanup resumes on the next read.

After migration, older LPM CLI versions cannot read the shared login. Keep LPM CLI current. If a credential fails verification, update the CLI and sign in again with [`lpm login`](/docs/infra/login).

A source build can use its own login or an environment token. It cannot share the official Keychain group.
