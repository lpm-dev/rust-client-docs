...
`lpm setup ci npmrc` and explicit `lpm swift-registry` commands use this refresh path before they copy a token to another client. A refresh failure stops either command.
