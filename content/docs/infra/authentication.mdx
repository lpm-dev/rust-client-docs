...
### A default npm operation ignores the `.npmrc` token on Unix

LPM CLI refuses the default npm-token fallback from an `.npmrc` file with permissions more open than `0600`. Restrict the file permissions:
