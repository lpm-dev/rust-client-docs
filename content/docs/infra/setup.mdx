...
The same OIDC signal can authenticate npm firewall checks for `lpm install`, `lpm ci`, `lpm fetch`, `lpm download`, and `lpm add` when `LPM_NPM_FIREWALL=monitor|enforce` is active. If `LPM_TOKEN` or stored LPM auth is already available, firewall checks use that and skip OIDC exchange.
