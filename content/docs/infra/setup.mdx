...
Write project auth files or print CI snippets for installing private LPM.dev Registry packages and pulling encrypted environment files.
...
The same OIDC signal can authenticate LPM Firewall checks for `lpm install`, `lpm ci`, `lpm fetch`, `lpm download`, and `lpm add` when `LPM_NPM_FIREWALL=monitor|enforce` is active. If `LPM_TOKEN` or stored LPM.dev Registry auth is already available, firewall checks use that and skip OIDC exchange.
