...
| `--filter <NAME>` | Only show subtrees that contain this package name. Diamond patterns (two parents to the same target) keep both branches. |
...
| `--dev` | devDependencies only |
| `--no-open` | With `--format html`: write the file but skip the auto-open call. Useful in headless / CI environments. Used without `--format html`, prints a one-line warning that the flag has no effect. |
