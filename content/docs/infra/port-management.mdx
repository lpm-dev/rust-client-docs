...
The readiness wait uses `readyTimeout`, with a default of 30 seconds.

If the process still runs after a readiness timeout, LPM CLI prints a `not ready` warning. It then continues the startup graph.

This warning state currently lets dependent services start. It does not stop the development run.
...
Independent branches start together. Dependent services start after the preceding dependency level reaches ready or readiness-warning state.
...
### A dependent starts after a readiness warning

The current readiness timeout is not fatal. LPM CLI marks the service ready for graph progress while it keeps the process active.
