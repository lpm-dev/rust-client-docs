...
If the bind fails, LPM reports who's holding the port. On macOS / Linux, `lsof -ti :{port}` produces the PID and process name. LPM does **not** kill that process, but it also doesn't stop there: it picks the next available port, persists that remap for the current project in `~/.lpm/ports.toml`, and continues startup with the reassigned value.
...
Reads `lpm.json > services`, then for each declared `port` runs the same `TcpListener::bind` check as the conflict detector. Prints one row per service with the declared port, whether it's `free` or `in use`, and (when in use) the owning PID + process name. Services with only `readyPort` and no declared `port` are omitted.
