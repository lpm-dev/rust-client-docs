...
| `lpm policy test <EXTENSION> --package <PACKAGE>` | Run one extension with a synthetic package candidate | You want to see its decision before an install |
...
Policy extensions use local user configuration. They are not part of the signed security floor or an OS-managed policy.

## Configuration

Configure policy extensions in `~/.lpm/config.toml`. The path follows [`LPM_HOME`](/docs/reference/env-vars#filesystem-layout).
...
`warn` is an alias for report mode. `deny` and `block` are aliases for enforcement mode.

A command can contain at most 64 entries, including the executable. Disabled extensions skip the other value checks.

There is no command-line override or one-run opt-out for these extensions. To disable an extension, set its `enabled` value to `false`.
...
This configuration is not part of the signed security floor. Protect write access to `~/.lpm/config.toml` with your operating-system permissions.
...
List output redacts recognized credential arguments and URL credentials in both terminal and JSON output. Stored arguments and extension execution remain unchanged.
...
Use a valid package name and one npm-compatible concrete version. LPM CLI rejects malformed names before it starts the extension.

LPM CLI converts accepted loose versions to a canonical version. For example, `v19.0.0` and `19.0` become `19.0.0`.
...
The timeout includes the standard-input write, process exit, output reads, and response validation. Output limits are 64 KiB for standard output and 16 KiB for standard error.
...
For both commands, `success: true` means that the command produced a diagnostic response. Use `no_failures` and the process exit code for policy health.
...
| `test` | `--package <PACKAGE>` | Send a candidate such as `react@19.0.0` to the extension |
