...
| Cargo | Runs a version-pinned Cargo install | Returns the command and does not run it |
...
| Cargo | `cargo install --git https://github.com/lpm-dev/rust-client --tag v<version> lpm-cli --force --locked` |
...
Disable new background lookups for a command with this environment variable:

```bash
LPM_NO_UPDATE_CHECK=1 lpm install
```

`LPM_NO_UPDATE_CHECK` does not disable an explicit `lpm self-update`. It also does not suppress a notice that already exists in the cache.
...
2. It verifies that the signature time matches the GitHub release time.
3. It verifies the checksum of the manifest.
4. It verifies the downloaded binary against the manifest.
