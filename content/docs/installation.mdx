...
Works on every supported platform, including Windows and x64 Alpine Linux. The npm package is a dependency-free launcher; npm installs the matching platform-native Rust package through `optionalDependencies`, distinguishes glibc from musl on Linux x64, then runs a small `postinstall` verifier that checks the native binary and wires the global command to it.
...
Downloads the latest binary into `~/.lpm/bin` and adds it to your `PATH` (`.zshrc`, `.bashrc`, or `config.fish` depending on your shell). On Linux x64, it detects glibc or musl and selects the matching release binary automatically. Open a new terminal — or `source` the rc file — and you're done.

The installer verifies the downloaded binary against the signed checksums manifest before installing it. It also refuses releases below the installer's minimum security floor. Two recovery overrides exist for release incidents and broken bootstrap environments: `LPM_INSTALL_INSECURE=1` skips all integrity checks, while `LPM_INSTALL_MIN_VERSION_OVERRIDE=1` permits an older release but still verifies its checksum/signature.
